...
function containsMyName(args)
  local myName = GetResourceState("xcel") == "started" and string.lower(exports["xcel"]:GetPlayerName(PlayerId())) or GetPlayerName(PlayerId())
  local message = ""
  for i = 2, #args do
...
AddEventHandler('chatMessage', function(author, color, text, msgType, pfp)
  if not hideChat then
    local args = { text }
    if author ~= "" then
      table.insert(args, 1, author)
    end
   -- if GetResourceState("xcel") == "started" and exports["xcel"]:getChatPfpDisplay() == false then pfp = nil end
    SendNUIMessage({
      type = 'ON_MESSAGE',
      message = {
      msgType = msgType,
...
AddEventHandler("XCEL:hideChat",function(flag)
  hideChat = flag
  if hideChat then
...
AddEventHandler("XCEL:chatReduceOpacity", function(flag)
