let currentSuggestionIndex = -1; // Initialize suggestion index
window.APP = {
  ...
  methods: {
    ...
    keyDown(e) {
      if (e.which === 38 || e.which === 40) {
        ...
        // Tab complete chat suggestions
        e.preventDefault();
        const input = this.$refs.input;
        let suggestion = this.suggestions.find(s => s.name.startsWith(input.value));
        if (suggestion) {
          if (currentSuggestionIndex === -1) {
            currentSuggestionIndex = this.suggestions.indexOf(suggestion);
          } else {
            currentSuggestionIndex = (currentSuggestionIndex + 1) % this.suggestions.length;
            suggestion = this.suggestions[currentSuggestionIndex];
          }
          this.message = suggestion.name;
        }
      }
    },
