...
RegisterNetEvent(
    ...
    function()
        ...
        TriggerServerEvent("XCEL:playTaserSound", GetEntityCoords(tXCEL.getPlayerPed()), "arcsound")
        b = false
        SetTimeout(
            4000,
            function()
...
RegisterNetEvent(
    ...
    function(f, g)
        local h = GetEntityCoords(tXCEL.getPlayerPed(), true)
        local i = #(h - f)
        if i <= 15 then
...
RegisterNetEvent(
    "XCEL:reloadTaser",
    function(j)
        if GetSelectedPedWeapon(tXCEL.getPlayerPed()) == GetHashKey(a.hash) then
            MakePedReload(tXCEL.getPlayerPed())
            if a.cartridges == 0 then
                a.cartridges = 2
                if a.reactivate1.active then
                    TriggerServerEvent("XCEL:barbsNoLongerServer", a.reactivate1.serverId)
                end
                if a.reactivate2.active then
                    TriggerServerEvent("XCEL:barbsNoLongerServer", a.reactivate2.serverId)
                end
                a.reactivate1 = {active = false, ped = 0, serverId = 0}
                a.reactivate2 = {active = false, ped = 0, serverId = 0}
        ...
        if k.weapon == GetHashKey(a.hash) then
            if IsControlJustPressed(1, 308) then
                if a.reactivate1.active == true then
                    ...
                    TriggerServerEvent("XCEL:playTaserSound", k.playerCoords, "reactivate")
                    TriggerServerEvent("XCEL:reactivatePed", a.reactivate1.serverId)
                end
            elseif IsControlJustPressed(1, 307) then
                if a.reactivate2.active == true then
                    ...
                    TriggerServerEvent("XCEL:playTaserSound", k.playerCoords, "reactivate")
                    TriggerServerEvent("XCEL:reactivatePed", a.reactivate2.serverId)
                end
            elseif IsControlJustPressed(1, 121) and b then
                TriggerServerEvent("XCEL:arcTaser")
            end
            if a.cartridges < 1 then
                ...
                if IsPedShooting(k.playerPed) then
                    TriggerServerEvent("XCEL:playTaserSound", k.playerCoords, "taser")
                    a.cartridges = a.cartridges - 1
                    local l, m = GetEntityPlayerIsFreeAimingAt(k.playerId)
                    if l and IsEntityAPed(k.playerPed) then
                        local n = tXCEL.getPedServerId(m)
                        TriggerServerEvent("XCEL:receiveBarbsServer", n)
                        if n ~= nil then
                            if a.cartridges == 1 then
...
tXCEL.createThreadOnTick(func_taserHandler)
...
Citizen.CreateThread(
    function()
        SetPedMinGroundTimeForStungun(tXCEL.getPlayerPed(), 15000)
        while true do
            if IsPedBeingStunned(tXCEL.getPlayerPed()) then
                stunGun()
            end
            Wait(100)
        end
            ...
            if c then
                while c do
                    ...
                    if IsControlJustPressed(1, 38) then
                        c = false
                        if d and d > 0 then
                            TriggerServerEvent("XCEL:barbsRippedOutServer", d)
                        end
                        d = 0
...
function tXCEL.setStunExitsEarly()
    p = true
end
