            ...
            ontick_fishing = function(y)
                if globalFishingOnDuty then
                    if
                     ...
                     then
                        TriggerServerEvent("XCEL:fishingJobChecks", y.type)
                    end
                    if IsControlJustPressed(0, 47) then
                        local z = GetVehiclePedIsIn(PlayerPedId(), true)
                        tXCEL.notify("~y~Boat anchor set to " .. tostring(not x))
...
function func_fishingStatus(y)
    ...
    elseif g and c then
        if IsControlJustPressed(0, 102) then
            ...
            if d >= e and d <= f then
                TriggerServerEvent("XCEL:fishingCaughtFish", d)
            else
                TriggerServerEvent("XCEL:fishingCaughtFish", d)
...
tXCEL.createThreadOnTick(func_fishingStatus)
RegisterNetEvent(
    "XCEL:fishingJobSetCasted",
    function()
        g = true
...
tXCEL.createThreadOnTick(func_createBar)
