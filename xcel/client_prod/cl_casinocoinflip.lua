...
Citizen.CreateThread(
    function()
        Wait(1000)
        TriggerServerEvent("XCEL:requestCoinflipTableData")
        for q = 0, #c.coinflipTables do
            local D = c.coinflipTables[q]
            local E = tXCEL.loadModel(D.tableModel)
            ...
            if d then
                ...
                if
                    not l and IsControlJustPressed(0, 202) and not g and
                        not RageUI.Visible(RMenu:Get("xcelcoinflip", "confirm"))
                 then
                    local L = tXCEL.loadAnimDict("anim_casino_b@amb@casino@games@shared@player@")
                    TaskPlayAnim(tXCEL.getPlayerPed(), L, "sit_exit_left", 1.0, 1.0, 2500, 0, 0.0, false, false, false)
                    RemoveAnimDict("anim_casino_b@amb@casino@games@shared@player@")
                    d = false
                    e = true
                    showHowToCoinflip(false)
                    TriggerServerEvent("XCEL:cancelCoinflip", true)
                    TriggerServerEvent("XCEL:leaveCoinflipTable")
                    h = {}
                    m = false
                    SetTimeout(
                        5000,
            ...
            if not d then
                if a ~= -1 and b < 2 then
                    if IsControlJustPressed(0, 38) then
                        if f[a] == false then
                            TriggerServerEvent("XCEL:requestSitAtCoinflipTable", a)
                        end
                    end
                end
            end
...
function goToCoinflipSeat(o)
    ...
    TaskGoStraightToCoord(tXCEL.getPlayerPed(), S.x, S.y, S.z, 1.0, 5000, T.z, 0.01)
    Wait(500)
    local U = v(o)
    local V = A(o)
...
function showHowToCoinflip(X)
    ...
    RageUI.Visible(RMenu:Get("xcelcoinflip", "instructions"), X)
end
function showCoinflipMenu(X)
    RageUI.ActuallyCloseAll()
    RageUI.Visible(RMenu:Get("xcelcoinflip", "mainmenu"), X)
end
RMenu.Add(
    "xcelcoinflip",
    "instructions",
...
RMenu.Add(
    ...
    RageUI.CreateSubMenu(
        RMenu:Get("xcelcoinflip", "mainmenu"),
        "",
        "~b~COINFLIP",
        tXCEL.getRageUIMenuWidth(),
        tXCEL.getRageUIMenuHeight(),
...
RageUI.CreateWhile(
    ...
    function()
        RageUI.IsVisible(
            RMenu:Get("xcelcoinflip", "instructions"),
            true,
            false,
            true,
            function()
        ...
        RageUI.IsVisible(
            ...
            function()
                RageUI.ButtonWithStyle(
                    ...
                    function(Y, Z, _)
                        if _ then
                            local a0 = tXCEL.KeyboardInput("Bet Amount", "", 30)
                            if tonumber(a0) then
                                a0 = tonumber(a0)
                                if a0 > 0 then
                                    TriggerServerEvent("XCEL:proposeCoinflip", a0)
                                end
                            end
                        end
                    end
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(Y, Z, _)
                                if _ then
                                    ...
                                    if M.user_id == tXCEL.getUserId() then
                                        k = true
                                    else
                                        k = false
                                    end
                            ...
                            RMenu:Get("xcelcoinflip", "confirm")
                        )
                    end
                else
            ...
            function()
                ...
                if k then
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(Y, Z, _)
                            if _ then
                                TriggerServerEvent("XCEL:cancelCoinflip")
                            end
                        end,
                        RMenu:Get("xcelcoinflip", "mainmenu")
                    )
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(Y, Z, _)
                            if _ then
                                TriggerServerEvent("XCEL:acceptCoinflip", i)
                            end
                        end,
                        RMenu:Get("xcelcoinflip", "mainmenu")
                    )
...
RegisterNetEvent(
    "XCEL:addCoinflipProposal",
    function(a1, M)
        h[a1] = M
    end
)
RegisterNetEvent(
    "XCEL:cancelCoinflipBet",
    function(a1)
        h[a1] = nil
    end
)
RegisterNetEvent(
    "XCEL:clearCoinflipBets",
    function(a1)
        h = {}
    end
)
RegisterNetEvent("XCEL:coinflipOutcome",function(a4, a5)
        l = true
        ...
        SetTimeout(10000,function()
                a7 = true
                if a4 then
                    tXCEL.notify("~g~ You won £" .. getMoneyStringFormatted(a5.amount) .. "!")
                    SendNUIMessage({transactionType = "playCasinoWin"})
                else
                    tXCEL.notify("~r~ You lost £" .. getMoneyStringFormatted(a5.amount) .. "!")
