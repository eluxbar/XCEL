...
RageUI.CreateWhile(
    ...
    function()
        if RageUI.Visible(RMenu:Get("loginrewards", "mainmenu")) then
            RageUI.DrawContent(
                {header = true, glare = true, instructionalButton = false},
                function()
                    ...
                    RageUI.Button(
                        ...
                        function(p, p, u)
                            if u then
                                TriggerServerEvent("XCEL:claimNextLoginReward")
                                b = b + 1
...
RegisterNetEvent(
    "XCEL:setDailyRewardInfo",
    function(w, x, y)
        b = w
        c = GetGameTimer() + (86400 - (y - x)) * 1000
        if w == 0 then
            Citizen.Wait(20000)
            tXCEL.notify("~y~You have not started your daily reward. Do /dailyrewards to start now!")
        end
    end
)
RegisterCommand(
...
Citizen.CreateThread(
    function()
        ...
        while true do
            if r() and b > 0 then
                tXCEL.notify("~g~You can claim your next daily reward at /dailyrewards!")
                break
            end
            Citizen.Wait(6000)
        end
