...
RegisterCommand(
    "getcasinotable",
    function()
        local K = GetEntityCoords(tXCEL.getPlayerPed())
        ...
        for O = 0, #G.blackjackTables, 1 do
            ...
            tXCEL.createDynamicPed(
                ...
                function(Q)
                    ...
                    tXCEL.loadAnimDict(dealerAnimDict)
        ...
        while true do
            ...
            if h then
                if IsDisabledControlJustPressed(0, 22) then
                    local W = tXCEL.KeyboardInput("Bet Amount", "", 30)
                    if tonumber(W) then
                        W = tonumber(W)
                        if W > 0 then
                ...
                if IsControlJustPressed(0, 201) then
                    if tonumber(z) >= 0 then
                        ...
                        elseif isChairHighRollers(F) and z < B then
                            tXCEL.notify("~r~Minimum bet is " .. tostring(getMoneyStringFormatted(B)))
                        elseif z > 100000000000 then
                            tXCEL.notify("~r~Maximum bet is £1,000,000,000")
                        else
    ...
    function()
        while true do
            if sittingAtBlackjackTable and A then
                SetPedCapsule(tXCEL.getPlayerPed(), 0.2)
                if IsControlJustPressed(0, 202) and not i then
                    ...
                    TaskPlayAnim(tXCEL.getPlayerPed(), blackjackAnimDictToLoad, "sit_exit_left", 1.0, 1.0, 2500, 0)
...
RMenu:Get("xcelblackjack", "instructions"):SetSubtitle("Blackjack Table")
RMenu.Add(
    "xcelblackjack_high",
    "instructions",
    RageUI.CreateMenu("", "test", 1300, 50, "casinoui_cards_blackjack_high", "casinoui_cards_blackjack_high")
)
RMenu:Get("xcelblackjack_high", "instructions"):SetSubtitle("High Rollers Table")
RageUI.CreateWhile(
    1.0,
    true,
    function()
        if RageUI.Visible(RMenu:Get("xcelblackjack", "instructions")) then
            RageUI.DrawContent(
                ...
                function()
                    RageUI.FakeButton(
                        ...
                        RMenu:Get("xcelblackjack", "instructions")
                    )
                end,
                function()
                end
        ...
        if RageUI.Visible(RMenu:Get("xcelblackjack_high", "instructions")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = true},
                function()
                    RageUI.FakeButton(
                        ...
                        RMenu:Get("xcelblackjack_high", "instructions")
                    )
                end,
                function()
                end
...
function showHowToBlackjack(a3)
    ...
    if not isChairHighRollers(a) then
        RageUI.Visible(RMenu:Get("xcelblackjack", "instructions"), a3)
    else
        RageUI.Visible(RMenu:Get("xcelblackjack_high", "instructions"), a3)
    end
end
...
function goToBlackjackSeat(an)
    ...
    c = an
    fVar3 = blackjack_func_217(tXCEL.getPlayerPed(), blackjack_func_218(c, 0), 1)
    fVar4 = blackjack_func_217(tXCEL.getPlayerPed(), blackjack_func_218(c, 1), 1)
    fVar5 = blackjack_func_217(tXCEL.getPlayerPed(), blackjack_func_218(c, 2), 1)
    ...
    TaskGoStraightToCoord(tXCEL.getPlayerPed(), ao.x, ao.y, ao.z, 1.0, 5000, ap, 0.01)
...
function getNewCardFromMachine(nextCard, Y, ab)
    local aR = tXCEL.loadModel(nextCard)
