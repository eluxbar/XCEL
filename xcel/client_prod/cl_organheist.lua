...
AddEventHandler(
    ...
    function(p, q)
        if q then
            tXCEL.addMarker(b.x, b.y, b.z, 0.7, 0.7, 0.5, 0, 125, 255, 125, 150, 20, false, false, true)
            tXCEL.addBlip(b.x, b.y, b, 378, 3, "Organ Heist Police")
            tXCEL.addMarker(c.x, c.y, c.z, 0.7, 0.7, 0.5, 255, 0, 0, 125, 150, 20, false, false, true)
            tXCEL.addBlip(c.x, c.y, c.z, 378, 1, "Organ Heist Criminals")
        end
    end
)
    ...
    function()
        while true do
            if f then
                ...
                if IsControlJustPressed(0, 38) then
                    if not globalOnPoliceDuty then
                        if not g then
                            TriggerServerEvent("XCEL:joinOrganHeist")
                            Wait(100)
                        else
                            tXCEL.notify("~r~Already joined the Organ Heist!")
                        end
                    else
                        tXCEL.notify("~r~You are police, please use the other entrance!")
                    end
                end
            elseif e then
                ...
                if IsControlJustPressed(0, 38) then
                    if globalOnPoliceDuty then
                        ...
                        tXCEL.notify("~r~You are a civillian, please use the other entrance!")
                    end
                end
            end
            ...
            if inGamePhase or inWaitingStage then
                ...
                RemoveWeaponFromPed(tXCEL.getPlayerPed(), "WEAPON_MOLOTOV")
                RemoveWeaponFromPed(tXCEL.getPlayerPed(), "WEAPON_FLASHBANG")
                RemoveWeaponFromPed(tXCEL.getPlayerPed(), "WEAPON_SMOKEGRENADEFR")
                RemoveWeaponFromPed(tXCEL.getPlayerPed(), "WEAPON_SMOKEGRENADEFRPD")
                RemoveWeaponFromPed(tXCEL.getPlayerPed(), "WEAPON_BZGAS")
                RemoveWeaponFromPed(tXCEL.getPlayerPed(), "WEAPON_STUNGUN")
                local r = PlayerPedId()
...
RegisterNetEvent("XCEL:organHeistKillConfirmed",function(C,Amount)
    PlaySoundFrontend(-1, "Weapon_Upgrade", "DLC_GR_Weapon_Upgrade_Soundset", true)
    tXCEL.playScreenEffect("MP_Celeb_Win", 0.25)
    tXCEL.notify("~g~Killed " .. C .. " received £"..getMoneyStringFormatted(Amount).." and 25% Armour")
end)
local function D()
    local E = 1000.0
    local F = 0.0
        ...
        while true do
            if inWaitingStage then
                ...
                if E < 3.0 then
                    ...
                    NetworkFadeInEntity(PlayerPedId(), 0)
                    DoScreenFadeIn(1000)
                    tXCEL.notify("~r~You got too far from the organ heist and have been teleported back.")
                end
            end
            if inGamePhase then
                local l, F = D()
...
RegisterNetEvent(
    ...
    function()
        ...
        inGamePhase = false
        d = 600
        tXCEL.deleteAtm("Organ Heist")
        for l, A in pairs(k.gunStores[h]) do
            tXCEL.deleteGunStore(A[1])
        end
        h = ""
        k = nil
...
function tXCEL.setDeathInOrganHeist()
    inOrganHeist = false
    inWaitingStage = false
    inGamePhase = false
end
RegisterCommand(
    ...
    function()
        if isInGreenzone then
            TriggerServerEvent("XCEL:TPToOrganHeist")
        else
