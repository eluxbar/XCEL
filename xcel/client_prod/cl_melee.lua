...
RegisterCommand(
    "togglemelee",
    function()
        if tXCEL.getUserId() == 0 or tXCEL.getUserId() == -1 then
            c = not c
            tXCEL.notify("Melee Combat: " .. (c and "~g~Enabled" or "Disabled"))
        end
    end
)
function tXCEL.enablePunching(d)
    c = d
end
Citizen.CreateThread(
    function()
        tXCEL.enablePunching(false)
        while true do
            local e = PlayerPedId()
            ...
            if
                h == `WEAPON_UNARMED` or h == GetHashKey("WEAPON_STAFFGUN") or
    ...
    function(h, i)
        if i then
            ...
            for n, o in pairs(a) do
                tXCEL.createArea("boxing_" .. n, o.pos, o.radius, 6, j, l, m, {})
            end
        end
    end
)
    ...
    function(h, i)
        if i then
            ...
            for n, u in pairs(t) do
                tXCEL.addMarker(u.x, u.y, u.z, 1.0, 1.0, 1.0, 0, 0, 255, 100, 50, 27, false, false, true)
                tXCEL.createArea(n .. "_boxingGloves", u, 1.5, 6, j, l, m, {})
            end
        end
    end
)
...
function putGloves()
    ...
    local x = tXCEL.loadModel("prop_boxing_glove_01")
