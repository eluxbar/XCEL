...
RMenu.Add(
    "emotesmenu",
    "mainmenu",
    RageUI.CreateMenu("", "XCEL Emotes", tXCEL.getRageUIMenuWidth(), tXCEL.getRageUIMenuHeight(),"banners", "emotes")
)
RMenu.Add(
    "emotesmenu",
    "emotes",
...
local function p(q)
    ...
    if not q and GetGameTimer() - e < 1500 then
        tXCEL.notify("Emotes are being rate limited.")
        return
    end
    b = false
    f = nil
...
local function F(E)
    if tXCEL.getPlayerVehicle() ~= 0 then
        tXCEL.notify("Can not use scenarios whilst in a vehicle.")
        return
    end
    local A = PlayerPedId()
    ...
    elseif E[1] == "MaleScenario" then
        if tXCEL.getModelGender() == "male" then
            ...
            tXCEL.notify("This scenario is male only.")
        end
...
local function a7(E)
    if not tXCEL.canAnim() or tXCEL.isHandcuffed() then
        return true
    end
    local A = PlayerPedId()
    ...
    local a8 = tXCEL.getPlayerVehicle()
...
local function a6(E)
    if a7(E) then
        tXCEL.notify("Can not this emote at this time.")
        return
    end
...
local function aa()
    ...
    local a9 = -1
    local ab = 2.0
    local ac = tXCEL.getPlayerCoords()
...
local function ag(ah)
    if a7() then
        tXCEL.notify("Can not use shared emotes at this time.")
        return
    end
    local ai = aa()
    if ai ~= 0 then
        i = ah
        j = ai
        TriggerServerEvent("XCEL:sendSharedEmoteRequest", ai, ah)
    else
        tXCEL.notify("No player is near by.")
    end
end
local function aj(E)
    ...
    function()
        ...
        if RageUI.Visible(RMenu:Get("emotesmenu", "searchemotes")) then
            RageUI.DrawContent(
                ...
                function()
                    if az == false then
                        ay = tXCEL.KeyboardInput("Enter Emote Name", "", 10)
                        if ay == nil then
                            ay = ""
                        end
                    end
...
local function aH(aE, aF, aG)
    if #aF < 1 then
        tXCEL.notify("No emote name was specified.")
        return
    end
    ...
    elseif a.guns[ah] then
        ...
        tXCEL.notify("~r~Invalid emote name was specified.")
    end
end
...
RegisterCommand(
    ...
    function(aE, aF, aG)
        if #aF < 1 then
            tXCEL.notify("No walk name was specified.")
            return
        end
        local ah = aF[1]
        if not ah then
        ...
        if a.walks[ah] then
            ...
            tXCEL.notify("~r~Invalid walk name was specified.")
        end
    end
)
RegisterNetEvent(
    "XCEL:sendSharedEmoteRequest",
    function(aI, ah)
        if a.shared[ah] and not a7() then
            k = ah
            l = aI
            m = GetGameTimer()
            tXCEL.notify("~y~Y~w~ to accept, ~r~L~w~ to refuse (~g~" .. a.shared[ah][3] .. "~w~)")
        end
    end
)
RegisterNetEvent(
    "XCEL:receiveSharedEmoteRequest",
    function(ah)
        p(false)
        Citizen.Wait(300)
        a6(a.shared[ah])
...
RegisterNetEvent(
    "XCEL:receiveSharedEmoteRequestSource",
    function()
...
local function aV()
    if k then
        ...
        elseif IsControlJustPressed(1, 246) then
            if a7() then
                tXCEL.notify("You can not use emotes at this time.")
            else
                TriggerServerEvent("XCEL:receiveSharedEmoteRequest", l, a.shared[k][4])
            end
            aM()
        end
    end
    if b and f and g then
        ...
        if not IsEntityPlayingAnim(PlayerPedId(), f, g, 3) or tXCEL.getPlayerCombatTimer() > 0 then
            p(true)
        end
    end
end
function tXCEL.playEmote(ah)
    if a.emotes[ah] then
        a6(a.emotes[ah])
    elseif a.dances[ah] then
        a6(a.dances[ah])
