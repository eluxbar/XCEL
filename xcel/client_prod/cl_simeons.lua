...
RegisterNetEvent(
    "XCEL:gotCarDealerInstances",
    function(m)
        a = m
    end
)
RegisterNetEvent(
    "XCEL:gotCarDealerCategories",
    function(n)
        b = n
    end
)
RegisterNetEvent(
    "XCEL:carDealerPurchased",
    function()
        DeleteVehicle(f)
        f = 0
        g = 0
        ...
        SetEntityCoords(tXCEL.getPlayerPed(), o.x, o.y, o.z)
        --SetEntityCoords(tXCEL.getPlayerPed(), -54.357288360596,-1108.6693115234,26.438051223755)
    end
)
local function p(q)
    if q ~= nil then
...
local function s(t, u, v, w)
    ...
    if HasModelLoaded(x) then
        local z = CreateVehicle(x, u, v, w, GetEntityHeading(tXCEL.getPlayerPed()), false, false)
        ...
        TaskWarpPedIntoVehicle(tXCEL.getPlayerPed(), z, -1)
        ...
        tXCEL.notify("Could not load vehicle")
        return -1
    end
end
CreateThread(
    function()
        for A, B in pairs(l.simeonsInstances) do
            local C, D, E, F, G, H, I =
                ...
                B.permissionsTable,
                B.categorys
            tXCEL.addBlip(C.x, C.y, C.z, D, E)
            tXCEL.addMarker(C.x, C.y, C.z, 0.7, 0.7, 0.5, G[1], G[2], G[3], 125, 50, F, true, true)
        end
    end
)
Citizen.CreateThread(
...
function isNearPreviewVector(J)
    if #(GetEntityCoords(tXCEL.getPlayerPed()) - J) < 5.0 then
        return true
    end
    return false
end
            ...
            if a ~= nil then
                for A, B in pairs(a) do
                    local C, D, E, F, G, H, I =
                        ...
                        B.permissionsTable,
                        B.categorys
                    c[A] = #(GetEntityCoords(tXCEL.getPlayerPed()) - C)
                end
            end
            Wait(250)
        end
                ...
                function()
                    if d ~= nil then
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(N, O, P)
                                if P then
                                    DeleteVehicle(GetVehiclePedIsIn(tXCEL.getPlayerPed()))
                                    if not k then
                                        RageUI.ActuallyCloseAll()
                                        hash = GetHashKey(j)
                                        ...
                                        if HasModelLoaded(hash) then
                                            ...
                                            testDriveSeconds = 60
                                            local S = GetEntityCoords(tXCEL.getPlayerPed())
                                            DecorSetInt(testDriveCar, decor, 955)
                                            SetModelAsNoLongerNeeded(hash)
                                            TaskWarpPedIntoVehicle(tXCEL.getPlayerPed(), testDriveCar, -1)
                                            setVehicleFuel(testDriveCar, 100)
                                            tXCEL.notify("~g~You have 1 minute to test drive this vehicle!")
                                            for T = 0, 24 do
                                            ...
                                            SetTimeout(
                                                ...
                                                function()
                                                    if k then
                                                        ...
                                                        SetEntityCoords(tXCEL.getPlayerPed(), S.x, S.y, S.z)
                                                        tXCEL.notify("Test drive over!")
                                                        p()
                                                    end
                                                end
                                            )
                                            ...
                                            Citizen.CreateThread(
                                                function()
                                                    while k do
                                                        local U = GetVehiclePedIsIn(tXCEL.getPlayerPed(), false)
                                                        if U ~= nil then
            ...
            RageUI.DrawContent(
                ...
                function()
                    RageUI.ButtonWithStyle(
                        ...
                        function(N, O, P)
                            if P then
                                TriggerServerEvent("XCEL:purchaseCarDealerVehicle", e, j)
                                TriggerServerEvent("XCEL:FetchCars", "Standard Garage")
                            end
                        end,
                        RMenu:Get("cardealer", "mainmenu")
                    )
...
tXCEL.createThreadOnTick(func_previewSimeonsVehicle)
