...
RMenu.Add(
    "scubadiving",
    "main",
    RageUI.CreateMenu("", "", tXCEL.getRageUIMenuWidth(), tXCEL.getRageUIMenuHeight(), "xcel_scubajob", "xcel_scubajob")
)
RMenu:Get("scubadiving", "main"):SetSubtitle("XCEL | ~b~Scuba Diving")
local function j()
...
AddEventHandler(
    ...
    function(l, m)
        if m then
            ...
            tXCEL.addBlip(a.jobPosition.x, a.jobPosition.y, a.jobPosition.z, 308, 0, "Scuba Diving")
            tXCEL.addMarker(
                a.jobPosition.x,
                a.jobPosition.y,
                a.jobPosition.z - 0.2,
            ...
            tXCEL.createArea("scubajob", a.jobPosition, 1.5, 6, q, s, t, {})
        end
    end
)
RageUI.CreateWhile(
                ...
                function()
                    if not e then
                        RageUI.Button(
                            ...
                            function(u, v, w)
                                if w then
                                    TriggerServerEvent("XCEL:requestScubaJob")
                                end
                            end
                        )
                    end
                    if d ~= 0 then
                        RageUI.Button(
                            ...
                            function(u, v, w)
                                if w then
                                    TriggerServerEvent("XCEL:claimScubaReward", c)
                                    d = 0
                                    c = nil
                                    e = false
                                    i = false
                                    ...
                                    tXCEL.setCustomization(h)
                                    local x = PlayerPedId()
                                    SetEnableScuba(x, false)
                                    SetPedMaxTimeUnderwater(x, 10.0)
                                    tXCEL.setMaxUnderWaterUITimenewTime(10.0)
                                end
                            end
                        )
                    end
...
local function y()
    for z, A in pairs(b) do
        tXCEL.removeMarker(A.objectMarker)
        tXCEL.removeArea(A.objectArea)
...
function startScubaDiving()
    tXCEL.loadAnimDict("missheistchem2")
    local x = PlayerPedId()
    ...
    tXCEL.drawNativeNotification("Press ~INPUT_PICKUP~ to start Scuba Diving.")
    while not F do
    ...
    TriggerServerEvent("XCEL:finishScubaJob", c, d)
end
RegisterNetEvent(
    "XCEL:createScubaJob",
    function(J, K)
        c = K
        e = true
        tXCEL.disableCustomizationSave(e)
        DoScreenFadeOut(500)
        Wait(600)
        ...
        SetBlipAlpha(L, 180)
        local q = function(r)
            tXCEL.drawNativeNotification("Press ~INPUT_DETONATE~ to begin diving!")
        end
        local s = function(r)
        end
        local t = function(r)
            if IsControlJustReleased(1, 47) then
                local x = tXCEL.getPlayerPed()
                local M = tXCEL.getPlayerVehicle()
                if M ~= 0 then
                    if GetEntityModel(M) ~= GetHashKey("dinghy") then
                        tXCEL.notify("You must be in a dinghy to start scuba diving!")
                    else
                        tXCEL.removeArea("scubajob_" .. J.name)
                        f = "~y~Search for treasures and return to your boat when finished"
                        createRewardObjects(J)
                        CreateThread(startScubaDiving)
                    end
                else
                    tXCEL.notify("You must be in a boat to start scuba diving!")
                end
            end
        end
        SetWaypointOff()
        local N = tXCEL.addBlip(J.position.x, J.position.y, J.position.z, J.blipId, J.blipColour, "Scuba Diving")
        SetNewWaypoint(J.position.x, J.position.y)
        tXCEL.createArea("scubajob_" .. J.name, J.position, 250, 15, q, s, t, {})
        h = tXCEL.getCustomization()
        local p = math.random(1, 15)
        local O = PlayerPedId()
        ...
        TriggerServerEvent("XCEL:scubaSetVehicle", c, R)
        DoScreenFadeIn(2000)
        SetPedScubaGearVariation(PlayerPedId())
            ...
            function()
                while e do
                    if i then
                        ...
                        SetWaypointOff()
                        TriggerServerEvent("XCEL:finishScubaJob", c, 0)
                        TriggerServerEvent("XCEL:claimScubaReward", c)
                        d = 0
                        c = nil
                        e = false
...
function createRewardObjects(J)
    for z, A in pairs(J.rewardObjects) do
        local T = tXCEL.loadModel(A.objectModel)
        ...
        local q = function(r)
            tXCEL.drawNativeNotification("Press ~INPUT_PICKUP~ to collect!")
        end
        local s = function(r)
        end
        local t = function(r)
            if IsControlJustReleased(1, 38) and e then
                tXCEL.notify("~g~Item collected.")
                d = d + 1
                tXCEL.removeMarker(b[r.objectId].objectMarker)
                tXCEL.removeArea(b[r.objectId].objectArea)
        ...
        tXCEL.createArea("scubajob_" .. J.name .. "_obj_" .. z, A.objectPosition, 3.0, 3.0, q, s, t, {objectId = z})
        b[z] = {}
        b[z].objectId = U
        b[z].objectMarker = V
