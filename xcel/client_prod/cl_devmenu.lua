...
-- to add a weapon just copy the line below and replace the weapon name with the weapon you want to blacklist
-- after you do that make sure to put a comma at the end of the one before it

function isWeaponBlacklisted(weaponName)
    for _, blacklistedWeapon in ipairs(blacklistedWeapons) do
...
RMenu.Add(
    "devmenu",
    "main",
    RageUI.CreateMenu("XCEL Dev Tools", "Developer Tools", tXCEL.getRageUIMenuWidth(), tXCEL.getRageUIMenuHeight())
)
RMenu.Add(
    "devmenu",
    "animations",
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                tXCEL.clientPrompt(
                                    "Customization [JSON]",
                                    json.encode(tXCEL.getCustomization()),
                                    function()
                                    end
                                )
                        ...
                        function(A, B, C)
                            if C then
                                tXCEL.clientPrompt(
                                    ...
                                    "",
                                    function(D)
                                        tXCEL.setCustomization(json.decode(D))
                        ...
                        function(A, B, C)
                            if C then
                                tXCEL.clientPrompt(
                                    ...
                                    "",
                                    function(G)
                                        if G ~= nil and G ~= "" and tonumber(G) then
                                            TriggerServerEvent("XCEL:getArmour", tonumber(G))
                                            tXCEL.notify("~g~Gave Player Armour")
                                        else
                                            tXCEL.notify("~r~Invalid User ID")
                                        end
                                    end
                                )
                            end
                                ...
                                tXCEL.clientPrompt(
                                    ...
                                    function(G)
                                        if G ~= nil and G ~= "" then
                                            ...
                                            if not tXCEL.getUserId() ~= 2 and isWeaponBlacklisted(G) then
                                                notify("~r~This weapon is blacklisted: " .. G)
                                            else
                                                tXCEL.giveWeapons({[G] = {ammo = 250}})
                                ...
                                if string.match(t, I[c]) then
                                    RageUI.ButtonWithStyle(
                                        ...
                                        function(A, B, C)
                                            if C then
                                                u(H)
                                                TaskPlayAnim(tXCEL.getPlayerPed(), H, I[c], 1.0, -1, -1, 1, 0, 0, 0, 0)
                                                RemoveAnimDict(H)
                                            end
                                        end
                                    )
                    ...
                    for c = 1, #K.anims do
                        RageUI.ButtonWithStyle(
                            ...
                            function(A, B, C)
                                if C then
                                    u(K.dict)
                                    TaskPlayAnim(tXCEL.getPlayerPed(), K.dict, K.anims[c], 1.0, -1, -1, 1, 0, 0, 0, 0)
...
RegisterCommand(
    "devmenu",
    function()
        local userId = tXCEL.getUserId()
        local playerName = tXCEL.GetPlayerName(tXCEL.getPlayerId())
        if tXCEL.isDev() then
            TriggerServerEvent("XCEL:SerDevMenu")
        else
            tXCEL.notify("~r~You are not a developer")
        end
    end,
    false
)
RegisterNetEvent("XCEL:CliDevMenu")
AddEventHandler(
    "XCEL:CliDevMenu",
    function()
        if tXCEL.isDev() then
...
function tXCEL.gridPositionSaver(Z, _, a0)
    tXCEL.notify("pick 1st coord, press Y when done")
    ...
    Wait(250)
    tXCEL.notify("pick 2nd coord, press Y when done")
    while not IsControlJustPressed(0, 246) do
        Wait(0)
    end
    local a2 = GetEntityCoords(PlayerPedId())
    Wait(250)
    tXCEL.notify("pick 3rd coord, press Y when done")
    while not IsControlJustPressed(0, 246) do
        Wait(0)
    end
    ...
    for c = 0, a0 - 1, 1 do
        for a5 = 0, _ - 1, 1 do
            ...
            tXCEL.loadModel("a_m_m_fatlatin_01")
            local a9 = XCEL.spawnVehicle(Z, a8.x, a8.y, a8.z, heading, false, false, false)
    ...
    function()
        while true do
            ...
            if a then
                ...
                tXCEL.DrawText(0.32, 0.00, "X:~s~ " .. roundx, 0.4)
                tXCEL.DrawText(0.38, 0.00, "Y:~s~ " .. roundy, 0.4)
                tXCEL.DrawText(0.445, 0.00, "Z:~s~ " .. roundz, 0.4)
                heading = GetEntityHeading(aa)
                roundh = tonumber(string.format("%.2f", heading))
                tXCEL.DrawText(0.50, 0.00, "H:~s~ " .. roundh, 0.4)
                local ab, ac, ad = table.unpack(GetEntityRotation(PlayerPedId(), 1))
                tXCEL.DrawText(0.38, 0.03, "RX:~s~ " .. tonumber(string.format("%.2f", ab)), 0.4)
                tXCEL.DrawText(0.44, 0.03, "RY:~s~ " .. tonumber(string.format("%.2f", ac)), 0.4)
                tXCEL.DrawText(0.495, 0.03, "RZ:~s~ " .. tonumber(string.format("%.2f", ad)), 0.4)
                speed = GetEntitySpeed(PlayerPedId())
                rounds = tonumber(string.format("%.2f", speed))
                tXCEL.DrawText(0.40, 0.92, "Player Speed: ~s~" .. rounds, 0.4)
                health = GetEntityHealth(PlayerPedId())
                tXCEL.DrawText(0.40, 0.95, "Player Health: ~s~" .. health, 0.4)
                camRotX = GetGameplayCamRot(2).x
                tXCEL.DrawText(0.36, 0.88, "CR X: ~s~" .. tonumber(string.format("%.2f", camRotX)), 0.4)
                camRotY = GetGameplayCamRot(2).y
                tXCEL.DrawText(0.44, 0.88, "CR Y: ~s~" .. tonumber(string.format("%.2f", camRotY)), 0.4)
                camRotZ = GetGameplayCamRot(2).z
                tXCEL.DrawText(0.51, 0.88, "CR Z: ~s~" .. tonumber(string.format("%.2f", camRotZ)), 0.4)
                ...
                if IsPedInAnyVehicle(PlayerPedId(), 1) then
                    ...
                    tXCEL.DrawText(0.015, 0.76, "Engine Health: ~s~" .. vehenground, 0.4)
                    tXCEL.DrawText(0.015, 0.73, "Body Health: ~s~" .. vehbodround, 0.4)
                    tXCEL.DrawText(
                        0.015,
                        0.70,
                        "Vehicle Fuel: ~s~" ..
...
function tXCEL.createDebugObject(aN)
    local aO = tXCEL.loadModel(aN)
