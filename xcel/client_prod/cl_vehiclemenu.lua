...
Citizen.CreateThread(
    function()
        while true do
            if m and tXCEL.getPlayerVehicle() ~= k.limitingVehicle then
                m = false
                SetVehicleMaxSpeed(tXCEL.getPlayerVehicle(), k.predictedMax)
                predictedMax = nil
                tXCEL.notify("~r~Vehicle Changed, stopping limiter")
            end
            Wait(500)
        end
    end
    ...
    Citizen.CreateThread(
        function()
            while k.autoDrive and u do
                speed = k.adSpeed
                mode = j[k.adMode]
                local v = GetVehiclePedIsIn(tXCEL.getPlayerPed(), false)
                if v ~= 0 then
                    if IsWaypointActive() then
                        if not l or speed ~= q or mode ~= nil then
                            ...
                            local q = speed
                            local w = mode
                            TaskVehicleDriveToCoord(tXCEL.getPlayerPed(),v,
                                waypoint.x,
                                waypoint.y,
                                waypoint.z,
                                convert(speed),
                    ...
                    tXCEL.notify("~r~You are not in a vehicle!")
                    k.autoDrive = false
                end
                local x = GetIsVehicleEngineRunning(v)
                local inWater = IsEntityInWater(v) and not IsPedInAnyBoat(tXCEL.getPlayerPed())
                u = x and not inWater
                if not x then
                    tXCEL.notify("~r~AutoDrive automatically disabled because the engine is inactive")
                    k.autoDrive = false
                elseif inWater then
                    tXCEL.notify("~r~AutoDrive automatically disabled because you are in water")
                    k.autoDrive = false
                end
                Wait(100)
            end
...
function cruise()
    ...
    Citizen.CreateThread(
        function()
            cruiseVehicle = GetVehiclePedIsUsing(tXCEL.getPlayerPed())
            local y = GetEntitySpeed(cruiseVehicle)
            local u = true
            while k.cruise and IsPedInAnyVehicle(tXCEL.getPlayerPed(), false) and u do
                while not accelerating and k.cruise and u do
                    ...
                    inWater = IsEntityInWater(cruiseVehicle) and not IsPedInAnyBoat(tXCEL.getPlayerPed())
            ...
            if collision then
                tXCEL.notify("~r~Cruise control automatically disabled because a collision was detected")
            elseif not engineStatus then
                tXCEL.notify("~r~Cruise control automatically disabled because the engine was turned off")
            elseif inWater then
                tXCEL.notify("~r~Cruise control automatically disabled because you are in water")
            end
            n = false
            k.cruise = false
        end
...
function acceleratingToNewSpeed(cruiseVehicle)
    while IsControlPressed(1, 71) or IsControlPressed(1, 72) or IsControlPressed(1, 76) or
        GetVehicleCurrentGear(cruiseVehicle) == 0 and IsPedInAnyVehicle(tXCEL.getPlayerPed(), false) do
        Wait(100)
    end
    ...
    accelerating = false
    local cruiseVehicle = GetVehiclePedIsUsing(tXCEL.getPlayerPed())
...
function limiter()
    Citizen.CreateThread(
        function()
            while k.limiter do
                local z = tXCEL.getPlayerVehicle()
...
function waypointNearest(A)
    ...
    local F = GetEntityCoords(tXCEL.getPlayerPed())
    local G = 1000000
    for D = 1, #array, 1 do
        local H = #(F - array[D])
        if H < G then
...
RMenu.Add("vehicle_menu", "main", RageUI.CreateMenu("Vehicle", "", tXCEL.getRageUIMenuWidth(), tXCEL.getRageUIMenuHeight()))
...
RageUI.CreateWhile(
    ...
    function()
                        ...
                        if K then
                            ...
                            if L ~= 0 then
                                if GetVehicleDoorLockStatus(L) ~= 2 then
                                    ...
                                    tXCEL.notify("~w~Doors are now ~r~Locked")
                                else
                                    SetVehicleDoorsLocked(L, 0)
                                    tXCEL.notify("~w~Doors are now ~g~Unlocked")
                                end
                            end
                        end
                    end
                        ...
                        if K then
                            ...
                            if L ~= 0 then
                                ...
                                if M == 9 then
                                    ...
                                    tXCEL.notify("~r~Closed ~w~all doors.")
                                elseif M == 8 then
                                    if r then
                                        r = false
                                        RollDownWindow(L, 0)
                                        RollDownWindow(L, 1)
                                        tXCEL.notify("Rolled windows ~r~Down")
                                    else
                                        r = true
                                        RollUpWindow(L, 0)
                                        RollUpWindow(L, 1)
                                        tXCEL.notify("Rolled windows ~g~Up")
                                    end
                                elseif IsVehicleDoorDamaged(L, N) then
                                    tXCEL.notify("~r~Cannot shut this door when the door is damaged")
                                elseif GetVehicleDoorAngleRatio(L, N) == 0 then
                                    SetVehicleDoorOpen(L, N, false, false)
                                    tXCEL.notify(string.format("The ~b~%s ~w~door is now ~g~Open", h[M]))
                                else
                                    SetVehicleDoorShut(L, N, false)
                                    tXCEL.notify(string.format("The ~b~%s ~w~door is now ~r~Closed", h[M]))
                                end
                            end
                        end
                        k.door = M
                    ...
                    function(I, K, J, O)
                        if K then
                            if k.limiter then
                                ...
                                k.cruise = false
                                if not p then
                                    local speed = GetEntitySpeed(tXCEL.getPlayerVehicle())
                                    if convert(speed) < 10.0 then
                                        p = true
                                        limiter()
                                    else
                                        p = false
                                        tXCEL.notify("~r~Alert~w~: Please slow down to enable the speed limiter.")
                                    end
                                end
                            else
                        ...
                        if K then
                            ...
                            if GetIsVehicleEngineRunning(L) then
                                if L ~= 0 then
                                    ...
                                    tXCEL.notify("You've turned the ignition into the ~r~off ~w~position.")
                                end
                            else
                                if L then
                                    SetVehicleEngineOn(L, true, false, true)
                                    tXCEL.notify("You've turned the ignition to the ~g~on ~w~position.")
                                end
                            end
                        end
                    end
                )
                local isthere, spawncode, distance = tXCEL.getNearestOwnedVehicle(1)
                RageUI.Button(
                    "Manage Boot Keys",
                    ...
                    function(I, J, K)
                        if K then
                            TriggerServerEvent("XCEL:VehicleBoot:Check", spawncode)
                        end
                    end,
        ...
        RageUI.IsVisible(
            ...
            true,
            function()
                local _, spawncode = tXCEL.getNearestOwnedVehicle(1)
                local hasboot = true
                local boottable = {
                    {user_id = 1, name = "Envy"},
                ...
                if next(boottable) then
                    for _, v in pairs(boottable) do
                        RageUI.Button(
                            ...
                            function(I, J, K)
                                if K then
                                    TriggerServerEvent("XCEL:VehicleBoot:Remove", spawncode, v.user_id)
                                end
                            end
                        )
                    end
                ...
                RageUI.Button(
                    ...
                    function(I, J, K)
                        if K then
                            TriggerServerEvent("XCEL:VehicleBoot:Add", spawncode)
                        end
                    end
                )
                if not hasboot then
        ...
        RageUI.IsVisible(
            ...
            function()
                ...
                RageUI.Button(
                    ...
                    function(I, J, K)
                        if K then
                            TriggerServerEvent("XCEL:VehicleBoot:Purchase", spawncode)
                            if hasboot then
                                RageUI.GoBack()
                            end
                        end
...
RegisterNetEvent("XCEL:VehicleBoot:Return")
AddEventHandler(
    "XCEL:VehicleBoot:Return",
...
local function T()
    if IsControlJustPressed(1, 244) then
        if IsPedInAnyVehicle(tXCEL.getPlayerPed(), false) or RageUI.Visible(RMenu:Get("vehicle_menu", "main")) then
            RageUI.CloseAll()
