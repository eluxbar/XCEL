...
RageUI.CreateWhile(
    ...
    true,
    function()
        if RageUI.Visible(RMenu:Get("xcelscenemenu", "main")) then
            RageUI.DrawContent(
                ...
                function()
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        RMenu:Get("xcelscenemenu", "objects")
                    )
                    RageUI.ButtonWithStyle(
                        "Speed Zone",
                        nil,
                        ...
                        RMenu:Get("xcelscenemenu", "speedZone")
                    )
                    if tXCEL.globalOnPoliceDuty() then
                        RageUI.ButtonWithStyle(
                            ...
                            RMenu:Get("xcelscenemenu", "spikes")
                        )
                    end
                    RageUI.ButtonWithStyle(
                        "Close Menu",
                        ...
                        function(J, K, L)
                            if L then
                                RageUI.Visible(RMenu:Get("xcelscenemenu", "main"), false)
                            end
                        end
                    )
                end
        ...
        if RageUI.Visible(RMenu:Get("xcelscenemenu", "objects")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    RageUI.List(
        ...
        if RageUI.Visible(RMenu:Get("xcelscenemenu", "speedZone")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    RageUI.List(
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(J, K, L)
                            if L then
                                if o == 0 then
                                    tXCEL.notify("Please set a radius")
                                elseif p == 0 then
                                    tXCEL.notify("Please set a speed")
                                else
                                    TriggerServerEvent("XCEL:createSpeedZone", tXCEL.getPlayerCoords(), p + 0.0, o + 0.0)
                                    tXCEL.notify("~b~Speed zone created")
                                end
                            end
                        end,
                        nil
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(J, K, L)
                            if L then
                                ...
                                if Q then
                                    TriggerServerEvent("XCEL:deleteSpeedZone", Q)
                                    tXCEL.notify("Speed zone removed")
                                end
                            end
                        end,
                        nil
        ...
        if RageUI.Visible(RMenu:Get("xcelscenemenu", "spikes")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    RageUI.ButtonWithStyle(
...
RegisterNetEvent(
    "XCEL:toggleTrafficMenu",
    function()
        RageUI.Visible(RMenu:Get("xcelscenemenu", "main"), not RageUI.Visible(RMenu:Get("xcelscenemenu", "main")))
        if globalOnPoliceDuty and tXCEL.hasPoliceCallsign() then
            RMenu:Get("xcelscenemenu", "main"):SetSubtitle(
                "~b~MPD~w~: " ..
                    tXCEL.getPoliceCallsign() .. " - " .. tXCEL.getPoliceRank() .. " - " .. tXCEL.getPoliceName()
            )
        end
    end
)
function deleteObject()
    for m, n in pairs(j) do
        ...
        local X, Y, Z = table.unpack(GetEntityCoords(tXCEL.getPlayerPed(), true))
        if DoesObjectOfTypeExistAtCoords(X, Y, Z, 0.9, W, true) then
            ...
            if NetworkGetEntityIsNetworked(_) and not NetworkHasControlOfEntity(_) then
                local a0 = tXCEL.getNetId(_, "deleteObject()")
                if a0 ~= 0 then
                    TriggerServerEvent("XCEL:requestSceneObjectDelete", a0)
                end
            else
                DeleteObject(_)
            end
            ...
            for m, a1 in pairs(a) do
                if DoesEntityExist(a1) then
                    if NetworkGetEntityIsNetworked(a1) then
                        ...
                        if a2 ~= 0 then
                            TriggerServerEvent("XCEL:requestSceneObjectDelete", a2)
                        end
                    else
                        DeleteEntity(a1)
                    end
...
function deleteSpike()
    local P = GetEntityCoords(tXCEL.getPlayerPed())
    local a7 = getClosetSpike(P)
    if a7 then
        TriggerServerEvent("XCEL:removeSpike", a7[2])
    end
end
RegisterNetEvent(
    "XCEL:addSpike",
...
Citizen.CreateThread(
    function()
        while true do
            local a3, af = tXCEL.getPlayerVehicle()
            if a3 ~= 0 and af then
                local ag = false
                local ah = 1000.0
