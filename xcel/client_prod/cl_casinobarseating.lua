...
Citizen.CreateThread(
    function()
        while true do
            if not e then
                local h = GetEntityCoords(tXCEL.getPlayerPed())
...
function createSitAtBarScene(o, p)
    ...
    NetworkAddPedToSynchronisedScene(tXCEL.getPlayerPed(), q, a, animName, 2.0, -1000, 9, 0, 1148846080, 0)
    ...
    function()
        while true do
            if insideDiamondCasino then
                if e then
                    SetPedCapsule(tXCEL.getPlayerPed(), 0.13)
                    if IsControlJustPressed(0, 202) then
                        tXCEL.loadAnimDict("anim@amb@office@seating@male@var_a@base@")
                        TaskPlayAnim(
                            tXCEL.getPlayerPed(),
