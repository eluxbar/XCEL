...
RMenu.Add('LicenseCentre', 'main', RageUI.CreateMenu("", "~b~" .. "Job Centre", tXCEL.getRageUIMenuWidth(), tXCEL.getRageUIMenuHeight(), "banners", "groups"))
RMenu.Add("LicenseCentre", "confirm", RageUI.CreateSubMenu(RMenu:Get('LicenseCentre', 'main',  tXCEL.getRageUIMenuWidth(), tXCEL.getRageUIMenuHeight())))
    ...
    if q then
        ...
        local u = function(s)
            if IsControlJustPressed(1, 38) then
                TriggerServerEvent('XCEL:getOwnedLicenses')
                Wait(500)
            ...
            tXCEL.addMarker(licensecentre.location.x, licensecentre.location.y,licensecentre.location.z-0.98,0.7,0.7,0.5,255, 255,255,250,50,27,false,false)
        end
        tXCEL.createArea("licensecentre", licensecentre.location, 1.5, 6, r, t, u, {})
        tXCEL.addBlip(licensecentre.location.x, licensecentre.location.y, licensecentre.location.z, 457, 2, "Job Centre", 0.6, true)
    end
end)
...
function func_licenseMenuControl()
    if IsControlJustPressed(0, 167) then
        if a then
            ...
            TriggerServerEvent("XCEL:GetLicenses")
        end
    end
    if a then
        DrawRect(0.50, 0.222, 0.223, 0.075, 16, 86, 229, 255)
        DrawAdvancedText(0.595, 0.213, 0.005, 0.0028, 1.0, "XCEL Licenses", 255, 255, 255, 255, 1, 0)
...
if not tXCEL.isPurge() then
    tXCEL.createThreadOnTick(func_licenseMenuControl)
end
RegisterNetEvent("XCEL:RecievedLicenses",function(i)
    b = i
    a = true
end)
RegisterNetEvent("XCEL:gotOwnedLicenses",function(i)
    for k,v in pairs(licensecentre.licenses) do
        v.notOwned = true
