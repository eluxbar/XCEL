...
function tXCEL.setTime(f, g, h)
    b = {h = f, m = g, s = h}
end
function tXCEL.overrideTime(f, g, h)
    b.h = f
    b.m = g
    b.s = h
    c = true
end
function tXCEL.cancelOverrideTimeWeather()
    c = false
end
function tXCEL.setWeather(i)
    a = i
end
...
tXCEL.createThreadOnTick(func_manageTimeAndWeather)
RegisterNetEvent(
    "XCEL:setTime",
    function(f, g, h)
        tXCEL.setTime(f, g, h)
    ...
    function()
        while true do
            if j then
                ...
                n(z + A + 0.293, 0.866, 0.005, 0.0028, 0.25, tostring(m["EXCELSUNNY"]), 255, 74, 53, 255, 0, 1)
...
function voteWeather(C)
    if j then
        if not k then
            TriggerServerEvent("XCEL:vote", C)
            k = true
            tXCEL.notify("~g~Vote sent!")
        else
            tXCEL.notify("You have already voted!")
        end
    else
        tXCEL.notify("Vote not in progress, start a vote with /voteweather!")
    end
end
function resetVotes()
    m = {
...
RegisterCommand(
    "voteweather",
    function()
        TriggerServerEvent("XCEL:tryStartWeatherVote")
    end,
    false
)
RegisterCommand(
