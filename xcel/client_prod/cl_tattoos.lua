    ...
    function(D, E)
        if E then
            local F = function()
                ...
                c = true
                i = deepcopy(h)
                j = tXCEL.getCustomization()
                RageUI.Visible(RMenu:Get("XCELtattoos", "mainMenu"), true)
            end
            local G = function()
                ...
                RageUI.Visible(RMenu:Get("XCELtattoos", "mainMenu"), false)
                tXCEL.setCustomization(j)
                j = nil
                ClearPedDecorations(tXCEL.getPlayerPed())
                for H = 1, #i, 1 do
            ...
            local I = function()
                ...
                if RageUI.Visible(RMenu:Get("XCELtattoos", "mainMenu")) then
            ...
            for B, J in pairs(f) do
                ...
                Wait(100)
                tXCEL.createArea("tattoParlour_" .. B, J, 1.5, 6, F, G, I)
                tXCEL.addBlip(J.x, J.y, J.z, 75, 0, "Tattoo Parlour")
                tXCEL.addMarker(
                    J.x,
                    J.y,
                    J.z,
...
Citizen.CreateThread(
    function()
        ...
        while true do
            Wait(500)
            if tXCEL.isInComa() and not P then
                P = true
            end
            if not tXCEL.isInComa() and P then
                Wait(5000)
                TriggerServerEvent("XCEL:getPlayerTattoos")
                P = false
            end
                            ...
                            function(U, V, W, X)
                                if V then
                                    ...
                                    if c and not l and IsControlJustReleased(0, 178) then
                                        ...
                                        TriggerServerEvent("XCEL:saveTattoos", i, 1000)
                                    end
                                end
                                if W then
...
function drawTattoo(_, a0, a1)
    SetEntityHeading(tXCEL.getPlayerPed(), 297.7296)
    local C = tXCEL.getPlayerPed()
    local a2 = false
    if _ ~= nil and a0 ~= nil and a1 ~= nil then
        if p then
            ...
            if GetEntityModel(tXCEL.getPlayerPed()) == -1667301416 then
                tXCEL.loadCustomisationPreset("TattooMale")
            else
                tXCEL.loadCustomisationPreset("TattooFemale")
            end
            ClearPedDecorations(tXCEL.getPlayerPed())
            for H = 1, #i, 1 do
                AddPedDecorationFromHashes(
            ...
            AddPedDecorationFromHashes(tXCEL.getPlayerPed(), GetHashKey(a0), GetHashKey(e[a0][_].nameHash))
            if not DoesCamExist(b) then
                b = CreateCam("DEFAULT_SCRIPTED_CAMERA", true)
                SetCamCoord(b, GetEntityCoords(tXCEL.getPlayerPed()))
                SetCamRot(b, 0.0, 0.0, 0.0)
                SetCamActive(b, true)
                RenderScriptCams(true, false, 0, true, true)
                SetCamCoord(b, GetEntityCoords(tXCEL.getPlayerPed()))
            end
            local S, a3, a4 = table.unpack(GetEntityCoords(tXCEL.getPlayerPed()))
...
function drawSkinInstructionKeys()
    local a5 = {
        {["label"] = "Enter Tattoo ID ", ["button"] = "~INPUT_CELLPHONE_EXCEL_OPTION~"},
