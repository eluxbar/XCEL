        ...
        if tXCEL.globalOnPoliceDuty() then
            if m[1] ~= nil then
                if tonumber(m[1]) > 100 then
                    d = tonumber(m[1])
                    tXCEL.notify(string.format("~g~Maximum speed set to %smph", tonumber(m[1])))
                else
                    d = 101
                    tXCEL.notify("Minimum speed you can set is 101mph!")
                end
            end
        else
            tXCEL.notify("Speed gun is not enabled!")
        end
    end
)
...
tXCEL.createThreadOnTick(func_drawRadar)
Citizen.CreateThread(
    function()
        while true do
            if tXCEL.globalOnPoliceDuty() and not usingDelgun then
                if GetSelectedPedWeapon(tXCEL.getPlayerPed()) == a then
                    b = true
                else
                    b = false
                end
                if b and tXCEL.getPlayerVehicle() == 0 then
                    local n = tXCEL.getPlayerId()
                         ...
                         then
                            ...
                            if h > d and h > 101 then
                                ...
                                if not j[q] then
                                    ...
                                    TriggerServerEvent(
                                        "XCEL:speedGunFinePlayer",
                                        GetPlayerServerId(NetworkGetPlayerIndexFromPed(e)),
                                        h - d
                                    )
                                    Citizen.Wait(3000)
                                else
                                    tXCEL.notify("This vehicle has been fined recently!")
                                end
                            else
                                i = "~w~"
                            end
...
RegisterNetEvent("XCEL:speedGunPlayerFined")
AddEventHandler(
    "XCEL:speedGunPlayerFined",
    function()
