    ...
    function()
        ...
        while true do
            if a.active and a.id ~= 0 then
                ...
                if e ~= `WEAPON_UNARMED` and (not b[a.id].allowPistol or not table.has(c, e)) then
...
RMenu.Add(
    "policeshields",
    "main",
    RageUI.CreateMenu("Police Shields", "Shield Management", tXCEL.getRageUIMenuWidth(), tXCEL.getRageUIMenuHeight())
)
RageUI.CreateWhile(
    1.0,
    true,
...
function removeShield(l)
    if DoesEntityExist(a.prop) then
        ...
        if l then
            tXCEL.notify("~g~Success~w~: Shield removed")
        end
    end
end
function createShield(m)
    ...
    a.id = m
    local p = tXCEL.loadModel(b[m].model)
    ...
    tXCEL.loadAnimDict(b[m].animDict)
