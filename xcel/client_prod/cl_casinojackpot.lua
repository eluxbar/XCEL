...
RMenu.Add(
    "XCELjackpot",
    "instructions",
    RageUI.CreateMenu("", "", tXCEL.getRageUIMenuWidth(), tXCEL.getRageUIMenuHeight(), "banners", "casino_jackpot", "xcel_jackpotui", "xcel_jackpotui")
)
RMenu:Get("XCELjackpot", "instructions"):SetSubtitle("JACKPOT")
RageUI.CreateWhile(
    1.0,
    true,
    function()
        if RageUI.Visible(RMenu:Get("XCELjackpot", "instructions")) then
            RageUI.DrawContent(
                ...
                function()
                    RageUI.FakeButton(
                        ...
                        RMenu:Get("XCELjackpot", "instructions")
                    )
                end
            )
        end
    ...
    function()
        while true do
            if insideDiamondCasino then
                b = 1000
                a = -1
                local y = GetEntityCoords(tXCEL.getPlayerPed())
                for z = 0, 15, 1 do
                    local A = t[z]
                    local B = #(y - A)
                    if B < b then
...
function goToJackpotChair(E)
    ...
    PlayFacialAnim(tXCEL.getPlayerPed(), "enter_left_readyidle_facial", H)
    RemoveAnimDict(H)
    local O
    local P
                ...
                if j then
                    ...
                    if u ~= nil then
                        for R, S in pairs(u) do
                            if S.user_id == tXCEL.getUserId() then
                                DrawRect(
                                    0.72,
                                    0.82,
                                    0.115,
        ...
        while true do
            if f then
                if IsDisabledControlJustPressed(0, 22) then
                    local T = tXCEL.KeyboardInput("Bet Amount", "", 30)
                    if tonumber(T) then
                        T = tonumber(T)
                        if T > 0 then
                            p = T
                ...
                if IsControlJustPressed(0, 201) then
                    if h then
                        TriggerServerEvent("XCEL:setJackpotBet", p)
                    else
...
function showHowToJackpot(a4)
    ...
    if a ~= -1 then
        RageUI.Visible(RMenu:Get("XCELjackpot", "instructions"), a4)
    end
end
Citizen.CreateThread(
    function()
        while true do
            if a ~= -1 and b < 2 then
                if IsControlJustPressed(0, 38) then
                    if d[a] == false then
                        TriggerServerEvent("XCEL:requestSitAtJackpot", a)
                    end
                end
            end
            Wait(0)
...
AddEventHandler(
    ...
    function(a5, a6)
        if a6 then
            TriggerServerEvent("XCEL:requestJackpotChairData")
        end
    end
)
RegisterNetEvent(
    "XCEL:sendJackpotChairData",
    function(a7)
        d = a7
    end
)
        ...
        while true do
            if sittingAtInsideTrack and f then
                ...
                if IsControlJustPressed(0, 202) and not g then
                    ...
                    PlayFacialAnim(tXCEL.getPlayerPed(), "exit_left_facial", H)
                    SetTimeout(
                        2500,
                        function()
                    ...
                    f = false
                    showHowToJackpot(false)
                    TriggerServerEvent("XCEL:leaveJackpotChair")
                    e = -1
                    SetTimeout(
                        5000,
                        function()
...
function getTrackGenderAnimDict()
    local aw = GetEntityModel(tXCEL.getPlayerPed())
...
RegisterNetEvent(
    ...
    function()
        ...
        if e ~= -1 then
            ...
            NetworkAddPedToSynchronisedScene(tXCEL.getPlayerPed(), ay, H, I, 2.0, -2.0, 5, 0, 1148846080, 0)
