...
local function c()
    if GetEntityHealth(tXCEL.getPlayerPed()) <= 102 then
        tXCEL.notify("~r~You try to move but you remember you're dead")
        return true
    elseif IsEntityDead(tXCEL.getPlayerPed()) then
        tXCEL.notify("~r~You try to move but you remember you're dead")
        return true
    else
        return false
...
RegisterCommand(
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and tXCEL.canAnim() and not IsEntityDead(h) and d() then
...
function surrender()
    ...
    if DoesEntityExist(h) and tXCEL.canAnim() and not IsEntityDead(h) and (d() or isPlayerNearPrison()) then
        tXCEL.loadAnimDict("random@arrests")
        tXCEL.loadAnimDict("random@arrests@busted")
...
function tXCEL.isPlayerSurrendered()
...
function tXCEL.isPlayerSurrenderedNoProgressBar()
    return j()
end
local u
RegisterNetEvent(
    "XCEL:startRobbingPlayer",
    function()
        u = GetGameTimer()
    end
)
RegisterNetEvent(
    "XCEL:endRobbingPlayer",
    function()
        u = nil
    end
)
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if B then
                ...
                tXCEL.notify("Press ~r~[E]~w~ to take a picture!")
                B = true
            end
        end
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if y then
                ...
                Wait(120)
                tXCEL.notify("Press ~r~[E]~w~ to take a toke!")
                y = true
            end
        end
        while y do
            ...
            if IsControlJustPressed(0, 38) then
                ...
                Wait(100)
                tXCEL.notify("You take a huge rip!")
                Wait(7250)
            end
        end
    end,
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if v then
                ...
                Wait(120)
                tXCEL.notify("Press ~r~[E]~w~ to wave the wand.")
                v = true
            end
        end
            ...
            if
             ...
             then
                if not Y and globalNHSOnDuty then
                    ...
                    Y = false
                    DetachEntity(tXCEL.getPlayerPed(), false, false)
                    ClearPedSecondaryTask(tXCEL.getPlayerPed())
                    local a5 = GetClosestPlayer(3)
                    local a7 = GetPlayerServerId(a5)
                    if a7 ~= 0 then
                        TriggerServerEvent("XCEL:animationSyncStop", a7)
                    end
                end
                Wait(250)
            end
...
RegisterNetEvent(
    ...
    function()
        ...
        SetPedMotionBlur(tXCEL.getPlayerPed(), true)
        SetPedMovementClipset(tXCEL.getPlayerPed(), "move_m@drunk@verydrunk", true)
        RemoveAnimSet("move_m@drunk@verydrunk")
        SetPedIsDrunk(tXCEL.getPlayerPed(), true)
        DoScreenFadeIn(1000)
        ClearPedSecondaryTask(tXCEL.getPlayerPed())
        DeleteObject(prop)
        Wait(60000)
        ...
        ResetPedMovementClipset(tXCEL.getPlayerPed(), 0)
        SetPedIsDrunk(tXCEL.getPlayerPed(), false)
        SetPedMotionBlur(tXCEL.getPlayerPed(), false)
        RemoveAnimDict(H)
    end
)
RegisterNetEvent(
...
RegisterNetEvent(
    ...
    function(ai)
        ...
        tXCEL.loadAnimDict("misshair_shop@hair_dressers")
        ...
        tXCEL.loadModel("prop_clippers_01")
...
RegisterNetEvent(
    ...
    function(au)
        ...
        local aw = tXCEL.getPlayerCoords()
