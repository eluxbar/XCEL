    ...
    Citizen.CreateThread(
        function()
            ...
            if not g then
                ...
                TriggerServerEvent("XCEL:unstuckSuccessful", d, e)
            else
                tXCEL.notifyPicture(
                    "CHAR_BLOCKED",
                    "CHAR_BLOCKED",
...
RegisterCommand(
    ...
    function(q, r, s)
        local h = PlayerPedId()
        local t, p = o()
        if not t or tXCEL.getPlayerCombatTimer() > 0 then
            tXCEL.notifyPicture(
                "CHAR_BLOCKED",
                "CHAR_BLOCKED",
            ...
            tXCEL.loadAnimDict("timetable@amanda@ig_4")
            TaskPlayAnim(h, "timetable@amanda@ig_4", "ig_4_base", 8.0, -8.0, -1, 0, 0.0, 0, 0, 0)
            RemoveAnimDict("timetable@amanda@ig_4")
            Wait(1000)
            b(45, tXCEL.getPlayerCoords(), p)
        end
    end
)
local u = 0
