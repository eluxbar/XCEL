...
RMenu.Add(
    "xcelpdelevators",
    "main",
    RageUI.CreateMenu("XCEL Elevators", "Vespucci PD", tXCEL.getRageUIMenuWidth(), tXCEL.getRageUIMenuHeight())
)
RageUI.CreateWhile(
    1.0,
    true,
...
AddEventHandler(
    ...
    function(i, j)
        if j then
            local k = function()
                RageUI.Visible(RMenu:Get("xcelpdelevators", "main"), true)
            end
            local l = function()
                RageUI.Visible(RMenu:Get("xcelpdelevators", "main"), false)
            end
            local m = function()
                RMenu:Get("xcelpdelevators", "main"):SetSubtitle(string.format("~w~You are on ~b~%s", a[b].name))
            end
            for n, o in pairs(a) do
                tXCEL.createArea("pdelevator_" .. n, o.coords, 1.5, 6, k, l, m)
                tXCEL.addMarker(
                    o.coords.x,
                    o.coords.y,
                    o.coords.z - 0.98,
