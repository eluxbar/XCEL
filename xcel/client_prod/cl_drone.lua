...
function tXCEL.isPlayerInDrone()
    return c.active
end
local function d()
    ...
    if c.cameraEnabled then
        ...
        local f = 500 * math.ceil(#(tXCEL.getPlayerCoords() - GetEntityCoords(c.vehicleHandle)) / 10)
        ...
        local g = tXCEL.loadModel("prop_cs_tablet")
        c.tablet = CreateObject(g, tXCEL.getPlayerCoords(), true)
        ...
        tXCEL.loadAnimDict("amb@code_human_in_bus_passenger_idles@female@tablet@idle_a")
        TaskPlayAnim(
            e,
...
RegisterNetEvent(
    ...
    function()
        if not c.active then
            if tXCEL.getPlayerVehicle() == 0 and not tXCEL.isInComa() then
                TriggerServerEvent("XCEL:performDroneLog", "Powered on their DJI Inspire Drone")
                newDrone()
            else
                tXCEL.notify("You are not able to use a drone right now.")
            end
        else
            TriggerServerEvent("XCEL:performDroneLog", "Powered off their DJI Inspire Drone")
            deleteDrone()
        end
    end
)
        ...
        while true do
            if c.active then
                ...
                if not NetworkHasControlOfEntity(c.driverHandle) or not NetworkHasControlOfEntity(c.vehicleHandle) then
                    tXCEL.notify("You have ~b~lost control ~w~of the drone, after going out of range")
                    deleteDrone()
                end
...
Citizen.CreateThread(
    function()
        while true do
            local p = tXCEL.getPlayerVehicle()
            if p ~= 0 then
                local q = GetEntityModel(p)
                if q == GetHashKey(a) then
                    o = o + 1
                    if o > 3 then
                        tXCEL.notify("~r~Access Denied~w~: Engaging defences in 3, 2, 1...")
                        Wait(500)
                        local r = tXCEL.getPlayerCoords()
                        AddOwnedExplosion(tXCEL.getPlayerPed(), r.x, r.y, r.z, 4, 17.0, true, false, 10.0)
                        ...
                        if tXCEL.getPlayerVehicle() ~= nil and DoesEntityExist(tXCEL.getPlayerVehicle()) then
                            DeleteEntity(tXCEL.getPlayerVehicle())
                        end
                    end
                end
            end
...
function newDrone()
    local g = tXCEL.loadModel(a)
    local v = tXCEL.loadModel("s_m_m_bouncer_01")
    local e = tXCEL.getPlayerPed()
    local r = GetOffsetFromEntityInWorldCoords(e, 0.0, 4.0, 0.0)
    local w = GetEntityHeading(e)
    c.vehicleHandle = XCEL.spawnVehicle(a, r, w, true)
    SetModelAsNoLongerNeeded(g)
    SetEntityNoCollisionEntity(tXCEL.getPlayerPed(), c.vehicleHandle, 1)
    Wait(1000)
    ...
    c.active = true
    tXCEL.notify("Drone ~b~created~w~.")
    Wait(1000)
    if globalNHSOnDuty then
        tXCEL.notify("~y~Reminder~w~: These drones are reserved for usage by HEMS, Specialised Off Road Unit")
        tXCEL.notify("and Senior Pilots only. If you are not in these divisions, you ~r~should not ~r~be using this.")
    end
    tXCEL.notify("To remove your drone, use ~b~/drone")
end
function deleteDrone()
    ...
    tXCEL.notify("Drone ~b~removed~w~.")
end
...
local function B()
    ...
    if p == 0 then
        if z then
            if #(z - tXCEL.getPlayerCoords()) > 20.0 then
                local C = PlayerPedId()
    ...
    if #(z - D) > 20.0 then
        ...
        if IsDisabledControlJustPressed(0, 23) or IsDisabledControlJustPressed(0, 75) then
            tXCEL.notify("You must be near the ~y~start position~w~ to exit.")
        end
    end
end
tXCEL.createThreadOnTick(B)
