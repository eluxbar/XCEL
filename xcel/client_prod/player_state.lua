    ...
    function()
        while true do
            ...
            if IsPlayerPlaying(PlayerId()) and a then
                if json.encode(b) ~= json.encode(tXCEL.getWeapons()) then
                    b = tXCEL.getWeapons()
                    XCELserver.updateWeapons({tXCEL.getWeapons()})
                end
            end
        end
    end
...
Citizen.CreateThread(
    function()
        print("[XCEL] Loading User Data.")
        c = json.decode(GetResourceKvpString("xcel_userdata") or "{}")
        if type(c) ~= "table" then
            c = {}
            print("[XCEL] Loading User Data - Failed To Load Setting To Default.")
        else
            print("[XCEL] Loaded User Data")
        end
    end
)
function tXCEL.updateCustomization(d)
    local e = tXCEL.getCustomization()
    if e.modelhash ~= 0 and IsModelValid(e.modelhash) then
        c.customisation = e
        if d then
            SetResourceKvp("xcel_userdata", json.encode(c))
        end
    end
end
function tXCEL.updateHealth(d)
    c.health = GetEntityHealth(PlayerPedId())
    if d then
        SetResourceKvp("xcel_userdata", json.encode(c))
    end
end
function tXCEL.updateArmour(d)
...
Citizen.CreateThread(
    function()
        ...
        while true do
            Wait(5000)
            if not XCEL.isInHouse() and not inOrganHeist and not tXCEL.isPlayerInRedZone() and not tXCEL.isInSpectate() then
                tXCEL.updatePos()
            end
            if not tXCEL.isStaffedOn() and not tXCEL.isPlayerInAnimalForm() then
                tXCEL.updateCustomization()
            end
            tXCEL.updateHealth()
            tXCEL.updateArmour()
            SetResourceKvp("xcel_userdata", json.encode(c))
...
function tXCEL.checkCustomization()
    local e = c.customisation
    if e == nil or e.modelhash == 0 or not IsModelValid(e.modelhash) then
        tXCEL.setCustomization(getDefaultCustomization(), true, true)
    else
        tXCEL.setCustomization(e, true, true)
    end
end
...
function tXCEL.spawnAnim()
    ...
    TriggerEvent("XCEL:ForceRefreshData", -1)
    DoScreenFadeIn(1000)
    SetBigmapActive(false)
    ShutdownLoadingScreenNui()
    if not tXCEL.isDev() then
    ...
    print("[XCEL] Setting User Health", c.health)
    print("[XCEL] Setting User Armour", c.armour)
    SetEntityHealth(PlayerPedId(), c.health or 200)
    tXCEL.setArmour(c.armour)
...
RegisterNetEvent("XCEL:setHairstyle")
AddEventHandler(
    "XCEL:setHairstyle",
    function(o)
        if o then
