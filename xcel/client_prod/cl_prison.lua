...
RegisterNetEvent(
    "XCEL:prisonBeanbagShotgunClient",
    function(o)
...
AddEventHandler(
    ...
    function(p, q)
        if q then
            ...
            local s = function()
                if b then
                    TriggerServerEvent("XCEL:prisonerSentBackToCell")
                    tXCEL.teleport(a.prisonCells[d].coords.x, a.prisonCells[d].coords.y, a.prisonCells[d].coords.z)
                    tXCEL.notify("Naughty, go back.")
                end
                globalPlayerInPrisonZone = false
            end
            local t = function()
            end
            tXCEL.createArea("forceStayInPrison", a.prisonMainCoords, 225.0, 100.0, r, s, t, {})
            local u = function(v)
                    ...
                    if IsControlJustPressed(0, 38) then
                        if not h then
                            if v.type == "startingLocations" then
                                h = true
                                i = v.job
                                TriggerServerEvent("XCEL:prisonStartJob", v.job)
                            elseif i == v.job then
                                h = true
                            end
                            if h then
                                        ...
                                        while y do
                                            if v.data.isScenario == nil then
                                                if
                                                 ...
                                                 then
                                                    tXCEL.loadAnimDict(v.data.animDict)
                                                    TaskPlayAnim(
                                                        PlayerPedId(),
                                                        v.data.animDict,
                                                        v.data.animName,
                                ...
                                Citizen.CreateThread(
                                    function()
                                        ...
                                        tXCEL.notify(v.data.nextAction)
                                        if v.type == "endingLocations" then
                                            i = ""
                                            TriggerServerEvent("XCEL:prisonEndJob", v.job)
                                        end
                                        h = false
                                    end
                                )
                ...
                for B, C in pairs(A) do
                    if type(C.coords) == "table" then
                        for D = 1, #C.coords, 1 do
                            ...
                            tXCEL.add3DTextForCoord(C.action, C.coords[D].x, C.coords[D].y, C.coords[D].z + 1.0, 2.2)
                        end
                    else
                        tXCEL.createArea(
                            "prisonJob_" .. z .. B,
                        ...
                        tXCEL.add3DTextForCoord(C.action, C.coords.x, C.coords.y, C.coords.z + 1.0, 2.2)
                    end
                end
            end
        end
...
RegisterNetEvent(
    "XCEL:setPrisonGuardOnDuty",
    function(E)
        globalOnPrisonDuty = E
    end
)
RegisterNetEvent(
    "XCEL:forcePlayerInPrison",
    function(E)
        b = E
        globalInPrison = E
    end
...
tXCEL.createThreadOnTick(func_drawPrisonTimerBar)
RegisterNetEvent(
    "XCEL:prisonCreateBreakOutAreas",
    function()
        local H = false
        local I = function()
        ...
        local K = function(v)
            if IsControlJustPressed(0, 38) and not g then
                if tXCEL.TriggerServerCallback("XCEL:prisonStartWireCutting") then
                    g = true
                    tXCEL.loadAnimDict("anim@gangops@facility@servers@")
                    FreezeEntityPosition(PlayerPedId(), true)
                    TaskPlayAnim(
                        PlayerPedId(),
                    ...
                    tXCEL.notify("~g~Started wire cutting (Press X to cancel)")
                    Citizen.CreateThread(
                        function()
                            SetTimeout(
                                60000,
                            ...
                            if H then
                                FreezeEntityPosition(PlayerPedId(), false)
                                ClearPedTasks(PlayerPedId())
                                TriggerServerEvent("XCEL:prisonBreakOutSuccess")
                                local L = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 2.5, 0.0)
                                tXCEL.teleport(L.x, L.y, L.z)
                            end
                        end
                    )
                else
                    tXCEL.notify("You do not have the required equipment!")
                end
            end
            if IsControlJustPressed(0, 73) and g then
                ...
                ClearPedTasks(PlayerPedId())
                FreezeEntityPosition(PlayerPedId(), false)
                tXCEL.notify("Wire cutting cancelled!")
            end
        end
...
RegisterNetEvent(
    ...
    function(M)
        ...
        local P = function(v)
            if globalInPrison then
                tXCEL.DrawText3D(v.coords, "[E] Pickup " .. v.item, 0.5)
                if IsControlJustPressed(0, 38) then
                    TriggerServerEvent("XCEL:prisonPickupItem", v.item)
                end
            end
        end
        for z, A in pairs(M) do
            tXCEL.removeArea("prisonPickupItem_" .. z)
            tXCEL.createArea("prisonPickupItem_" .. z, A.coords, 2.0, 5.0, N, O, P, {item = z, coords = A.coords})
        end
    end
)
RegisterNetEvent(
    "XCEL:prisonRemoveItemAreas",
    function(Q)
        tXCEL.removeArea("prisonPickupItem_" .. Q)
    end
)
RegisterNetEvent(
    "XCEL:prisonReleased",
    function()
        b = false
        globalInPrison = false
        tXCEL.notify("~g~You have been released!")
        tXCEL.teleport(a.prisonLeaveCoords.x, a.prisonLeaveCoords.y, a.prisonLeaveCoords.z)
        Citizen.Wait(100)
...
local function R()
    ...
    if S == GetHashKey("mp_m_freemode_01") then
        tXCEL.loadCustomisationPreset("PrisonerMale")
    else
        tXCEL.loadCustomisationPreset("PrisonerFemale")
    end
end
RegisterNetEvent(
    "XCEL:putInPrisonOnSpawn",
    function(T)
        Citizen.Wait(5000)
        tXCEL.teleport(a.prisonCells[T].coords.x, a.prisonCells[T].coords.y, a.prisonCells[T].coords.z)
        b = true
        globalInPrison = true
        d = T
        Citizen.Wait(100)
        ...
        if S ~= GetHashKey("mp_m_freemode_01") and S ~= GetHashKey("mp_f_freemode_01") then
            tXCEL.setCustomization({modelhash = "mp_m_freemode_01"})
        end
        Citizen.Wait(2000)
        R()
    end
...
RegisterNetEvent(
    "XCEL:prisonTransportWithBus",
    function(T)
        DetachEntity(PlayerPedId(), false, true)
        R()
        c = true
        local U = getClosestPoliceStation()
        tXCEL.teleport(U.coords.x, U.coords.y, U.coords.z)
        Citizen.CreateThread(
            function()
                while c do
        ...
        local V = XCEL.spawnVehicle("pbus", U.coords.x, U.coords.y, U.coords.z, U.heading, false, false, false)
        ...
        enter_prisonArrive = function()
            ...
            while #(tXCEL.getPlayerCoords() - a.prisonArriveMainDoor) > 1 do
                Citizen.Wait(0)
            end
            tXCEL.teleport(a.prisonCells[T].coords.x, a.prisonCells[T].coords.y, a.prisonCells[T].coords.z)
            Citizen.Wait(100)
            ...
            DeleteEntity(V)
            DeleteEntity(W)
            TriggerServerEvent("XCEL:prisonArrivedForJail")
            c = false
            b = true
            globalInPrison = true
            d = T
...
RegisterNetEvent(
    "XCEL:prisonSpawnInMedicalBay",
    function()
        Citizen.Wait(2000)
        tXCEL.teleport(a.medicalBayBed.x, a.medicalBayBed.y, a.medicalBayBed.z)
        b = true
        Citizen.Wait(100)
        SetEntityHeading(PlayerPedId(), a.medicalBayHeading)
        tXCEL.loadAnimDict("mp_bedmid")
...
RageUI.CreateWhile(
    ...
    true,
    function()
        if RageUI.Visible(RMenu:Get("xcelPrisonItemMenu", "main")) then
            RageUI.DrawContent(
                    ...
                    for D = 1, #a.craftableItems, 1 do
                        RageUI.ButtonWithStyle(
                            ...
                            function(X, Y, Z)
                                ...
                                if Z then
                                    if tXCEL.TriggerServerCallback("XCEL:prisonCraftItem", currentButton) then
                                        local _ = true
                                        FreezeEntityPosition(PlayerPedId(), true)
                                        tXCEL.loadAnimDict("amb@medic@standing@kneel@base")
                                        TaskPlayAnim(
                                            PlayerPedId(),
                                        ...
                                        tXCEL.notify("You do not have the required items to craft this!")
                                    end
                                end
                            end,
                            nil
    ...
    function()
        if globalOnPrisonDuty then
            if n then
                ...
                local a0 = tXCEL.TriggerServerCallback("XCEL:requestPrisonerData")
                if a0 ~= nil then
                    n = true
...
function createPrisonCraftMenu()
    tXCEL.removeMarker(f)
    tXCEL.removeArea("prisonCraftItemArea")
    local a2 = function()
        RageUI.Visible(RMenu:Get("xcelPrisonItemMenu", "main"), true)
    end
    local a3 = function()
        RageUI.Visible(RMenu:Get("xcelPrisonItemMenu", "main"), false)
    end
    local a4 = function()
    end
    local a5 = a.prisonMenuCoords[d].coords
    tXCEL.createArea("prisonCraftItemArea", a5, 1.0, 3.0, a2, a3, a4, {})
    f = tXCEL.addMarker(a5.x, a5.y, a5.z - 0.975, 0.4, 0.4, 0.5, 30, 50, 255, 255, 30, 27, false, false, false)
end
...
function getClosestPoliceStation()
    for D = 1, #a.allPoliceStations, 1 do
        if #(tXCEL.getPlayerCoords() - a.allPoliceStations[D].coords) <= 800 then
...
function nearPrisonPayPhone()
    for D = 1, #a.prisonPayPhones, 1 do
        if #(tXCEL.getPlayerCoords() - a.prisonPayPhones[D].coords) <= 3.0 then
            return true
        end
    end
    return false
