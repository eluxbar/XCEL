...
local function B()
    ...
    local D = 500 * math.ceil(GetDistanceBetweenCoords(tXCEL.getPlayerCoords(), GetEntityCoords(b), true) / 10)
    if j then
...
tXCEL.addMarker(d.x, d.y, d.z, 1.2, 1.2, 1.2, 0, 255, 125, 125, 50, 0, true, true)
local F = tXCEL.addBlip(d.x, d.y, d.z, 457, 18, "G4S Depot", 1.0, false)
local G = function()
...
tXCEL.createArea("g4sDepot_", d, 1.5, 6, G, H, I)
    ...
    function()
        while true do
            if not y.hackingEngaged and not a and not y.awaitingResponse then
                if table.count(s) > 0 and tXCEL.getPlayerVehicle() == 0 then
                    for K, L in pairs(s) do
                        ...
                        if table.has(p, O) or not DoesEntityExist(N) then
                            ...
                            if IsVehicleStopped(N) then
                                local P = #(tXCEL.getPlayerCoords() - GetEntityCoords(N))
                                ...
                                if P <= 11.0 then
                                    ...
                                    if IsControlJustPressed(1, 18) then
                                        ...
                                        y.entity = N
                                        y.src = O
                                        TriggerServerEvent("XCEL:requestHackingChanceServer", M, O)
...
function toggleCashbox()
    if A.active then
        ...
        local o = tXCEL.loadModel("cashbox")
        local C = tXCEL.getPlayerPed()
...
RegisterNetEvent(
    ...
    function(b, W, X, Y)
        if Y then
            ...
            local o = tXCEL.loadModel("prop_cs_tablet")
            ...
            if HasScaleformMovieLoaded(R) then
                ...
                if IsScaleformMovieMethodReturnValueReady(S) then
                    ...
                    elseif ProgramID == 87 then
                        ...
                        TriggerServerEvent("XCEL:updateHackingOutcome", y.netId, y.src, U, false)
                        DeleteObject(v)
                        break
                    ...
                    elseif ProgramID == 86 then
                        ...
                        TriggerServerEvent("XCEL:updateHackingOutcome", y.netId, y.src, U, true)
                        DeleteObject(v)
                        break
    ...
    function()
        if RageUI.Visible(RMenu:Get("g4s", "main")) then
            RageUI.DrawContent(
                ...
                function()
                    if not j then
                        if tXCEL.getPlayerVehicle() == 0 and w < 4 then
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(Z, _, a0)
                                if a0 then
                                    TriggerServerEvent("XCEL:toggleShiftG4S", false)
                                    a = false
                                end
                            end
                        )
                        ...
                        if not h then
                            RageUI.ButtonWithStyle(
                                ...
                                function(Z, _, a0)
                                    if a0 then
                                        TriggerServerEvent("XCEL:activatePanicServer", c)
                                        imageNotification(
                                            "notification_images",
                                            "g4slogo",
                    ...
                    if not a then
                        RageUI.ButtonWithStyle(
                            ...
                            function(Z, _, a0)
                                if a0 then
                                    RageUI.Visible(RMenu:Get("g4s", "shiftHandler"), false)
                                    TriggerServerEvent("XCEL:toggleShiftG4S", true)
                                end
                            end
                        )
                    else
...
function newG4sVehicle()
    ...
    if DoesEntityExist(b) and not IsVehicleDriveable(b, true) then
        ...
        b = XCEL.spawnVehicle(n, a1.x, a1.y, a1.z, GetEntityHeading(C), true, true)
        c = VehToNet(b)
        TriggerServerEvent("XCEL:updateCurrentVehicle", c)
        w = w + 1
    end
end
function securityCheck(C)
    local a2 = tXCEL.getPlayerVehicle()
    if a2 ~= 0 and a2 ~= nil and b ~= nil and b ~= 0 then
        if a2 ~= b then
            ...
            SetTimeout(
                15000,
                function()
                    a2 = GetVehiclePedIsIn(tXCEL.getPlayerPed(), false)
                    if a2 ~= 0 then
                        if a2 ~= b then
                            TriggerServerEvent("XCEL:toggleShiftG4S", false)
                            tXCEL.removeBlip(k)
...
RegisterNetEvent(
    "XCEL:resetPanicStatus",
    function()
        h = false
    end
)
RegisterNetEvent(
    "XCEL:updateJobInformation",
    function(a3)
        if l ~= 0 then
            tXCEL.removeBlip(l)
        end
        if a3.jobActive then
            ...
            Wait(3000)
            TriggerServerEvent("XCEL:acceptJob")
        end
    end
)
...
RegisterNetEvent(
    ...
    function(a4)
        ...
        local J = tXCEL.getPlayerCoords()
        a = true
        tXCEL.disableCustomizationSave(a)
        b = XCEL.spawnVehicle(n, z.x, z.y, z.z, z.heading, true, true)
        c = VehToNet(b)
        ...
        x = json.encode(tXCEL.getCustomization())
        if a8 then
            tXCEL.loadCustomisationPreset("G4SMale")
        else
            tXCEL.loadCustomisationPreset("G4SFemale")
        end
        Wait(2000)
        ...
        Wait(5000)
        TaskVehicleTempAction(tXCEL.getPlayerPed(), b, 27, 10.0)
        Wait(2000)
...
RegisterNetEvent(
    ...
    function()
        ...
        local C = tXCEL.getPlayerPed()
        tXCEL.setCustomization(json.decode(x))
        if A.active then
            toggleCashbox()
        end
        if k ~= 0 then
...
RegisterNetEvent(
    ...
    function()
        ...
        while not f do
            ...
            local P = #(tXCEL.getPlayerCoords() - ae)
            if P <= 1.0 then
                ad = true
        ...
        TriggerServerEvent("XCEL:updateMoneyInVehicle")
        t = false
    end
)
RegisterNetEvent(
    "XCEL:requestMoneyOutVehicle",
    function()
        local ac = tXCEL.getPlayerPed()
        while IsPedInAnyVehicle(ac, true) do
            Wait(400)
        ...
        w = 1
        TriggerServerEvent("XCEL:updateMoneyOutVehicle")
    end
)
...
RegisterNetEvent(
    "XCEL:activeVansUpdate",
    function(ax, O, aC)
        if not a then
            local b = tXCEL.getObjectId(ax, "XCEL:activeVansUpdate 1")
            if b ~= 0 and b ~= nil and DoesEntityExist(b) then
                if aC then
                    s[ax] = nil
                else
                    if not table.has(s, ax) then
                        local b = tXCEL.getObjectId(ax, "XCEL:activeVansUpdate")
                        s[ax] = {O, b, ax}
                    end
                end
            end
