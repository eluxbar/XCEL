...
local function e()
    tXCEL.loadAnimDict("anim@mp_player_intcelebrationfemale@air_guitar")
    TaskPlayAnim(
        PlayerPedId(),
...
local function i()
    ...
    if j == 0 then
        a = true
        SetResourceKvpInt("xcel_quests_completedall", 1)
        TriggerServerEvent("XCEL:claimQuestReward")
    end
    return a
end
local function l()
    hasCompletedAllQuestsInt = GetResourceKvpInt("xcel_quests_completedall")
    if not hasCompletedAllQuestsInt then
        SetResourceKvpInt("xcel_quests_completedall", 0)
    end
    if hasCompletedAllQuestsInt == 0 then
        for m = 1, 50 do
            local n = GetResourceKvpInt("xcel_quests_q" .. m .. "_status")
            if not n then
                SetResourceKvpInt("xcel_quests_q" .. m .. "_status", 0)
                n = 0
            end
            if n == 0 then
                c[m] = true
...
local function o(p)
    c[p] = nil
    SetResourceKvpInt("xcel_quests_q" .. p .. "_status", 1)
    TriggerServerEvent("XCEL:setQuestCompleted")
    i()
end
local function q()
    local j = 50
            ...
            local A = function(y)
                ...
                if IsControlJustReleased(0, 38) then
                    ...
                    if q() ~= 50 then
                        ...
                        DoScreenFadeIn(2000)
                        tXCEL.removeArea("quests_prop_id_" .. y.undiscoveredQID)
                        tXCEL.removeArea("quests_text_id_" .. y.undiscoveredQID)
                    else
                        r("~g~+14 Days XCEL Club Platinum", "You collected them all!", 600)
                        PlaySoundFrontend(-1, "Hack_Success", "DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS", 1)
                        ShakeGameplayCam("FAMILY5_DRUG_TRIP_SHAKE", 1.0)
