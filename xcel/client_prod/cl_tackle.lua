...
function tXCEL.isPedBeingTackled()
    return h
end
RegisterNetEvent(
    "XCEL:playTackle",
    ...
    for p, q in ipairs(GetActivePlayers()) do
        if q ~= PlayerId() then
            ...
            if t ~= "metpd" and t ~= "hmp" and t ~= "npas" then
                ...
                local v = #(u - tXCEL.getPlayerCoords())
                if v < n then
                    n = v
                    o = r
                end
...
function func_tackleManagement()
    ...
    if globalOnPoliceDuty or w() or tXCEL.isStaffedOn() or globalOnPrisonDuty and globalPlayerInPrisonZone then
        if IsControlPressed(0, a["LEFTSHIFT"]) and IsControlPressed(0, a["G"]) then
            if
                not b and GetGameTimer() - g > 10 * 1000 and GetEntityHealth(PlayerPedId()) > 102 and
                    not tXCEL.isKnockedOut()
             then
                local x = m()
                if x then
                    if
                        not b and not c and not IsPedInAnyVehicle(tXCEL.getPlayerPed()) and
                            not IsPedInAnyVehicle(GetPlayerPed(x))
                     then
                        b = true
                        g = GetGameTimer()
                        TriggerServerEvent("XCEL:tryTackle", x)
                    end
                end
            end
        end
...
tXCEL.createThreadOnTick(func_tackleManagement)
