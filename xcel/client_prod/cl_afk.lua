                ...
                if h == prevPos and i == previousHeading and j == prevCameraHeading then
                    if d > 0 then
                        if k and d < 1500 and not e then
                            e = true
                            TriggerServerEvent("XCEL:factionAfkAlert", "5 minutes")
                        end
                        if d == 1202 then
                            e = false
                            if not e and k then
                                TriggerServerEvent("XCEL:factionAfkAlert", "10 minutes")
                                e = true
                            end
                        end
                        if b and d == math.ceil(a / 2) then
                            tXCEL.notify(string.format("~r~You will be set to AFK in: %s seconds", d))
                        end
                        ...
                        if not f then
                            if k then
                                TriggerServerEvent("XCEL:factionAfkAlert", "30 minutes - Player set to AFK")
                            end
                            f = true
                        end
                    end
                    ...
                    if e and k then
                        TriggerServerEvent("XCEL:factionAfkAlert", "No longer AFK - Player moved again")
                        e = false
                    end
                    if f then
                        TriggerServerEvent("XCEL:setNoLongerAFK")
                    end
                    c = false
                    d = a
                    f = false
