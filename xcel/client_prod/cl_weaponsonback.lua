local a = module("xcel-weapons", "cfg/cfg_weaponsonback")
...
local function g()
    ...
    local i = tXCEL.getWeapons()
    local j = false
    local k = globalOnPoliceDuty and tXCEL.getPlayerVehicle() ~= 0
    for l in pairs(i) do
        local m = GetHashKey(l)
        local n = a.weapons[m]
        if n then
            local o =
                (not k or n.type ~= "Heavy") and (tXCEL.isEmergencyService() or not isInGreenzone) and
                not tXCEL.isPlayerInAnimalForm()
