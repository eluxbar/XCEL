local a = false
function func_ptt()
    if tXCEL.getRadioAnim() and tXCEL.canAnim() and not noclipActive and tXCEL.isInRadioChannel() then
        local b = tXCEL.getPlayerPed()
        local c = tXCEL.getPlayerId()
        if a then
...
tXCEL.createThreadOnTick(func_ptt)
...
local function f(g)
    ...
    local i = d[h]
    if i and not e then
        exports["xcel"]:setMaxProximityMode(i)
        e = true
    elseif not i and e then
        exports["xcel"]:clearMaxProximityMode()
        e = false
    end
end
tXCEL.createThreadOnTick(f)
