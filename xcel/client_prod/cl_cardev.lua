RMenu.Add(
    "cardev",
    "mainmenu",
    RageUI.CreateMenu("", "", tXCEL.getRageUIMenuWidth(), tXCEL.getRageUIMenuHeight(), "banners", "settings")
)
    ...
    function()
        if RageUI.Visible(RMenu:Get("cardev", "mainmenu")) then
            RageUI.DrawContent(
                ...
                function()
                    if b and tXCEL.getPlayerBucket() == 333 then
                            ...
                            function(h, i, j)
                                if j then
                                    tXCEL.clientPrompt(
                                        ...
                                        "",
                                        function(k)
                                            if k ~= "" and tXCEL.getPlayerBucket() == 333 then
                                                local l = tXCEL.loadModel(k)
                                                TriggerServerEvent("XCEL:logVehicleSpawn", k, "/cardev")
                                                local m = tXCEL.getPlayerCoords()
                                                local n =
                                                    XCEL.spawnVehicle(
                                                    l,
                                                    m.x,
                                                ...
                                                SetPedIntoVehicle(tXCEL.getPlayerPed(), n, -1)
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(h, i, j)
                                if j then
                                    local o = GetVehiclePedIsIn(tXCEL.getPlayerPed(), false)
                        ...
                        RageUI.Checkbox(
                            ...
                            function()
                                b = true
                                TriggerServerEvent("XCEL:setCarDevMode", b)
                                tXCEL.ClearWeapons()
                            end,
                            function()
                                b = false
            ...
            RageUI.DrawContent(
                ...
                function()
                    for w, x in pairs(f) do
                        if GetNumVehicleMods(tXCEL.getPlayerVehicle(), w) > 0 then
                            RageUI.ButtonWithStyle(
                                x,
                                "",
                                {RightLabel = "→→→"},
        ...
        if RageUI.Visible(RMenu:Get("cardev", "vehiclemodindexes")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    if GetNumVehicleMods(tXCEL.getPlayerVehicle(), selectedModType) == 0 then
                        RageUI.Text({message = "No available mod indexes for this mod type for this vehicle."})
                    else
                        for w = 0, GetNumVehicleMods(tXCEL.getPlayerVehicle(), selectedModType) do
                            RageUI.ButtonWithStyle(
                                "Mod " .. w,
                                "",
                                {RightLabel = "→→→"},
                                ...
                                function(h, i, j)
                                    if j then
                                        SetVehicleModKit(tXCEL.getPlayerVehicle(), 0)
                                        SetVehicleMod(tXCEL.getPlayerVehicle(), selectedModType, w)
                                    end
                                end,
                                RMenu:Get("cardev", "vehiclemodindexes")
                            )
        ...
        if RageUI.Visible(RMenu:Get("cardev", "extras")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    local r = tXCEL.getPlayerVehicle()
                    local y = false
                    if r ~= 0 then
                        for w = 1, 12 do
                                    ...
                                    RageUI.Button(
                                        ...
                                        function(h, i, j)
                                            if j then
                                                ...
                                                TriggerEvent("XCEL:staffFixVehicle")
                                            end
                                        end
                                    )
                                end
                ...
                function()
                    for z, A in pairs(g) do
                        RageUI.Button(
                            ...
                            function(h, i, j)
                                if j then
                                    SetVehicleColours(tXCEL.getPlayerVehicle(), A.colorindex, A.colorindex)
                                end
                            end
                        )
                    end
...
RegisterCommand(
    "cardev",
    function(C)
        if a and not tXCEL.isPurge() then
            B()
        end
    end
)
...
local function ae()
    if not b or tXCEL.getPlayerBucket() ~= 333 then
        if J then
            ac(false)
        end
        return
    ...
    L()
    local af = tXCEL.getFontId("Akrobat-ExtraBold")
    ...
    local t = tXCEL.getPlayerVehicle()
...
tXCEL.createThreadOnTick(ae)

function tXCEL.giveTestWeapons()
    if tXCEL.isCarDev() then
        tXCEL.giveWeapons({["WEAPON_MOSIN"] = {ammo = 250}})
        tXCEL.giveWeapons({["WEAPON_MXM"] = {ammo = 250}})
    end
end