...
local function u()
    ...
    if v ~= 1 then
        ...
        local z = tXCEL.rotationToDirection(x)
...
local function G(H)
    ...
    if noclipActive and tXCEL.getStaffLevel() == 0 then
        return
    end
    inventoryType = nil
    D = true
    ...
    if tXCEL.globalOnPoliceDuty() then
                        ...
                        if not g then
                            if GetHashKey("xs_prop_arena_bag_01") == L then
                                ...
                                if B() then
                                    ...
                                    G(
                                        function()
                                            TriggerServerEvent("XCEL:LootBag", N)
                                        end
                                    )
                                    Wait(1000)
                                end
                            elseif GetHashKey("prop_poly_bag_money") == L then
                                ...
                                if B() then
                                    ...
                                    local P = ObjToNet(O)
                                    TriggerEvent("XCEL:startCombatTimer", false)
                                    TriggerServerEvent("XCEL:Moneydrop", P)
                                    Wait(1000)
                                end
                            ...
                            elseif GetHashKey("xs_prop_arena_crate_01a") == L then
                                ...
                                if B() then
                                    ...
                                    G(
                                        function()
                                            TriggerServerEvent("XCEL:openCrate", Q)
                                            inventoryType = "Crate"
                                        end
                                    )
                                    Wait(1000)
...
local function aa(ab)
    TriggerEvent("XCEL:lockNearestVehicle")
end
local ac
local function ad(ab)
    ac = ab
    SetVehicleDoorOpen(ab, 5, true, false)
    TriggerEvent("XCEL:clOpenTrunk")
    trunkStatus = true
    SendNUIMessage({closeRadialMenu = true})
    local ae = GetSoundId()
...
RegisterNetEvent("XCEL:clCloseTrunk")
AddEventHandler(
    "XCEL:clCloseTrunk",
    function()
        if ac then
...
local function ag(ab)
    TriggerEvent("XCEL:verifyLockpick", ab)
end
local function ah(ai)
    local aj = GetPedInVehicleSeat(ai, -1)
    if aj ~= 0 then
        return tXCEL.notify("~r~Can not repair vehicle with a person in the driver seat.")
    end
    if NetworkGetEntityIsNetworked(ai) then
        local ak = NetworkGetNetworkIdFromEntity(ai)
        if ak ~= 0 then
            TriggerServerEvent("XCEL:attemptRepairVehicle", ai, ak)
        end
    end
end
local al = false
...
local function an(ab)
    if f then
        TriggerEvent("XCEL:searchClient", ab)
    end
end
local function ao(ab)
    if f then
        local ap = tonumber(DecorGetInt(ab, "xcel_owner"))
        if ap > 0 then
            exports.xcel:impound(ap, GetEntityModel(ab), VehToNet(ab), ab)
        else
            TriggerEvent("XCEL:Notify", "~r~Vehicle is not owned by anyone")
            if GetPedInVehicleSeat(ab, -1) == 0 and GetPedInVehicleSeat(ab, 0) == 0 and NetworkGetEntityIsNetworked(ab) then
                TriggerServerEvent("XCEL:impoundDeleteVehicle", NetworkGetNetworkIdFromEntity(ab))
            end
        end
    end
end
local function aq(ab)
    ...
    if as > 0 then
        if GetSelectedPedWeapon(PlayerPedId()) ~= `WEAPON_UNARMED` then
            TriggerServerEvent("XCEL:robPlayer", as)
        else
            TriggerEvent("XCEL:Notify", "~r~You need a weapon in your hands.")
        end
    end
end
local function at(ab)
    ...
    if aj ~= 0 then
        ...
        if as > 0 then
            TriggerServerEvent("XCEL:askId", as)
        end
    end
end
local au = 0
...
local function aw(ab)
    ...
    local as = GetPlayerServerId(ar)
    if as > 0 then
        TriggerServerEvent("XCEL:giveCashToPlayer", as)
    end
end
local function ax(ab)
    if not tXCEL.canAnim() then
        return
    end
    if GetEntityHealth(ab) <= 102 then
        TriggerEvent("XCEL:Notify", "~r~You can not search a player who is dead.")
        return
    end
    local ar = GetPlayerByEntityID(ab)
        ...
        if ay ~= nil then
            if
             ...
             then
                local as = GetPlayerServerId(ar)
                if as > 0 then
                    TriggerServerEvent("XCEL:searchPlayer", as)
                end
            else
                TriggerEvent("XCEL:Notify", "~r~Player must have their hands up or be on their knees!")
            end
        end
    else
        local as = GetPlayerServerId(ar)
...
local function az(ab)
    ...
    if as > 0 then
        if g then
            TriggerServerEvent("XCEL:nhsRevive", as)
        else
            TriggerServerEvent("XCEL:attemptCPR", as)
        end
    end
end
local function aA(ab)
...
local function aB(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("XCEL:dragPlayer", as)
        end
    end
end
local function aC(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("XCEL:putInVehicle", as)
        end
    end
end
local function aD(ab)
    ...
    local as = GetPlayerServerId(ar)
    if as > 0 then
        TriggerServerEvent("XCEL:gunshotTest", as)
    end
end
local function aE(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("XCEL:seizeWeapons", as)
            TriggerServerEvent("XCEL:jailPlayer", as)
        end
    end
end
local function aF(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("XCEL:requestTransport", as)
        end
    end
end
local function aG(ab)
...
local function aH(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("XCEL:seizeIllegals", as)
        end
    end
end
RegisterNUICallback(
    ...
    function(o)
        ...
        elseif aI == "leaveRadio" then
            TriggerEvent("XCEL:clientLeaveRadioChannel")
        elseif aI == "toggleMute" then
            ExecuteCommand("toggleradiomute")
        elseif aI == "radioConfig" then
            TriggerEvent("XCEL:openRadioConfig")
        elseif string.match(aI, "radioChannel") then
            local aJ = string.sub(aI, 13, #aI)
            TriggerEvent("XCEL:clientLeaveRadioChannel")
            Wait(2000)
            TriggerEvent("XCEL:clientJoinRadioChannel", tonumber(aJ))
        end
    end
)
RegisterNetEvent("XCEL:globalOnPoliceDuty")
AddEventHandler(
    "XCEL:globalOnPoliceDuty",
    function(aK)
        f = aK
    end
)
RegisterNetEvent("XCEL:globalOnNHSDuty")
AddEventHandler(
    "XCEL:globalOnNHSDuty",
    function(aK)
        g = aK
    end
)
RegisterNetEvent(
    "XCEL:globalOnPrisonDuty",
    function(aL)
        h = aL
    end
)
...
function tXCEL.getSelectedEntity()
    return b, c
end
AddEventHandler(
    "XCEL:showRadioWheel",
...
RegisterCommand(
    "lootclosestbag",
    function()
        if not playerIsAlive() or tXCEL.getPlayerVehicle() ~= 0 or GetRenderingCam() ~= -1 then
            return
        end
        local t = tXCEL.getPlayerPed()
        local aO = tXCEL.getPlayerCoords()
        local aP = 0
        local aQ = 3.0
        ...
        if aP ~= 0 then
            ...
            if not g then
                ...
                elseif aS == GetHashKey("prop_box_ammo03a") then
                    ...
                    G(
                        function()
                            TriggerServerEvent("XCEL:openCrate", aT)
                        end
                    )
...
RegisterNetEvent("XCEL:playZipperSound",function(j)
