...
RegisterNetEvent(
    ...
    function()
        ...
        a.onJob = true
        tXCEL.notify("~g~Bus Job started, exit the bus station and head to the first bus stop.")
        while a.onJob do
...
RegisterNetEvent(
    ...
    function(d)
        ...
        local f = tXCEL.loadModel(b[a.stopNumber].pedModel)
        local g = tXCEL.getPosition()
        local h = 10.0
...
RegisterNetEvent(
    "XCEL:setNextBusJobBlip",
    function(n)
        a.tempBlip = AddBlipForCoord(n.x, n.y, n.z)
        SetBlipSprite(a.tempBlip, 1)
        SetBlipRoute(a.tempBlip, true)
        a.tempMarker = tXCEL.addMarker(n.x, n.y, n.z, 1.2, 1.2, 1.2, 255, 0, 0, 125, 50, 0, true, true)
    end
)
RegisterNetEvent("XCEL:onClientSpawn")
AddEventHandler(
    "XCEL:onClientSpawn",
    function(o, p)
        if p then
            ...
            local s = function(r)
                if IsControlJustReleased(1, 38) and not a.onJob then
                    TriggerServerEvent("XCEL:attemptBeginBusJob")
                end
            end
            tXCEL.addBlip(a.startVector.x, a.startVector.y, a.startVector.z, 106, 1, "Bus Driver Job")
            tXCEL.addMarker(
                a.startVector.x,
                a.startVector.y,
                a.startVector.z - 1,
            ...
            tXCEL.createArea("bus", a.startVector, 1.5, 6, q, m, s, {})
        end
    end
)
function alert(t)
