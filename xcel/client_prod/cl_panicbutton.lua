...
RegisterCommand(
    ...
    function()
        if globalOnPoliceDuty or globalNHSOnDuty or globalOnPrisonDuty or globalLFBOnDuty or PlayerIsInGang then
            if b.Cooling == 0 then
                local f = {}
                f.Player = tXCEL.getPlayerId()
                f.Ped = tXCEL.getPlayerPed()
                f.Name = tXCEL.GetPlayerName(f.Player)
                ...
                if globalOnPoliceDuty then
                    TriggerServerEvent("XCEL:newPanic", f, 3)
                elseif globalNHSOnDuty then
                    TriggerServerEvent("XCEL:newPanic", f, 2)
                elseif globalOnPrisonDuty then
                    TriggerServerEvent("XCEL:newPanic", f, 4)
                elseif globalLFBOnDuty then
                    TriggerServerEvent("XCEL:newPanic", f, 5)
                elseif PlayerIsInGang then
                    TriggerServerEvent("XCEL:newGangPanic", f, 6)
                end
