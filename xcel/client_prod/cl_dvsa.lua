...
local j =
    tXCEL.addBlip(i.test.reception.x, i.test.reception.y, i.test.reception.z, 523, 47, "DVSA Test Centre", 1.0, false)
local k = function()
...
tXCEL.createArea("dvsaTestCentre_", i.test.reception, 1.5, 6, k, l, m, {})
RegisterNetEvent(
    "XCEL:dvsaData",
    function(n, o, p, q)
        h = true
        ...
        tXCEL.loadModel(i.models.camera)
...
RegisterNetEvent(
    "XCEL:dvsaMessage",
    function(w, x, y)
        tXCEL.notifyPicture(i.images.dict, i.images.govLarge, y, w, x)
    end
)
RegisterNetEvent(
    "XCEL:beginTestClient",
    function(z, A)
        if z then
            TriggerServerEvent("XCEL:dvsaBucket", false)
            currentTest = {
                active = false,
                ped = 0,
                vehicle = 0,
            ...
            local B = tXCEL.loadModel(i.test.examinerModel)
            ...
            tXCEL.loadAnimDict("amb@medic@standing@tendtodead@base")
...
function handleTestRoute()
    ...
    while currentTest.active do
        if not C then
            ...
            local E = #(tXCEL.getPlayerCoords() - i.test.routes[currentTest.route][currentTest.waypoint + 1].coords)
            ...
            if E < 3.5 then
                ...
                if currentTest.waypoint >= #i.test.routes[currentTest.route] then
                    ...
                    if currentTest.blip ~= 0 then
                        tXCEL.removeBlip(currentTest.blip)
                    end
                    if currentTest.marker ~= 0 then
                        tXCEL.removeMarker(currentTest.marker)
                    end
                    returnToTestCentre()
                else
...
function policeChase()
    ...
    local I = XCEL.spawnVehicle(G, H, 270.93, false, true)
    ...
    tXCEL.loadModel(M)
    local N = XCEL.spawnVehicle(M, H, 270.93, false, true)
...
function returnToTestCentre()
    ...
    local T = false
    while true do
        local E = #(tXCEL.getPlayerCoords() - i.test.parkingSpaces[currentTest.parkingSpace].coords)
        if E < 15.0 then
...
function finishTest(S)
    ...
    if S then
        ...
        for r, s in pairs(currentTest.seriousReason) do
            ...
            tXCEL.notify("~r~Serious Fault~w~: " .. currentTest.seriousReason[r])
            Wait(500)
        end
        tXCEL.notifyPicture(i.images.dict, i.images.govLarge, "Your ~y~minor ~w~faults were:", "UK Government", "DVSA")
        Wait(3000)
        dvsaSound("minorFaults")
        for r, s in pairs(currentTest.minorsReason) do
            U = U .. ", " .. currentTest.minorsReason[r]
            tXCEL.notify("~y~Minor Fault~w~: " .. currentTest.minorsReason[r])
            Wait(500)
        end
        Wait(4000)
    ...
    if tXCEL.teleport ~= nil then
        tXCEL.teleport(i.test.finishTestTpCoords.x, i.test.finishTestTpCoords.y, i.test.finishTestTpCoords.z)
    end
    Wait(3000)
    SetTimeout(
        20000,
    ...
    TriggerServerEvent("XCEL:dvsaBucket", true)
    if S then
        TriggerServerEvent("XCEL:candidatePassed", currentTest.serious, currentTest.minors, U)
        currentTest.active = false
    else
        TriggerServerEvent("XCEL:candidateFailed", currentTest.serious, currentTest.minors, V, U)
        currentTest.active = false
    end
    if currentTest.blip ~= 0 then
        tXCEL.removeBlip(currentTest.blip)
...
function pullUpOnRight()
    ...
    tXCEL.notifyPicture(i.images.dict, i.images.govLarge, "Listen carefully for directions", "Controlled Stop", "DVSA")
    local T = false
    local Y = false
...
function operateHeadlights()
    PlaySoundFrontend(-1, "Out_Of_Bounds_Timer", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 1)
    tXCEL.notifyPicture(i.images.dict, i.images.govLarge, "Listen carefully for directions", "Show Me Question", "DVSA")
...
function emergencyStop()
    ...
    tXCEL.notifyPicture(i.images.dict, i.images.govLarge, "Listen carefully for directions", "Test Instruction", "DVSA")
    currentTest.subtitle =
    ...
    tXCEL.notifyPicture(i.images.dict, i.images.govLarge, "Perform an emergency stop immediately", "STOP", "DVSA")
    Wait(3000)
    if not GetEntitySpeed(currentTest.vehicle) == 0.0 then
...
function useTablet()
    local B = tXCEL.loadModel("prop_cs_tablet")
...
function cameraTransition(t)
    ...
    local ped = tXCEL.getPlayerPed()
    FreezeEntityPosition(ped, true)
    local a5 = tXCEL.getPlayerCoords()
                                    ...
                                    function(a8, a9, aa)
                                        if aa then
                                            if a.banned then
                                                ...
                                                if not currentTest.requested then
                                                    TriggerServerEvent("XCEL:beginTest")
                                ...
                                RageUI.Button(
                                    ...
                                    function(a8, a9, aa)
                                        if aa then
                                            ...
                                            TriggerServerEvent("XCEL:surrenderLicence")
                                        end
                                    end
                                )
                            end
                            ...
                            if a.active then
                                ...
                                if not f then
                                    RageUI.Button(
                                        ...
                                        function(a8, a9, aa)
                                            if aa then
                                                TriggerServerEvent("XCEL:activateLicence")
                                                f = true
                                                tXCEL.notifyPicture(
                                                    i.images.dict,
                                                    i.images.govLarge,
    ...
    function()
        while true do
            ...
            if not a.full and not a.banned and not currentTest.active then
                local vehicle = tXCEL.getPlayerVehicle()
                if vehicle ~= 0 and GetPedInVehicleSeat(vehicle, -1) == tXCEL.getPlayerPed() then
...
function flashCamera(ai)
    ...
    TriggerServerEvent(
        "XCEL:speedCameraFlashServer",
        math.ceil(GetEntitySpeed(GetVehiclePedIsIn(tXCEL.getPlayerPed(), false)) * 2.2369)
    )
...
function createPlate(vehicle)
    e.active = true
    e.vehicle = vehicle
    local B = tXCEL.loadModel("prop_lplate")
    local t = tXCEL.getPlayerCoords()
    ...
    tXCEL.notifyPicture(i.images.dict, i.images.lPlate, i.notifications.lPlatesAdded, "DVSA", "Licence Services")
    tXCEL.notifyPicture(i.images.dict, i.images.lPlate, i.notifications.lPlatesAdded2, "DVSA", "Licence Services")
end
function makeVehicleCrash(aq, J, H, L, ar)
    tXCEL.loadModel(aq)
    local I = XCEL.spawnVehicle(aq, H, ar, true, true)
    while not DoesEntityExist(I) do
        Wait(0)
    end
