...
local function a2(Q)
    Citizen.CreateThread(
        function()
            ...
            Q = d.driveOut
            local a3, a4, a5 = tXCEL.getNearestOwnedVehicle(1)
            h.model = a5
            h.spawncode = a4
            if y ~= 0 then
                                     ...
                                     then
                                        if Q.locked then
                                            if not LSC_Config.lock then
                                                ...
                                                tXCEL.DrawText(0.5, 0.8, "~r~Locked, please wait~w~", 1.0, 4, 0)
                                            end
                                        else
                                            if IsControlJustPressed(1, 201) then
                                                b = true
                                        ...
                                        tXCEL.DrawText(0.5, 0.8, "~r~This vehicle can't be upgraded~w~", 1.0, 4, 0)
                                    end
                                end
                            else
                                if
...
function CanPurchase(I, an)
    if an then
        ...
        A:showNotification("~r~You cannot afford this purchase.")
        return false
    end
end
function UnfakeVeh()
    ...
    function(aN, aO)
        if aO then
            ...
            for aQ, aR in pairs(f) do
                tXCEL.addBlip(aR.inside.position.x, aR.inside.position.y, aR.inside.position.z, 72, nil, "LS Customs")
            end
        end
    end
)
...
function tXCEL.isInsideLsCustoms()
    return b
end
