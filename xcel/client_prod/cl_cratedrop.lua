...
RegisterNetEvent(
    "XCEL:addCrateDropRedzone",
    function(f, e)
        tXCEL.loadModel("xs_prop_arena_crate_01a")
        a[f] = {}
        ...
        d = GetSoundId()
        PlaySoundFromEntity(d, "Crate_Beeps", a[f].crate, "MP_CRATE_DROP_SOUNDS", true, 0)
        radiusBlip = AddBlipForRadius(e.x, e.y, e.z, 250.0)
        SetBlipColour(radiusBlip, 1)
        SetBlipAlpha(radiusBlip, 250)
    end
)
RegisterNetEvent(
    "XCEL:removeCrateRedzone",
    function()
        SetTimeout(
            300000,
            function()
