...
AddEventHandler(
    ...
    function(t, u)
        if u then
            TriggerServerEvent("XCEL:getStoreRobBlips")
            while p == nil do
                Wait(0)
            end
            while true do
    ...
    if not IsPedDeadOrDying(x) then
        if IsEntityPlayingAnim(x, "mp_am_hold_up", "holdup_victim_20s", 3) then
            ...
            if not IsPedDeadOrDying(x) then
                ...
                local A = tXCEL.loadModel("prop_poly_bag_01")
                        ...
                        function()
                            while true do
                                ...
                                if DoesEntityExist(B) then
                                    if #(GetEntityCoords(tXCEL.getPlayerPed()) - GetEntityCoords(B)) <= 1.5 then
                                        PlaySoundFrontend(
                                            -1,
                                            "ROBBERY_MONEY_TOTAL",
                                        ...
                                        TriggerServerEvent("XCEL:pickupCashFromApu", v)
                                        DeleteObject(B)
                                        break
                                    end
                                else
...
Citizen.CreateThread(
    function()
        while true do
            local t = GetEntityCoords(tXCEL.getPlayerPed(), true)
            if r ~= nil then
                for v, w in pairs(r) do
...
function refreshStoreBlips(u)
    ...
    for D, E in pairs(p) do
        ...
        if E.beingrobbed == true then
            tempBlip = tXCEL.addBlip(F.x, F.y, F.z, 52, 1, "Robbable Store [BEING ROBBED]")
        else
            tempBlip = tXCEL.addBlip(F.x, F.y, F.z, 52, 2, "Robbable Store")
        end
...
Citizen.CreateThread(
    function()
        for v, w in pairs(r) do
            local G = tXCEL.loadModel(w.shopNpcModel)
            local H =
                CreatePed(
                26,
                G,
            ...
            SetPedFleeAttributes(H, 0, 0)
            local I = tXCEL.loadModel(w.prop_safe)
            local J = tXCEL.loadModel(w.prop_door)
            local K = tXCEL.loadModel(w.money_prop)
            local L = tXCEL.loadModel(w.money_prop2)
            local M = tXCEL.loadModel(w.money_prop3)
            local N = tXCEL.loadModel(w.money_prop4)
        ...
        while true do
            local U = GetEntityCoords(tXCEL.getPlayerPed())
            for v, w in pairs(r) do
                    ...
                    if X ~= 0 then
                        if IsPlayerFreeAimingAtEntity(PlayerId(), X) then
                            if
                             ...
                             then
                                if not q then
                                    if GetSelectedPedWeapon(tXCEL.getPlayerPed()) == `WEAPON_UNARMED` then
                                        tXCEL.notify("~r~You need a weapon in your hands to rob this store!")
                                    else
                                        TriggerServerEvent("XCEL:initiateStoreRobbery", v)
...
RegisterNetEvent(
    "XCEL:beginStoreRobbingAnimations",
    function(v)
...
RegisterNetEvent(
    "XCEL:storeRobberyInProgress",
    function(Y, v)
        q = Y
        local Z = GetEntityCoords(tXCEL.getPlayerPed())
        while q do
            local U = GetEntityCoords(tXCEL.getPlayerPed())
            if #(Z - U) > 20 then
                TriggerServerEvent("XCEL:forceEndRobbery", v)
...
RegisterNetEvent(
    ...
    function(v)
        ...
        local G = tXCEL.loadModel(r[v].shopNpcModel)
        local H =
            CreatePed(
            26,
            G,
                ...
                if w.distanceToPlayer < 2.0 then
                    if safeCrackingState == "complete" then
                        ...
                        if IsControlJustReleased(1, 51) then
                            ...
                            TriggerServerEvent("XCEL:completeSafeCracking", v)
                            r[v].robberyInProgress = false
                            safeCrackingState = "setup"
                            Wait(5000)
                            TriggerServerEvent("XCEL:syncCloseSafeDoor", v)
                        end
                    elseif w.robberyInProgress then
                        holdup_DisplayHelpText2("Press ~INPUT_CONTEXT~ to start cracking the safe!")
                        if IsControlJustReleased(1, 51) then
                            tXCEL.notify("~g~Started cracking safe..")
                            LoadResources()
...
function RunMiniGame(v, ab, ac)
    ...
    while safeCrackingInProgress do
        ...
        elseif safeCrackingState == "cracking" then
            ...
            local ae = GetEntityHealth(tXCEL.getPlayerPed())
            if ae <= 102 or tXCEL.isHandcuffed() then
                ...
                if IsControlJustPressed(0, 172) then
                    if af == a[ad] then
                        ...
                        if IsSafeUnlocked(b, ad) then
                            ...
                            safeCrackingInProgress = false
                            ClearPedTasksImmediately(tXCEL.getPlayerPed())
                        end
                    else
                        ad = 1
                        b = InitSafeLocks(a)
...
function GetSafeDoorAnimOffsetPosition(ah, ai, aj)
    ...
    return vector3(ah.x + ao, ah.y + ap, GetEntityCoords(tXCEL.getPlayerPed()).z)
end
function PlaySafeCrackIntroAnim(aq, h)
    local ar = "mini@safe_cracking"
    local as = "step_into"
    loadAnimDict(ar)
    TaskPlayAnimAdvanced(tXCEL.getPlayerPed(), ar, as, aq.x, aq.y, aq.z, 0.0, 0.0, h, 8.0, 8.0, -1, 2, 0.7, 0, 0)
    Wait(0)
    Wait(1000)
    safeCrackingState = "cracking"
end
function HandleSafeDialMovement()
    local at = tXCEL.getPlayerPed()
    local ar = "mini@safe_cracking"
    local au = ""
    ...
    loadAnimDict(ar)
    if not IsEntityPlayingAnim(tXCEL.getPlayerPed(), ar, au, 3) then
        TaskPlayAnimAdvanced(tXCEL.getPlayerPed(), ar, au, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    end
end
function HandleIncorrectMovement(ad, b)
    ...
    loadAnimDict(ar)
    TaskPlayAnimAdvanced(tXCEL.getPlayerPed(), ar, ax, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    d = "rotation.idle"
end
...
function EndMiniGame(v, ab)
    ...
    Wait(2500)
    TriggerServerEvent("XCEL:syncOpenSafeDoor", v)
    safeCrackingState = "setup"
