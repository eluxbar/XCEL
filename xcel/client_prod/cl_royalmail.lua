...
RegisterNetEvent(
    ...
    function()
        ...
        a.onJob = true
        tXCEL.notify("~g~Royal Mail Job started, exit the car park on your left and head to your first drop off point.")
        while a.onJob do
...
RegisterNetEvent(
    ...
    function(c, d)
        ...
        if a.tempMarker then
            tXCEL.removeMarker(a.tempMarker)
        end
        ...
        local f = tXCEL.loadModel("prop_drug_package")
...
RegisterNetEvent(
    "XCEL:royalMailJobSetNextBlip",
    function(g)
        a.tempBlip = AddBlipForCoord(g.x, g.y, g.z)
        SetBlipSprite(a.tempBlip, 1)
        SetBlipRoute(a.tempBlip, true)
        a.tempMarker = tXCEL.addMarker(g.x, g.y, g.z - 1, 2.0, 2.0, 1.0, 200, 20, 0, 50, 50)
    end
)
AddEventHandler(
    "XCEL:onClientSpawn",
        ...
        if i then
            ...
            local m = function(k)
                if IsControlJustReleased(1, 38) and not a.onJob then
                    TriggerServerEvent("XCEL:attemptBeginRoyalMailJob")
                end
            end
            tXCEL.addBlip(a.startVector.x, a.startVector.y, a.startVector.z, 67, 1, "Royal Mail Job")
            tXCEL.addMarker(
                a.startVector.x,
                a.startVector.y,
                a.startVector.z,
            ...
            tXCEL.createArea("royalmail", a.startVector, 1.5, 6, j, l, m, {})
        end
    end
)
