RMenu.Add(
    "nhs",
    "main",
    RageUI.CreateMenu("NHS", "Status: ~b~Available", tXCEL.getRageUIMenuWidth(), tXCEL.getRageUIMenuHeight())
)
RMenu.Add(
    "nhs",
    "equipment",
                    ...
                    if e.active then
                        ...
                        if e.readyToRevive then
                            RageUI.Button(
                                ...
                                function(g, h, i)
                                    if i then
                                        TriggerServerEvent("XCEL:finishRevive", e.perm_id)
                                        e.readyToRevive = false
                                        TriggerServerEvent("XCEL:playNhsSound", "shockAdvisedCharging")
                                    end
                                end
                            )
                        else
                            RageUI.Button(
                                ...
                                function(g, h, i)
                                    if i then
                                        ...
                                        TriggerServerEvent("XCEL:playNhsSound", "threeBeeps")
                                    end
                                end
                            )
                        end
            ...
            RageUI.DrawContent(
                ...
                function()
                            ...
                            RageUI.Button(
                                ...
                                function(g, h, i)
                                    if i then
                                        TriggerServerEvent("XCEL:finishRevive", e.perm_id)
                                        e.readyToRevive = false
                                        TriggerServerEvent("XCEL:playNhsSound", "carryOutShock")
                                    end
                                end
                            )
                        end
                    ...
                    RageUI.Button(
                        ...
                        function(g, h, i)
                            if i then
                                TriggerServerEvent("XCEL:playNhsSound", "metronome")
                            end
                        end
                    )
...
function carryLifePak()
    ...
    f.active = true
    local l = tXCEL.loadModel("prop_lifepak")
    ...
    Citizen.CreateThread(
        function()
            ...
            if not e.dead then
                ...
                if n == 3 then
                    Wait(3000)
                    TriggerServerEvent("XCEL:playNhsSound", "checkPatientAlarm")
                end
...
function beginObs(a, u, v, w, d)
    TriggerServerEvent("XCEL:playNhsSound", "threeBeeps")
    Wait(3000)
    TriggerServerEvent("XCEL:playNhsSound", "analysingHeartRythm")
    e = {}
    ...
    if e.coma then
        ...
        if IsEntityDead(GetPlayerPed(u)) then
            ...
            if e.readyToRevive then
                ...
                TriggerServerEvent("XCEL:playNhsSound", "pushToShock")
            end
        end
    end
end
function connectObsToPatient()
    TriggerServerEvent("XCEL:attachLifepakServer")
end
RegisterNetEvent(
    "XCEL:clientPlayNhsSound",
    function(x, y)
...
RegisterNetEvent(
    ...
    function()
        ...
        e.coma = false
        Wait(3000)
        TriggerServerEvent("XCEL:playNhsSound", "safeToTouch")
        Wait(12000)
        TriggerServerEvent("XCEL:playNhsSound", "threeBeeps")
    end
)
    ...
    if globalNHSOnDuty or globalLFBOnDuty then
        ...
        if E ~= `WEAPON_UNARMED` then
            if not table.has(B, E) then
                tXCEL.setWeapon(D, "WEAPON_UNARMED", true)
            end
        end
    end
end
tXCEL.createThreadOnTick(C)
local F = false
RegisterNetEvent(
    "XCEL:attemptCPR",
    function()
        F = true
        tXCEL.loadAnimDict("missheistfbi3b_ig8_2")
        TaskPlayAnim(
            PlayerPedId(),
        ...
        while F do
            if not IsEntityPlayingAnim(PlayerPedId(), "missheistfbi3b_ig8_2", "cpr_loop_paramedic", 3) then
                F = false
                TriggerServerEvent("XCEL:cancelCPRAttempt")
                break
            end
            Citizen.Wait(0)
        end
...
RegisterNetEvent(
    "XCEL:cancelCPRAttempt",
    function()
        F = false
        if tXCEL.canAnim() then
