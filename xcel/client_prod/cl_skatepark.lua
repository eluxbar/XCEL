            ...
            local h = function(f)
                ...
                if IsControlJustPressed(1, 38) then
                    if GetGameTimer() - b > 60000 then
                        local i = tXCEL.loadModel(a[f.skateparkId].carID)
                        local j =
                            CreateVehicle(
                            i,
                        ...
                        SetPedIntoVehicle(tXCEL.getPlayerPed(), j, -1)
                        SetModelAsNoLongerNeeded(i)
                        b = GetGameTimer()
                    else
                        tXCEL.notify("Please wait before spawning another BMX.")
                    end
                end
            end
            for k, l in pairs(a) do
                tXCEL.createArea("skatepark_" .. k, l.position, 1.5, 6, e, g, h, {skateparkId = k})
                tXCEL.addMarker(
                    l.position.x,
                    l.position.y,
                    l.position.z,
