...
Citizen.CreateThread(
    function()
        while true do
            ...
            local g = GetVehiclePedIsIn(tXCEL.getPlayerPed(), false)
            if g ~= 0 and GetPedInVehicleSeat(g, -1) == f then
                local h, i = tXCEL.getVehicleInfos(g)
                if h == tXCEL.getUserId() then
                    if b[i] ~= nil and b[i] ~= 0 then
                        c = i
                        d = true
                        while GetVehiclePedIsIn(tXCEL.getPlayerPed(), false) ~= 0 and e == false do
                            Wait(100)
                        end
                        e = false
                        d = false
            ...
            if d then
                if not IsControlPressed(1, 71) and not IsControlPressed(1, 72) then
                    ...
                    if IsPedInAnyVehicle(j) then
                        ...
                        local n = GetVehicleCurrentRpm(GetVehiclePedIsIn(tXCEL.getPlayerPed(), false))
