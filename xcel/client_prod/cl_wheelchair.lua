RegisterCommand(
    ...
    function()
        if globalNHSOnDuty then
            local a = tXCEL.loadModel("prop_wheelchair_01")
            local b = CreateObject(a, GetEntityCoords(tXCEL.getPlayerPed()), true)
...
RegisterCommand(
    ...
    function()
        if globalNHSOnDuty then
            local b =
                GetClosestObjectOfType(GetEntityCoords(tXCEL.getPlayerPed()), 10.0, GetHashKey("prop_wheelchair_01"))
...
Citizen.CreateThread(
    function()
        while true do
            ...
            local f = tXCEL.getPlayerVehicle()
            if f == 0 then
...
function SitOnWheelchair(x)
    ...
    AttachEntityToEntity(tXCEL.getPlayerPed(), x, 0, 0, 0.0, 0.4, 0.0, 0.0, 180.0, 0.0, false, false, false, false, 2)
    local z = GetEntityHeading(x)
    while IsEntityAttachedToEntity(tXCEL.getPlayerPed(), x) do
        Citizen.Wait(5)
        if IsPedDeadOrDying(tXCEL.getPlayerPed()) then
            DetachEntity(tXCEL.getPlayerPed(), true, true)
        end
        if not IsEntityPlayingAnim(tXCEL.getPlayerPed(), "missfinale_c2leadinoutfin_c_int", "_leadin_loop2_lester", 3) then
            TaskPlayAnim(
                tXCEL.getPlayerPed(),
        ...
        if IsControlJustPressed(0, 73) or IsControlJustPressed(0, 177) then
            ...
            SetEntityCoords(tXCEL.getPlayerPed(), B, C, D)
            A = true
        end
        if A and not NetworkHasControlOfEntity(x) and GetGameTimer() - w >= 200 then
            local E = NetworkGetNetworkIdFromEntity(x)
            if E > 0 then
                TriggerServerEvent("XCEL:syncWheelchairPosition", E, GetEntityCoords(x, true), GetEntityHeading(x))
                w = GetGameTimer()
            end
        end
    end
...
function PickUpWheelchair(x)
    ...
    TriggerServerEvent("XCEL:wheelchairAttachPlayer", F)
    while not NetworkHasControlOfEntity(x) or not IsEntityAttachedToEntity(x, tXCEL.getPlayerPed()) do
        Wait(0)
    end
    LoadAnim("anim@heists@box_carry@")
    Wait(0)
    while IsEntityAttachedToEntity(x, tXCEL.getPlayerPed()) do
        Citizen.Wait(5)
        if not IsEntityPlayingAnim(tXCEL.getPlayerPed(), "anim@heists@box_carry@", "idle", 3) then
            TaskPlayAnim(
                tXCEL.getPlayerPed(),
