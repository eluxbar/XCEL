...
RegisterNetEvent(
    "XCEL:flashbangExplode",
    function(coords)
        if #(coords - tXCEL.getPlayerCoords()) <= b then
...
function func_checkForFlashbang()
    if not a then
        if GetSelectedPedWeapon(tXCEL.getPlayerPed()) == GetHashKey("WEAPON_FLASHBANG") then
            a = true
        else
            a = false
        end
    else
        if IsPedShooting(tXCEL.getPlayerPed()) and not c then
            c = true
            Citizen.CreateThread(
                function()
                    ...
                    if e ~= 0 then
                        ...
                        AddExplosion(coords, 25, 0.0, 1.0, nil, true, true)
                        TriggerServerEvent("XCEL:flashbangThrown", f)
                    end
                end
            )
            SetTimeout(
...
tXCEL.createThreadOnTick(func_checkForFlashbang)
