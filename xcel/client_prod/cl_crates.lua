...
RageUI.CreateWhile(
    ...
    true,
    function()
        if RageUI.Visible(RMenu:Get("XCEL", "crate")) then
            RageUI.DrawContent(
                    ...
                    if a and a.silver > 0 or a and a.gold > 0 or a and a.plat > 0 then
                        RageUI.List(
                            ...
                            function(f, g, h, i)
                                e = i
                                if h then
                                    TriggerServerEvent("XCEL:Crate:Redeem", d[i])
                                end
                            end
                        )
                    end
...
RegisterNetEvent("XCEL:Crate:SendData")
AddEventHandler(
    "XCEL:Crate:SendData",
    function(j, k)
        a = j
        b = k
        RageUI.Visible(RMenu:Get("XCEL", "crate"), true)
    end
)
RegisterCommand(
    "keys",
    function()
        TriggerServerEvent("XCEL:Crate:OpenMenu")
    end
)
RegisterNetEvent("XCEL:Crate:StartRedeem")
AddEventHandler(
    "XCEL:Crate:StartRedeem",
    function(l)
        c = l
        RageUI.ActuallyCloseAll()
        RageUI.Visible(RMenu:Get("XCEL", "crate"), false)
        tXCEL.StartScaleForm(c)
    end
)
function tXCEL.StartScaleForm(m)
