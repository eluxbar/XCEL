...
local function q()
    if IsControlJustReleased(0, 299) then
        if NetworkHasControlOfEntity(c) then
            TriggerServerEvent("XCEL:toggleAmbulanceDoors", tXCEL.getNetId(c, "Stretchers"))
        else
...
local function C(D, E)
    ...
    if not y(D) or b then
        ...
        tXCEL.loadAnimDict(a.positions[e].animDict)
        DetachEntity(x, false, false)
        ...
        while true do
            ...
            if d ~= nil and d ~= 0 then
                ...
                if IsEntityPlayingAnim(x, "anim@heists@box_carry@", "idle", 3) and c == 0 then
                    ...
                    if H ~= -1 and globalNHSOnDuty then
                        ...
                        if IsControlJustPressed(0, 183) then
                            ...
                            if L > 0 and M ~= 0 then
                                TriggerServerEvent("XCEL:forcePlayerOnToStretcher", L, M)
                            end
                        end
                    end
                    r(K)
                ...
                if
                 ...
                 then
                    if DecorGetInt(d, "inside_ambulance_netid") ~= 0 then
                        local O = tXCEL.getObjectId(DecorGetInt(d, "inside_ambulance_netid"), "Stretcher Thread")
...
function pushStretcher(x, d)
    Citizen.CreateThread(
        function()
            ...
            TriggerServerEvent("XCEL:stretcherAttachPlayer", P)
            AttachEntityToEntity(
                d,
                PlayerPedId(),
                PlayerPedId(),
            ...
            tXCEL.loadAnimDict("anim@heists@box_carry@")
            local Q = 0
                    ...
                    if DecorGetBool(O, "has_stretcher_inside") ~= false then
                        if not NetworkHasControlOfEntity(O) then
                            if GetGameTimer() - Q > 200 then
                                TriggerServerEvent(
                                    "XCEL:updateHasStretcherInsideDecor",
                                    tXCEL.getNetId(O, "Stretchers"),
                                    false
                                )
                                Q = GetGameTimer()
                            end
                    ...
                    if
                     ...
                     then
                        ...
                        if not NetworkHasControlOfEntity(d) then
                            TriggerServerEvent(
                                "XCEL:updateStretcherLocation",
                                tXCEL.getNetId(d, "Stretchers"),
                                tXCEL.getNetId(c, "Stretchers")
                            )
                        else
                            DecorSetInt(d, "inside_ambulance_netid", tXCEL.getNetId(c, "Stretchers"))
                        end
...
RegisterCommand(
    ...
    function()
        if globalNHSOnDuty then
            local x = tXCEL.getPlayerPed()
            local v = tXCEL.getPlayerCoords()
            local g = tXCEL.loadModel("trump-stretcher")
            local D = XCEL.spawnVehicle(g, v.x, v.y, v.z, GetEntityHeading(x), false, true, true)
            ...
            tXCEL.notify("Stretcher created.")
        end
    end,
    false
)
RegisterCommand(
    ...
    function()
        if globalNHSOnDuty then
            ...
            if V ~= nil then
                TriggerServerEvent("XCEL:removeStretcher", tXCEL.getNetId(V, "Stretchers"))
            end
        end
    end,
    false
...
RegisterNetEvent(
    "XCEL:setStretcherInside",
    function(P, W)
        local D = tXCEL.getObjectId(P, "XCEL:setStretcherInside")
        DecorSetInt(D, "inside_ambulance_netid", W)
    end
)
RegisterNetEvent(
    "XCEL:setHasStretcherInsideDecor",
    function(X, Y)
        local c = tXCEL.getObjectId(X, "XCEL:setHasStretcherInsideDecor")
        DecorSetBool(c, "has_stretcher_inside", Y)
    end
)
RegisterNetEvent(
    "XCEL:toggleAmbulanceDoorStatus",
    function(X)
        local c = tXCEL.getObjectId(X, "XCEL:toggleAmbulanceDoorStatus")
...
function tXCEL.isUsingStretcher()
    return b
end
RegisterNetEvent(
    "XCEL:forcePlayerOnToStretcher",
    function(P)
        if not NetworkDoesNetworkIdExist(P) then
            return
        end
