    ...
    if not a.InProgress and u then
        ...
        if i then
            ...
            if w ~= -1 then
                ...
                if x then
                    tXCEL.setWeapon(PlayerPedId(), foundWeapon, true)
                    a.InProgress = true
                    a.startTime = GetGameTimer()
                    a.targetSrc = w
                    TriggerServerEvent("XCEL:takeHostageSync", w)
                    a.type = "agressor"
                else
...
RegisterNetEvent(
    "XCEL:takeHostageReleaseHostage",
    function()
        a.InProgress = false
        a.type = ""
        DetachEntity(PlayerPedId(), true, false)
        ...
        while true do
            if a.type == "agressor" then
                ...
                if IsEntityDead(PlayerPedId()) then
                    ...
                    RemoveAnimDict("reaction@shove")
                    TriggerServerEvent("XCEL:takeHostageReleaseHostage", a.targetSrc)
                end
                ...
                elseif IsDisabledControlJustPressed(0, 74) and GetGameTimer() - a.startTime > 4000 then
                    ...
                    RemoveAnimDict("anim@gangops@hostage@")
                    TriggerServerEvent("XCEL:takeHostageKillHostage", a.targetSrc)
                    TriggerServerEvent("XCEL:takeHostageStop", a.targetSrc)
                    Wait(100)
...
function tXCEL.takeHostageInProgress()
    return a.InProgress
end
