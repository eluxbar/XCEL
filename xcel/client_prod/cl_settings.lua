...
local locations = {}
for k,v in pairs(module("cfg/cfg_respawn").spawnLocations) do
    table.insert(locations, k)
end
table.sort(locations,function(R, S)
    return R < S
end)
table.insert(locations, 1, "None")
Citizen.CreateThread(
    function()
        local I = GetResourceKvpString("xcel_diagonalweapons") or "false"
        if I == "false" then
            b = false
            TriggerEvent("XCEL:setVerticalWeapons")
        else
            b = true
            TriggerEvent("XCEL:setDiagonalWeapons")
        end
        local J = GetResourceKvpString("xcel_frontars") or "false"
        if J == "false" then
            c = false
            TriggerEvent("XCEL:setBackAR")
        else
            c = true
            TriggerEvent("XCEL:setFrontAR")
        end
        local K = GetResourceKvpString("xcel_hitmarkersounds") or "false"
        if K == "false" then
            d = false
            TriggerEvent("XCEL:hsSoundsOff")
        else
            d = true
            TriggerEvent("XCEL:hsSoundsOn")
        end
        local L = GetResourceKvpString("xcel_reducedchatopacity") or "false"
        if L == "false" then
            f = false
            TriggerEvent("XCEL:chatReduceOpacity", false)
        else
            f = true
            TriggerEvent("XCEL:chatReduceOpacity", true)
        end
        local M = GetResourceKvpString("xcel_hideeventannouncement") or "false"
        if M == "false" then
            g = false
        else
            g = true
        end
        local N = GetResourceKvpInt("xcel_favourite_respawn") or 1
        if N > 1 then
            favouriteRespawnLocation = N
        else
            favouriteRespawnLocation = 1
        end
        local N = GetResourceKvpString("xcel_healthpercentage") or "false"
        if N == "false" then
            h = false
        else
            h = true
        end
        local O = GetResourceKvpString("xcel_flashlightnotaiming") or "false"
        if O == "false" then
            i = false
        else
            i = true
        ...
        local P = GetResourceKvpInt("xcel_gang_name_distance")
        if P > 0 then
            j = P
            if k[j] then
                TriggerEvent("XCEL:setGangNameDistance", k[j])
            end
        end
        local Q = GetResourceKvpInt("xcel_gang_ping_sound")
        if Q > 0 then
            m = Q
        end
        local R = GetResourceKvpInt("xcel_gang_ping_volume")
        if R > 0 then
            o = R
        end
        local S = GetResourceKvpInt("xcel_gang_ping_marker")
        if S > 0 then
            q = S
        end
        local T = GetResourceKvpInt("xcel_gang_position_x")
        if T > 0 then
            r = T
            tXCEL.setGangUIXPos(s[r])
        end
        local U = GetResourceKvpInt("xcel_gang_position_y")
        if U > 0 then
            t = U
            tXCEL.setGangUIYPos(u[t])
        end
        local V = GetResourceKvpInt("xcel_doorbell_index")
        if V > 0 then
            w = V
        end
        local W = GetResourceKvpString("xcel_gang_ping_minimap") or "false"
        if W == "false" then
            gangPingMinimap = false
        else
            gangPingMinimap = true
        end
        local X = GetResourceKvpString("xcel_radio_anim") or "true"
        if X == "false" then
            x = false
        else
            x = true
        end
        local Y = GetResourceKvpString("xcel_low_props") or "false"
        if Y == "false" then
            y = false
        else
            y = true
        end
        local Z = GetResourceKvpString("xcel_frontsmgs") or "false"
        if Z == "false" then
            z = false
            TriggerEvent("XCEL:setBackSMG")
        else
            z = true
            TriggerEvent("XCEL:setFrontSMG")
        end
        local _ = GetResourceKvpInt("xcel_health_ui_type")
        if _ > 0 then
            A = _
            tXCEL.setHealthUIType(B[A])
        end
    end
)
function tXCEL.getFavouriteRespawn()
    return locations[favouriteRespawnLocation]
end
function tXCEL.setDiagonalWeaponSetting(i)
    SetResourceKvp("xcel_diagonalweapons", tostring(i))
end
function tXCEL.setFrontARSetting(i)
    SetResourceKvp("xcel_frontars", tostring(i))
end
function tXCEL.setFrontSMGSetting(i)
    SetResourceKvp("xcel_frontsmgs", tostring(i))
end
function tXCEL.setHitMarkerSetting(i)
    SetResourceKvp("xcel_hitmarkersounds", tostring(i))
end
function tXCEL.setCODHitMarkerSetting(i)
    SetResourceKvp("xcel_codhitmarkersounds", tostring(i))
end
function tXCEL.setKillListSetting(Q)
    SetResourceKvp("xcel_killlistsetting", tostring(Q))
end
function tXCEL.setOldKillfeed(Q)
    SetResourceKvp("xcel_oldkillfeed", tostring(Q))
end
function tXCEL.setDamageIndicator(Q)
    SetResourceKvp("xcel_damageindicator", tostring(Q))
end
function tXCEL.setDamageIndicatorColour(Q)
    SetResourceKvp("xcel_damageindicatorcolour", tostring(Q))
end
function tXCEL.setReducedChatOpacity(K)
    SetResourceKvp("xcel_reducedchatopacity", tostring(K))
end
function tXCEL.setHideEventAnnouncementFlag(K)
    SetResourceKvp("xcel_hideeventannouncement", tostring(K))
end
function tXCEL.getHideEventAnnouncementFlag()
    return g
end
function tXCEL.setShowHealthPercentageFlag(K)
    SetResourceKvp("xcel_healthpercentage", tostring(K))
end
function tXCEL.getShowHealthPercentageFlag()
    return h
end
function tXCEL.setFlashlightNotAimingFlag(R)
    SetFlashLightKeepOnWhileMoving(R)
    i = R
    SetResourceKvp("xcel_flashlightnotaiming", tostring(R))
end
function tXCEL.setRadioAnim(R)
    x = R
    SetResourceKvp("xcel_radio_anim", tostring(R))
end
function tXCEL.getRadioAnim()
    return x
end
function tXCEL.setLowProps(R)
    y = R
    SetResourceKvp("xcel_low_props", tostring(R))
end
function tXCEL.getGangPingSound()
    return m
end
function tXCEL.getGangPingVolume()
    return o
end
function tXCEL.getGangPingMarkerIndex()
    return q
end
function tXCEL.displayPingsOnMinimap()
    return gangPingMinimap
end
function tXCEL.getDoorbellIndex()
    return w
end
local function a0(j)
    RageUI.CloseAll()
...
local N
RegisterNetEvent(
    "XCEL:gotCustomWeaponsOwned",
    function(O)
        J = O
    end
)
RegisterNetEvent(
    "XCEL:generatedAccessCode",
    function(a2)
        M = a2
    end
)
RegisterNetEvent(
    "XCEL:getWhitelistedUsers",
    function(P)
        N = P
    end
)
...
local function _(S, T, a3)
    ...
    SetResourceKvp("xcel_graphic_presets", a4)
end
local a5 = {
    "0%",
    "5%",
...
local function ag()
    ...
    SetResourceKvp("xcel_kill_effects", ah)
end
local ai = {head = 1, body = 1, arms = 1, legs = 1}
local function aj()
    local ak = json.encode(ai)
    SetResourceKvp("xcel_blood_effects", ak)
end
Citizen.CreateThread(
    function()
        Citizen.Wait(1000)
        local a4 = GetResourceKvpString("xcel_graphic_presets")
        if a4 and a4 ~= "" then
            Q = json.decode(a4) or {}
        end
        ...
        local ah = GetResourceKvpString("xcel_kill_effects")
        if ah and ah ~= "" then
        ...
        local ak = GetResourceKvpString("xcel_blood_effects")
        if ak and ak ~= "" then
    ...
    function()
        RageUI.IsVisible(
            ...
            function()
                ...
                RMenu:Get("SettingsMenu", "graphicpacks")
                )
                RageUI.ButtonWithStyle(
                    "Kill Effects",
        ...
        RageUI.IsVisible(
            ...
            function()
                RageUI.Checkbox(
                    "Streetnames",
                    "",
                    tXCEL.isStreetnamesEnabled(),
                    {Style = RageUI.CheckboxStyle.Car},
                    function(an, ap, ao, aq)
                    end,
                    function()
                        tXCEL.setStreetnamesEnabled(true)
                    end,
                    function()
                        tXCEL.setStreetnamesEnabled(false)
                    end
                )
                RageUI.Checkbox(
                    "Compass",
                    "",
                    tXCEL.isCompassEnabled(),
                    {Style = RageUI.CheckboxStyle.Car},
                    function(an, ap, ao, aq)
                    end,
                    function()
                        tXCEL.setCompassEnabled(true)
                    end,
                    function()
                        tXCEL.setCompassEnabled(false)
                    end
                )
                local function ar()
                    tXCEL.hideUI()
                ...
                RageUI.Checkbox(
                    ...
                    function()
                        f = true
                        tXCEL.setReducedChatOpacity(true)
                        TriggerEvent("XCEL:chatReduceOpacity", true)
                    end,
                    function()
                        f = false
                        tXCEL.setReducedChatOpacity(false)
                        TriggerEvent("XCEL:chatReduceOpacity", false)
                    end
                )
                RageUI.Checkbox(
                    ...
                    function()
                        h = true
                        tXCEL.setShowHealthPercentageFlag(true)
                    end,
                    function()
                        h = false
                        tXCEL.setShowHealthPercentageFlag(false)
                    end
                )
                RageUI.ButtonWithStyle(
                    "Crosshair",
                ...
                RageUI.List("Favourite Spawn Location",locations,favouriteRespawnLocation,"Change the spawn location displayed first.",{},true,function(ae, af, ag, ah)
                    if ah ~= favouriteRespawnLocation then
                        favouriteRespawnLocation = ah
                        SetResourceKvpInt("xcel_favourite_respawn", ah)
                    end
                end)
                -- RageUI.ButtonWithStyle(
        ...
        RageUI.IsVisible(
            ...
            function()
                local function ar()
                    TriggerEvent("XCEL:setDiagonalWeapons")
                    b = true
                    tXCEL.setDiagonalWeaponSetting(b)
                end
                local function as()
                    TriggerEvent("XCEL:setVerticalWeapons")
                    b = false
                ...
                local function ar()
                    TriggerEvent("XCEL:hsSoundsOn")
                    d = true
                    tXCEL.setHitMarkerSetting(d)
                    tXCEL.notify("~y~Experimental Headshot sounds now set to " .. tostring(d))
                end
                local function as()
                    TriggerEvent("XCEL:hsSoundsOff")
                    d = false
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(an, ao, ap)
                        if ap then
                            ...
                            L = nil
                            N = nil
                            TriggerServerEvent("XCEL:getCustomWeaponsOwned")
                        end
                    end,
                    RMenu:Get("SettingsMenu", "weaponswhitelist")
                )
            ...
            function()
                RageUI.List(
                    ...
                    function(at, au, av, aw)
                        if aw ~= q then
                            q = aw
                            SetResourceKvpInt("xcel_gang_ping_marker", aw)
                        end
                    end
                )
                RageUI.List(
                    ...
                    function(at, au, av, aw)
                        if aw ~= o then
                            o = aw
                            SetResourceKvpInt("xcel_gang_ping_volume", aw)
                        end
                    end
                )
                RageUI.List(
                    ...
                    function(at, au, av, aw)
                        if aw ~= m then
                            m = aw
                            SetResourceKvpInt("xcel_gang_ping_sound", aw)
                        end
                    end
                )
                RageUI.List(
                    ...
                    function(at, au, av, aw)
                        if aw ~= j then
                            j = aw
                            SetResourceKvpInt("xcel_gang_name_distance", aw)
                            TriggerEvent("XCEL:setGangNameDistance", k[aw])
                        end
                    end
                )
                RageUI.List(
                    ...
                    function(at, au, av, aw)
                        if aw ~= r then
                            r = aw
                            tXCEL.setGangUIXPos(s[aw])
                            SetResourceKvpInt("xcel_gang_position_x", r)
                        end
                    end
                )
                RageUI.List(
                    ...
                    function(at, au, av, aw)
                        if aw ~= t then
                            t = aw
                            tXCEL.setGangUIYPos(u[aw])
                            SetResourceKvpInt("xcel_gang_position_y", t)
                        end
                    end
                )
                RageUI.Checkbox(
                    ...
                    function()
                        gangPingMinimap = true
                        SetResourceKvp("xcel_gang_ping_minimap", tostring(true))
                    end,
                    function()
                        gangPingMinimap = false
                        SetResourceKvp("xcel_gang_ping_minimap", tostring(false))
                    end
                )
                RageUI.ButtonWithStyle("Set Chat Tag","Display a custom chat tag for your gang.",{RightLabel = "→→→"},true,function(Hovered, Active, Selected)
                    if Selected then
                        local chatTag = tXCEL.KeyboardInput("Enter your chat tag.", "", 4)
                        if chatTag then
                            TriggerServerEvent("XCEL:setGangChatTag", chatTag)
                        end
                    end
                end)
            end,
        ...
        RageUI.IsVisible(
            ...
            function()
                ...
                RageUI.Checkbox(
                    ...
                    function()
                        tXCEL.setFlashlightNotAimingFlag(true)
                    end,
                    function()
                        tXCEL.setFlashlightNotAimingFlag(false)
                    end
                )
                RageUI.Checkbox(
                    ...
                    function()
                        tXCEL.setRadioAnim(true)
                    end,
                    function()
                        tXCEL.setRadioAnim(false)
                    end
                )
                RageUI.Checkbox(
                    ...
                    function()
                        g = true
                        tXCEL.setHideEventAnnouncementFlag(true)
                    end,
                    function()
                        g = false
                        tXCEL.setHideEventAnnouncementFlag(false)
                    end
                )
                RageUI.ButtonWithStyle(
                    ...
                    function(an, ao, ap)
                        if ap then
                            TriggerServerEvent("XCEL:changeLinkedDiscord")
                        end
                    end
                )
                RageUI.List(
                    ...
                    function(at, au, av, aw)
                        if aw ~= w then
                            w = aw
                            SetResourceKvpInt("xcel_doorbell_index", aw)
                        end
                    end
                )
            end,
        ...
        RageUI.IsVisible(
            ...
            function()
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(an, ao, ap)
                        if ap then
                            TriggerServerEvent("XCEL:enterDiscordCode")
                        end
                    end
                )
            end,
        ...
        RageUI.IsVisible(
            ...
            function()
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(an, ao, ap)
                        if ap then
                            local az = tXCEL.KeyboardInput("User ID of player purchasing your weapon whitelist.", "", 4)
                            if tonumber(az) then
                                az = tonumber(az)
                                if az > 0 then
                                    TriggerServerEvent("XCEL:generateWeaponAccessCode", L, az)
                                end
                            end
                        end
                    end
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(an, ao, ap)
                        if ap then
                            TriggerServerEvent("XCEL:requestWhitelistedUsers", L)
                        end
                    end,
        ...
        RageUI.IsVisible(
            ...
            function()
                ...
                RageUI.Checkbox(
                    ...
                    function()
                        tXCEL.setLowProps(true)
                    end,
                    function()
                        tXCEL.setLowProps(false)
                    end
                )
                RageUI.List(
        ...
        RageUI.IsVisible(
            ...
            function()
                ...
                RageUI.List(
                    ...
                    function(an, ao, ap, aD)
                        if ao and ae.particle ~= aD then
                            if not tXCEL.isPlusClub() and not tXCEL.isPlatClub() then
                                notify("~y~You need to be a subscriber of XCEL Plus or XCEL Platinum to use this feature.")
                                notify("~y~Available @ store.xcelstudios.com")
                            end
                            ae.particle = aD
                            ag()
                        end
                ...
                if GetGameTimer() - af > aE + 1000 then
                    tXCEL.addKillEffect(PlayerPedId(), true)
                    af = GetGameTimer()
                end
            ...
            function()
                RageUI.List(
                    ...
                    function(an, ao, ap, aD)
                        ...
                        if ap then
                            tXCEL.addBloodEffect("head", 0x796E, PlayerPedId())
                        end
                    end
                )
                RageUI.List(
                    ...
                    function(an, ao, ap, aD)
                        ...
                        if ap then
                            tXCEL.addBloodEffect("body", 0x0, PlayerPedId())
                        end
                    end
                )
                RageUI.List(
                    ...
                    function(an, ao, ap, aD)
                        ...
                        if ap then
                            tXCEL.addBloodEffect("arms", 0xBB0, PlayerPedId())
                            tXCEL.addBloodEffect("arms", 0x58B7, PlayerPedId())
                        end
                    end
                )
                RageUI.List(
                    ...
                    function(an, ao, ap, aD)
                        ...
                        if ap then
                            tXCEL.addBloodEffect("legs", 0x3FCF, PlayerPedId())
                            tXCEL.addBloodEffect("legs", 0xB3FE, PlayerPedId())
                        end
                    end
                )
            end,
...
RegisterNetEvent("XCEL:OpenSettingsMenu")
AddEventHandler(
    "XCEL:OpenSettingsMenu",
    function(aF)
        if not aF then
...
RegisterCommand(
    "opensettingsmenu",
    function()
        TriggerServerEvent("XCEL:OpenSettings")
    end
)
...
Citizen.CreateThread(
    function()
        while true do
            ...
            if not (tXCEL.getStaffLevel() > 0) then
...
function tXCEL.addKillEffect(aS, aT)
    if ae.lightning then
        ForceLightningFlash()
    end
    if ae.pedFlash then
    ...
    if ae.particle ~= 1 and (tXCEL.isPlatClub() or aT) then
        Citizen.CreateThreadNow(
            function()
                aP(aS)
            end
...
function tXCEL.addBloodEffect(aU, aV, av)
    local aW = ai[aU]
    if aW > 1 then
        local aQ = ac[aW]
        tXCEL.loadPtfx(aQ[1])
...
AddEventHandler(
    ...
    function(aX)
        ...
        if aZ then
            ...
            tXCEL.addBloodEffect(b1, aV, aX)
        end
    end
)
RegisterNetEvent("XCEL:gotDiscord")
AddEventHandler(
    "XCEL:gotDiscord",
    function()
