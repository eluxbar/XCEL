...
RegisterNetEvent("XCEL:frameworkCleanup")
AddEventHandler(
    "XCEL:frameworkCleanup",
    function()
        DeleteEntity(b)
    end
)
        ...
        RageUI.IsVisible(
            ...
            function()
                RageUI.ButtonWithStyle(
                    "Spin the wheel of XCEL (50,000 Chips)",
                    "",
                    {RightLabel = "→→→"},
                    true,
                    function(e, f, g)
                        if g then
                            TriggerServerEvent("XCEL:requestSpinLuckyWheel")
                        end
                    end
                )
            end,
...
Citizen.CreateThread(
    function()
        ...
        local i = tXCEL.loadModel("vw_prop_vw_luckywheel_02a")
        local j = tXCEL.loadModel(c)
...
function goToWheel()
    if not d then
        ...
        local D = false
        while not D do
            local E = GetEntityCoords(tXCEL.getPlayerPed())
        ...
        SetEntityHeading(tXCEL.getPlayerPed(), 10.9754)
        Wait(2000)
    ...
    function(D, E)
        if E then
            ...
            for K, L in pairs(a) do
                tXCEL.addBlip(L.entryPosition.x, L.entryPosition.y, L.entryPosition.z, 681, 0, "XCEL Wheel", 0.7, true)
                tXCEL.createArea("frwheel_" .. K, L.entryPosition, 1.5, 6, F, H, I, {})
            end
        end
    end
)
RegisterNetEvent("XCEL:syncLuckyWheel")
AddEventHandler(
    "XCEL:syncLuckyWheel",
    function(M)
        Wait(900)
...
RegisterNetEvent("XCEL:spinLuckyWheel")
AddEventHandler(
    "XCEL:spinLuckyWheel",
    function()
        goToWheel()
...
RegisterNetEvent("XCEL:spinLuckyWheelReaction")
AddEventHandler(
    "XCEL:spinLuckyWheelReaction",
    function(N)
        local y = tXCEL.getPlayerPed()
