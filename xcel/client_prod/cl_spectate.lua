...
local function r()
    return tXCEL.getStaffLevel() >= 6 or tXCEL.isDev()
end
RegisterNetEvent(
    "XCEL:spectatePlayer",
    function(s, t)
...
RegisterCommand(
    ...
    function(s, t)
        if r() then
            ...
            if u then
                TriggerServerEvent("XCEL:spectatePlayer", u)
            end
        end
    end
)
...
RegisterNetEvent(
    ...
    function()
        ...
        if #(tXCEL.getPlayerCoords() - g) > 2142.0 then
            if tXCEL.isInCayoPerico() then
...
local function z(A, B, C, D, E, F, G, H, I, J)
    tXCEL.DrawText(A - C / 2, B - D / 2 + 0.005, F, E, 0, 1, {G, H, I, J})
end
local function K(L)
...
local function aI(aJ)
    ...
    if IsControlJustPressed(0, 121) then
        TriggerServerEvent("XCEL:checkEnhancedPerms")
        if m == 0 then
            m = 1
        elseif m == 1 then
            m = 2
                ...
                if aP ~= -1 then
                    ...
                    if aJ ~= 0 then
                        ...
                        if #(GetFinalRenderedCamCoord() - g) < 2142.0 then
                            if not tXCEL.isInCayoPerico() then
                    ...
                    tXCEL.notify("~r~Couldn't spectate, person not in your zone")
                end
            end
        end
    end
...
tXCEL.createThreadOnTick(b7)
function tXCEL.isInSpectate()
    return a
end
