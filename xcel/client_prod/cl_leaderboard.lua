local a = {
    ...
    rows = {
        {"slothid", "BLUE TEAM", "", false, 5, 2, 0, "1.4"},
        {"dribble", "BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"jack", "RED TEAM", "", false, 5, 0, 2, "0.5"},
        {"slothid", "BLUE TEAM", "", false, 5, 0, 2, "1.4"},
        {"dribble", "BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"jack", "RED TEAM", "", false, 5, 2, 0, "0.5"},
        {"slothid", "BLUE TEAM", "", false, 5, 2, 0, "1.4"},
        {"dribble", "BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"jack", "RED TEAM", "", false, 5, 2, 0, "0.5"}
    }
...
RegisterCommand(
    "leaderboard",
    function(source)
        local b = tXCEL.createLeaderboard(a)
        Wait(1000)
        tXCEL.closeLeaderboard(b)
    end,
    false
)
...
function tXCEL.closeLeaderboard(b)
    c[b] = false
end

function tXCEL.createLeaderboard(m)
    local n = Scaleform("SC_LEADERBOARD")
    n.RunFunction("SET_DISPLAY_TYPE", {1})
    n.RunFunction("SET_MULTIPLAYER_TITLE", {"XCEL Event"})
...
function tXCEL.updateScoreboard(u, v, w)
    local x
    ...
    if u == -1 then
        ...
        if x then
            ...
            print("error: XCEL:updateScoreboard couldn't find row with values", u, v, w)
        end
    end
end

RegisterNetEvent(
    "XCEL:updateScoreboard",
    function(u, v, w)
        tXCEL.updateScoreboard(u, v, w)
    end
)

function tXCEL.setPlayerCanOpenLeaderboard(F)
    e = F
end

function tXCEL.clearLeaderboardData()
    d = {columns = {}, rows = {}}
end

function tXCEL.isLeaderboardSetup()
    return table.count(d.rows) > 0
end

function tXCEL.removePlayerFromLeaderboard(u)
