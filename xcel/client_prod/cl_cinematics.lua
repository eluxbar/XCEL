RMenu.Add(
    "mainmenu",
    "cinematic",
    RageUI.CreateMenu("", "", tXCEL.getRageUIMenuWidth(), tXCEL.getRageUIMenuHeight(), "new_editor", "r_editor_header")
)
RMenu:Get("mainmenu", "cinematic"):SetSubtitle("XCEL Cinematic Editor")
RMenu.Add(
    "load_scene",
    "cinematic",
...
Citizen.CreateThread(
    function()
        m = json.decode(GetResourceKvpString("xcel_scenedata") or "{}")
    end
)
RegisterNetEvent(
    "XCEL:openCinematicMenu",
    function()
        a(true)
    end
)
function tXCEL.createCinematicScene(P)
    m[P] = {}
    SetResourceKvp("xcel_scenedata", json.encode(m))
    tXCEL.notify("~g~Scene created & saved.")
end
function tXCEL.deleteCinematicScene(P)
    m[P] = nil
    SetResourceKvp("xcel_scenedata", json.encode(m))
    tXCEL.notify("~g~Scene deleted & saved.")
end
function tXCEL.createCamera(P, Q)
    if #m[P] == 0 then
        Q.transition = 100
    end
    table.insert(m[P], Q)
    ...
    SetResourceKvp("xcel_scenedata", json.encode(m))
    tXCEL.notify("~g~Camera created & saved.")
end
function tXCEL.modifyCamera(P, Q, R)
    m[P][R] = Q
    SetResourceKvp("xcel_scenedata", json.encode(m))
    tXCEL.notify("~g~Camera modified & saved.")
end
function tXCEL.deleteCamera(P, S)
    table.remove(m[P], S)
    SetResourceKvp("xcel_scenedata", json.encode(m))
    tXCEL.notify("~g~Camera deleted & saved.")
end
function tXCEL.modifyTransition(n, q, T)
    m[n][q].transition = tonumber(T)
    SetResourceKvp("xcel_scenedata", json.encode(m))
    tXCEL.notify("~g~Transition change saved.")
end
function tXCEL.modifyCameraBlending(n, q, U)
    m[n][q].blending = l[U]
    SetResourceKvp("xcel_scenedata", json.encode(m))
    tXCEL.notify("~g~Camera Blending change saved.")
end
function tXCEL.modifyScreeneffect(n, q, V)
    m[n][q].screeneffect = V
    SetResourceKvp("xcel_scenedata", json.encode(m))
    tXCEL.notify("~g~Screeneffect change saved.")
end
function tXCEL.modifyTimecycleEffect(n, q, W)
    m[n][q].timecycleEffect = W
    SetResourceKvp("xcel_scenedata", json.encode(m))
    tXCEL.notify("~g~Timecycle Effect change saved.")
end
function tXCEL.setTimecycleIntensity(n, q, f)
    m[n][q].timecycleIntensity = f
    SetResourceKvp("xcel_scenedata", json.encode(m))
    tXCEL.notify("~g~Timecycle intensity change saved.")
end
function tXCEL.modifyShakeEffect(n, q, X)
    m[n][q].shake = X
    SetResourceKvp("xcel_scenedata", json.encode(m))
    tXCEL.notify("~g~Shake change saved.")
end
function tXCEL.modifyCameraType(n, q, type)
    m[n][q].type = type
    SetResourceKvp("xcel_scenedata", json.encode(m))
    tXCEL.notify("~g~Camera type change saved.")
end
function tXCEL.addCameraFocus(n, q, Y, Z)
    m[n][q].focusType = Y
    m[n][q].focusData = Z
    SetResourceKvp("xcel_scenedata", json.encode(m))
    tXCEL.notify("~g~Follow player change saved.")
end
function tXCEL.setCameraFov(n, q)
    m[n][q].fov = i[A]
    SetResourceKvp("xcel_scenedata", json.encode(m))
    tXCEL.notify("~g~FOV change saved.")
end
                    ...
                    if q and m[n][q] then
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(a1, a2, a3)
                                if a3 then
                                    ...
                                    if tonumber(T) then
                                        tXCEL.modifyTransition(n, q, T)
                                    else
                                        tXCEL.notify("Failed to change transition time.")
                                    end
                                end
                            end,
                            RMenu:Get("camera_manager", "cinematic")
                        ...
                        RageUI.List(
                            ...
                            function(a1, a2, a3, a8)
                                if a8 ~= w then
                                    w = a8
                                    tXCEL.modifyCameraType(n, q, j[w])
                                end
                            end,
                            function()
                            end,
                        ...
                        RageUI.List(
                            ...
                            function(a1, a2, a3, a8)
                                if a8 ~= x then
                                    x = a8
                                    tXCEL.modifyCameraBlending(n, q, k[x])
                                end
                            end,
                            function()
                            end,
                            ...
                            function(a1, a2, a3, a8)
                                ...
                                if a8 ~= e then
                                    ...
                                    if m[n][q].timecycleEffect ~= "" then
                                        ...
                                        tXCEL.setTimecycleIntensity(n, q, f[e])
                                    end
                                end
                            end,
                            function()
                    ...
                    for g = 1, #c do
                        RageUI.ButtonWithStyle(
                            ...
                            function(a1, a2, a3)
                                ...
                                if a3 then
                                    tXCEL.modifyScreeneffect(n, q, c[g])
                                end
                            end,
                            RMenu:Get("camera_manager", "cinematic")
                        )
                    ...
                    for g = 1, #d do
                        RageUI.ButtonWithStyle(
                            ...
                            function(a1, a2, a3)
                                ...
                                if a3 then
                                    tXCEL.modifyTimecycleEffect(n, q, d[g])
                                end
                            end,
                            RMenu:Get("camera_manager", "cinematic")
                        )
                    ...
                    for g = 1, #h do
                        RageUI.ButtonWithStyle(
                            ...
                            function(a1, a2, a3)
                                ...
                                if a3 then
                                    tXCEL.modifyShakeEffect(n, q, h[g])
                                end
                            end,
                            RMenu:Get("camera_manager", "cinematic")
                        )
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(a1, a2, a3)
                            if a3 then
                                ...
                                if a9 ~= "" then
                                    tXCEL.addCameraFocus(n, q, "player", a9)
                                end
                            end
                        end,
                        RMenu:Get("camera_manager", "cinematic")
                            ...
                            if a3 then
                                tXCEL.clientPrompt(
                                    ...
                                    function(aa)
                                        ...
                                        tXCEL.addCameraFocus(n, q, "coord", vector3(ad, ae, af))
                                    end
                                )
                            end
                        end,
        ...
        if RageUI.Visible(RMenu:Get("weather_time_manager", "cinematic")) then
            RageUI.DrawContent(
                ...
                function()
                    ...
                    local function a4()
                        B = true
                        tXCEL.overrideTime(C[D], E[F], G[H])
                    end
                    local function a5()
                        B = false
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            if (o or p) and noclipActive and tXCEL.getPlayerVehicle() ~= 0 then
                tXCEL.notify("You may not noclip when in a vehicle.")
                tXCEL.toggleNoclip()
            end
                ...
                if a0.focusType ~= "" then
                    if a0.focusType == "player" then
                        ...
                        if aj then
                            ...
                            print("[XCEL Cinematic] Failed to point cam at player, could not get entity.")
                        end
...
Citizen.CreateThread(
    function()
        while true do
            local al, am = tXCEL.getPlayerVehicle()
            ...
            if an ~= 0 and IsEntityAPed(an) and IsPedAPlayer(an) and not IsEntityVisible(an) then
                ...
                if ao ~= -1 then
                    ...
                    if ap > 0 then
                        if not tXCEL.clientGetPlayerIsStaff(tXCEL.clientGetUserIdFromSource(ap)) then
