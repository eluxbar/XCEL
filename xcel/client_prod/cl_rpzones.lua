...
local function f()
    ...
    g.position = tXCEL.getPlayerCoords()
    g.radius = g.radius + 0.0
    g.vehicles.speed = g.vehicles.speed + 0.0
    TriggerServerEvent("XCEL:createRPZone", g)
end
...
local function k()
    ...
    if l then
        TriggerServerEvent("XCEL:removeRPZone", l)
    end
end
local p = false
RMenu.Add(
...
local function w(g, x)
    ...
    elseif not g.weapons.shotgun and y == GetHashKey("GROUP_SHOTGUN") then
        tXCEL.setWeapon(j, "WEAPON_UNARMED")
    elseif not g.weapons.xcel and y == GetHashKey("GROUP_XCEL") then
        tXCEL.setWeapon(j, "WEAPON_UNARMED")
...
local function I(g)
    local n, x = GetCurrentPedWeapon(tXCEL.getPlayerPed())
    if x ~= 0 then
        w(g, x)
    end
    local A, J = tXCEL.getPlayerVehicle()
    if A ~= 0 and J then
        C(g, A)
    end
...
tXCEL.createThreadOnTick(K)
function tXCEL.GetRPZoneInfo()
    local M = tXCEL.getPlayerCoords()
    for n, g in ipairs(d) do
        if #(M - g.position) < g.radius then
            return g
