globalBlips = {}
function tXCEL.addBlip(a, b, c, d, e, f, g, h)
...
function tXCEL.removeBlip(j)
    RemoveBlip(j)
end
function tXCEL.removeBlipAtCoords(x,y,z) -- used to remove green house blips for everyone when purchased
...
local k = {}
function tXCEL.setNamedBlip(l, a, b, c, d, e, f, g)
    tXCEL.removeNamedBlip(l)
    k[l] = tXCEL.addBlip(a, b, c, d, e, f, g)
    return k[l]
end
function tXCEL.removeNamedBlip(l)
    if k[l] ~= nil then
        tXCEL.removeBlip(k[l])
        k[l] = nil
    end
end
function tXCEL.setGPS(a, b)
    SetNewWaypoint(a + 0.0001, b + 0.0001)
end
function tXCEL.setBlipRoute(j)
    SetBlipRoute(j, true)
end
local m = {}
...
local p = {}
local q = {}
function tXCEL.addMarker(a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    local I = {
...
function tXCEL.setNamedMarker(l, a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    tXCEL.removeNamedMarker(l)
    o[l] = tXCEL.addMarker(a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    return o[l]
end
function tXCEL.removeNamedMarker(l)
    if o[l] ~= nil then
        tXCEL.removeMarker(o[l])
        o[l] = nil
    end
end
local J = {}
...
Citizen.CreateThread(
    function()
        while true do
            q = tXCEL.getNearbyMarkers()
            Citizen.Wait(10000)
        end
    end
)
function tXCEL.getNearbyMarkers()
    local N = {}
    local O = tXCEL.getPlayerCoords()
    local P = 0
...
function tXCEL.setArea(l, a, b, c, T, U)
...
function tXCEL.createArea(l, W, T, U, X, Y, Z, _)
...
function tXCEL.removeArea(l)
    if Q[l] then
        Q[l] = nil
    end
end
function tXCEL.doesAreaExist(l)
    if Q[l] then
        return true
    end
    return false
...
RegisterCommand("nextblip",function()
    ...
    a4 = tXCEL.addBlip(1103.9739990234, 211.95138549805, -49.440101623535, a5, 0, "Chips Cashier", 0.8, true)
end)
