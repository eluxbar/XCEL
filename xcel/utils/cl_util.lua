...
function tXCEL.getStreetNameAtCoord(v, w, H)
...
function tXCEL.randomNum(d, e)
    math.randomseed(GetGameTimer() * math.random() * 2)
    return math.random(d, e)
end
soundEventCode = 0
TriggerServerEvent("XCEL:soundCodeServer")
RegisterNetEvent("XCEL:soundCode",function(aH)
    soundEventCode = aH
end)
RegisterNetEvent("XCEL:playClientNuiSound",function(aI, aJ, ap) -- use for cuffing and other sounds in areas
    local ax = tXCEL.getPlayerCoords()
    if #(ax - aI) <= ap then
        SendNUIMessage({transactionType = aJ})
    end
end)
function tXCEL.getNetId(aW, aX)
    if aX == nil then
        aX = ""
    end
    local aY = 0
    ...
    if not aZ then
       -- tXCEL.debugLog(string.format("no such entity %s", aX))
    else
        aY = NetworkGetNetworkIdFromEntity(aW)
        if aY == aW then
           -- tXCEL.debugLog(string.format("no such networked entity %s", aX))
        end
    end
    return aY
end
function tXCEL.getObjectId(a_, aX)
    if aX == nil then
        aX = ""
    end
    local aL = 0
    ...
    if not b0 then
       -- tXCEL.debugLog(string.format("no object by ID %s\n%s", a_, aX))
    else
  ...
  Citizen.CreateThread(function()
    local a = module("xcel-vehicles", "cfg/cfg_garages")
...
function tXCEL.getVehicleNameFromId(aZ)
    if aT[string.lower(aZ)] then
        return aT[string.lower(aZ)].name
    end
    return ""
end
function tXCEL.getGarageNameFromId(aZ)
    return aT[string.lower(aZ)].garageType
end
function tXCEL.getVehicleIdFromModel(s)
    return aU[s]
end
local a_ = math.rad
local b0 = math.cos
