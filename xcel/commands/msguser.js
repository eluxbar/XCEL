...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    fivemexports.xcel.execute("SELECT user_id FROM `xcel_verification` WHERE discord_id = ?", [message.author.id], (adminResult) => {
        if (adminResult.length > 0) {
            const adminPermid = adminResult[0].user_id;

            fivemexports.xcel.execute("SELECT discord_id, user_id FROM `xcel_verification` WHERE user_id = ?", [userID], (result) => {
                if (result.length > 0) {
                    const discordID = result[0].discord_id;

                    fivemexports.xcel.execute("SELECT * FROM `xcel_users` WHERE id = ?", [adminPermid], (adminFetchResult) => {
                        if (adminFetchResult.length > 0) {
                            ...
                            if (targetUser) {
                                let dmEmbed = {
                                    "title": "Message From XCEL Staff Member",
