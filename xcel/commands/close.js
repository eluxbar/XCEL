...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    fivemexports.xcel.execute("SELECT * FROM cardevs WHERE userid = ?", [message.author.id], (user) => {
        ...
        if (user[0].currentreport == reportid) {
            fivemexports.xcel.execute("SELECT * FROM cardev WHERE reportid = ?", [reportid], (result) => {
                if (result.length === 0) {
                    return message.reply("Report not found.");
                }

                fivemexports.xcel.execute("UPDATE cardev SET completed = 1, notes = ? WHERE reportid = ?", [notes, reportid]);
                fivemexports.xcel.execute("UPDATE cardevs SET reportscompleted = ?, currentreport = ? WHERE userid = ?", [user[0].reportscompleted + 1, 0, message.author.id]);
