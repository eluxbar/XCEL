...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    fivemexports.xcel.getConnected([parseInt(params[0])], (connected) => {
        userConnected = connected || 'N/A';
    });

    fivemexports.xcel.execute("SELECT * FROM `xcel_user_vehicles` WHERE user_id = ?", [params[0]], (cars) => {
        fivemexports.xcel.execute("SELECT * FROM `xcel_user_data` WHERE user_id = ?", [params[0]], (userdata) => {
            fivemexports.xcel.execute("SELECT * FROM xcel_warnings WHERE user_id = ?", [params[0]], (warnings) => {
                fivemexports.xcel.execute("SELECT * FROM xcel_user_moneys WHERE user_id = ?", [params[0]], (money) => {
                    fivemexports.xcel.execute("SELECT * FROM xcel_casino_chips WHERE user_id = ?", [params[0]], (chips) => {
                        fivemexports.xcel.execute("SELECT * FROM `xcel_users` WHERE id = ?", [params[0]], (users) => {
                            fivemexports.xcel.execute("SELECT discord_id FROM `xcel_verification` WHERE user_id = ?", [params[0]], (discordid) => {
