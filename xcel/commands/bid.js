...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    fivemexports.xcel.execute("SELECT user_id FROM `xcel_verification` WHERE discord_id = ?", [user.id], (result) => {
        if (result.length > 0) {
            fivemexports.xcel.execute("SELECT * FROM xcel_user_moneys WHERE user_id = ?", [result[0].user_id], (result) => {
