...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    fivemexports.xcel.execute(
        "SELECT user_id FROM `xcel_verification` WHERE discord_id = ?",
        [message.author.id],
        (result) => {
            if (result.length === 0) {
                const embed = {
            ...
            fivemexports.xcel.execute("SELECT * FROM `xcel_stores` WHERE user_id = ?", [user_id], (itemsArray) => {
