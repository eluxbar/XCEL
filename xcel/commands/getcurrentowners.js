...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    let count = 0
    fivemexports.xcel.execute("SELECT * FROM `xcel_user_vehicles` WHERE vehicle = ?", [params[0]], (result) => {
        var owners = []
        for (i = 0; i < result.length; i++) { 
            ...
            fivemexports.xcel.execute("SELECT * FROM `xcel_users` WHERE id = ?", [user_id], (result) => {
                ...
                if (count == results){
                    let embed = {
                        ...
                        "color": 0x57f288,
                        "footer": {
                            "text": `XCEL Studios | xcelstudios.com`
                        },
                        "timestamp": new Date()
                    }
