...
exports.runcmd = async(fivemexports, client, message, params) => {
    ...
    fivemexports.xcel.execute("SELECT * FROM `xcel_user_ids` WHERE user_id = ?", [params[0]], (discordid) => {
        ...
        fivemexports.xcel.execute("SELECT * FROM xcel_user_moneys WHERE user_id = ?", [params[0]], (result) => {
            if (result) {
                bank = result[0].bank
                wallet = result[0].wallet
                fivemexports.xcel.execute("SELECT * FROM `xcel_users` WHERE id = ?", [params[0]], (userdata) => {
                    if (userdata) {
