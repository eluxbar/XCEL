...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    let count = 0
    fivemexports.xcel.execute("SELECT * FROM `xcel_user_vehicles` WHERE rentedid = ?", [params[0]], (result) => {
        if (result.length > 0) {
        var rentedvehicles = []
        ...
        for (i = 0; i < results; i++) {
            ...
            fivemexports.xcel.execute("SELECT * FROM `xcel_users` WHERE id = ?", [renter], (result) => {
                a = new Date()
