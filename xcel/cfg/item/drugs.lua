...
cocaine_sniff["Take"] = {function(player,choice)
  ...
  if user_id ~= nil then
    if XCEL.tryGetInventoryItem(user_id,"Cocaine",1) then
      XCELclient.notify(player,{"~g~Taking Cocaine."})
      TriggerEvent('XCEL:RefreshInventory', player)
      TriggerClientEvent('XCEL:cocaineEffect', player)
    end
  end
end}
...
heroin_take["Take"] = {function(player,choice)
  local user_id = XCEL.getUserId(player)
  if user_id ~= nil then
    if XCEL.tryGetInventoryItem(user_id,"Heroin",1) then
      XCELclient.notify(player,{"~g~Taking Heroin."})
      TriggerEvent('XCEL:RefreshInventory', player)
      TriggerClientEvent('XCEL:heroinEffect', player)
    end
  end
end}
...
lsd_take["Take"] = {function(player,choice)
  local user_id = XCEL.getUserId(player)
  if user_id ~= nil then
    if XCEL.tryGetInventoryItem(user_id,"LSD",1) then
      XCELclient.notify(player,{"~g~Taking LSD."})
      TriggerEvent('XCEL:RefreshInventory', player)
      TriggerClientEvent('XCEL:doAcid', player)
    end
  end
end}
...
morphine_choices["Take"] = {function(player,choice)
  local user_id = XCEL.getUserId(player)
  if user_id ~= nil then
    if XCEL.tryGetInventoryItem(user_id,"Morphine",1) then
      TriggerEvent('XCEL:RefreshInventory', player)
      TriggerClientEvent('XCEL:applyMorphine', player)
    end
  end
end}
...
taco_choices["Take"] = {function(player,choice)
  local user_id = XCEL.getUserId(player)
  if user_id ~= nil then
    if XCEL.tryGetInventoryItem(user_id,"Taco",1) then
      TriggerEvent('XCEL:RefreshInventory', player)
      TriggerClientEvent('XCEL:eatTaco', player)
    end
  end
end}
