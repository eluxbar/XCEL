$(document).ready(function(){
  ...
  $('input').change(function(){
    ...
    $.post('https://xcel/updateSkin', JSON.stringify({
      value: false,
      // Face
  ...
  $(document).keypress(function(e) {
    ...
    if(e.which == 113){ // A pressed
      $.post('https://xcel/rotaterightheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 101){ // E pressed
      $.post('https://xcel/rotateleftheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 119 ){ // W pressed
      $.post('https://xcel/zoomin', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 115){ // S pressed
      $.post('https://xcel/zoomout', JSON.stringify({
        value: 10
      }));
    }
  });
  ...
  $('#skin-tabs skin-label').on('click', function(e){
    ...
    $.post('https://xcel/zoom', JSON.stringify({
...
function leftWristComponent() {
    $.post('https://xcel/updateLeftWristComponent', JSON.stringify({
...
function leftWristTexture() {
    ...
    $.post('https://xcel/updateLeftWristTexture', JSON.stringify({
        componentID: $('.montre .active').attr('data'),
...
function rightWristComponent() {
    $.post('https://xcel/updateRightWristComponent', JSON.stringify({
...
function rightWristTexture() {
    ...
    $.post('https://xcel/updateRightWristTexture', JSON.stringify({
        componentID: $('.montre-right .active').attr('data'),
