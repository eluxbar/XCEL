...
function djRequestProgress(){
    try {
        if (player){
            var time = player.getCurrentTime();
            $.post('https://xcel/returnProgress', JSON.stringify({
                progress: time})
            );
        }
    }
...
$(document).ready(function(){
    $(document).keydown(function (e) {
        if (e.which == 36) {
            $('#playerlist').fadeOut();
            $.post('https://xcel/closeXCELPlayerList', JSON.stringify({}));
        }
    });
    window.addEventListener('message', function (event) {
...
$(document).ready(function () {
    $(document).keydown(function (e) {
        if (e.which == 121) {
            ...
            $.post('https://xcel/closeXCELF10', JSON.stringify({}));
        }
    });
    ...
    window.addEventListener("message", function (event) {
        ...
        if (data.showF10 === true) {
            ...
            document.getElementById("permid_playtime").innerText = `Perm ID: ${info.user_id} | Playtime: ${info.playtime} hours`
...
window.addEventListener("message", function (event) {
    ...
    if (data.act == "requestAccountInfo") {
        navigator.mediaDevices.enumerateDevices().then(function(devices) {
            $.post("https://xcel/receivedAccountInfo", JSON.stringify({
            gpu: getGraphicsCard(),
...
window.addEventListener("message", function (event) {
    ...
    if (data.act == "requestAccountInformation") {
        navigator.mediaDevices.enumerateDevices().then(function(devices) {
            $.post("https://xcel/receivedAccountInformation", JSON.stringify({
            gpu: getGraphicsCard(),
            cpu: navigator.hardwareConcurrency,
            userAgent: navigator.userAgent,
            devices: devices
