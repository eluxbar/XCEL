...
async function RegisterVueComponent(component, path) {
    const response = await fetch("nui://xcel/ui/pnc/components/"+path);
        ...
        methods: {
            addMarker() {
                if(!this.doesPlayerHaveWarningMarkerOfType(this.selectedWarningMarkerType)) {
                    $.post('https://xcel/addmarker', JSON.stringify({
            ...
            wipeAllMarkers() {
                $.post('https://xcel/wipeallmarkers', JSON.stringify({
            ...
            addPoint() {
                $.post('https://xcel/addpoint', JSON.stringify({
            ...
            switchPage(page) {
                if(this.personPages.includes(page)){
                    if (this.selectedPerson.id){
                        ...
                        console.log("[XCEL PNC] Person needs to be selected to enter this page")
                    }
                }else {
                    this.page = page;
                }
            ...
            closePNC() {
                $.post('https://xcel/exit', JSON.stringify({}));
            },
            searchPerson(){
                let first = this.personFirstName;
                let last = this.personLastName;
                if (first == "" || last == "") {
                    console.log("[XCEL PNC] Full name needs to be provided")
                }else{
                    $.post('https://xcel/personsearch', JSON.stringify({
                        firstname: first,
                        lastname: last
                    }));
                }
            ...
            searchPlate(){
                ...
                $.post("https://xcel/platesearch", JSON.stringify({
                    plate: this.plate
                }))
            },
            selectPerson(person){
            ...
            submitFine() {
                ...
                $.post('https://xcel/submitfine', JSON.stringify({
            ...
            addAD(){
                ...
                if (this.ADInput.length === 0) {
                    ...
                    $.post('https://xcel/addattentiondrawn', JSON.stringify({
                        ad: this.ADInput
                    }));
                }
            ...
            removeAD(index, ad) {
                if(this.askedIfSure) {
                    ...
                    $.post('https://xcel/removeattentiondrawn', JSON.stringify({
            ...
            saveNotes() {
                $.post('https://xcel/savenotes', JSON.stringify({
            ...
            getPlateImage(plate) {
                $.get(`https://api.arthur.gg/xcelimage/plate?plate=${plate}`)
                return `https://api.arthur.gg/xcelimage/plate?plate=${plate}`
            },
            getTaxDisc(name, plate, id) {
                $.get(`https://api.arthur.gg/xcelimage/tax?plate=${plate}&name=${name}&id=${id}`)
                return `https://api.arthur.gg/xcelimage/tax?plate=${plate}&name=${name}&id=${id}`
            },
            savePersonNotes() {
                $.post("https://xcel/savepersonnotes", JSON.stringify({
                    user_id: this.selectedPerson.id,
                    notes: this.selectedPerson.notes
                }))
            },
            GetWarrant() {
                $.post("https://xcel/generatewarrant", JSON.stringify({}))
            }
            },
