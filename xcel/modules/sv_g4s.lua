...
RegisterNetEvent("XCEL:toggleShiftG4S")
AddEventHandler("XCEL:toggleShiftG4S", function(Status)
    local source=source
    local user_id=XCEL.getUserId(source)
    if XCEL.hasGroup(user_id,"G4S Driver")then
        if Status then
            currentUsersOnG4SJob[user_id]={currentJob=""}
            TriggerClientEvent("XCEL:startShiftG4S",source,math.random(1000,9999))
            jobsCompleted[user_id] = 0
            Wait(10000)
            TriggerClientEvent("XCEL:updateJobInformation",source,GetCollection(user_id))
        else
            if currentUsersOnG4SJob[user_id]~=nil then
                currentUsersOnG4SJob[user_id]=nil
                jobsCompleted[user_id] = nil
                TriggerClientEvent("XCEL:endShiftG4S",source)
            end
        end
    else
        XCELclient.notify(source,{"~r~You are not clocked on as currentUsersOnG4SJob G4S Driver."})
    end
end)

RegisterNetEvent("XCEL:acceptJob")
AddEventHandler("XCEL:acceptJob",function()
    local source=source
    local user_id=XCEL.getUserId(source)
    if XCEL.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
            if k == user_id then    
                ...
                TriggerClientEvent("XCEL:updateJobInformation",source,v.currentJob)
            end
        end
    else
        XCELclient.notify(source,{"~r~You are not clocked on as currentUsersOnG4SJob G4S Driver."})
...
RegisterNetEvent("XCEL:updateMoneyInVehicle")
AddEventHandler("XCEL:updateMoneyInVehicle", function()
    local source=source
    local user_id=XCEL.getUserId(source)
    if XCEL.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
...
RegisterNetEvent("XCEL:updateMoneyOutVehicle")
AddEventHandler("XCEL:updateMoneyOutVehicle", function()
    local source=source
    local user_id=XCEL.getUserId(source)
    if XCEL.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
            ...
            if v.currentJob.jobActive then
                if XCEL.getUserSource(k)then
                    if not v.currentJob.collected then
                        if #(GetEntityCoords(GetPlayerPed(XCEL.getUserSource(k)))-v.currentJob.collectionCoords)<1.5 then
                            v.currentJob.collected=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("XCEL:updateJobInformation",XCEL.getUserSource(k),v.currentJob)
                            TriggerClientEvent("XCEL:requestMoneyInVehicle",XCEL.getUserSource(k))
                        end
                    elseif v.currentJob.moneyInVehicle and not v.currentJob.depositing and not v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(XCEL.getUserSource(k)))-v.currentJob.depositCoords)<20 then
                            v.currentJob.depositing=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("XCEL:updateJobInformation",XCEL.getUserSource(k),v.currentJob)
                            TriggerClientEvent("XCEL:requestMoneyOutVehicle",XCEL.getUserSource(k))
                        end
                    elseif not v.currentJob.deposited and v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(XCEL.getUserSource(k)))-v.currentJob.depositCoords)<1.5 then
                            v.currentJob.deposited=true
                            v.currentJob.depositing=false
                            TriggerClientEvent("XCEL:updateJobInformation",XCEL.getUserSource(k),GetCollection(k))
                            local pay = grindBoost*math.random(8000,12000)
                            XCEL.giveBankMoney(k,pay)
                        end
                    end
                end
            end
...
RegisterCommand('g4s', function(source, args)
    ...
    for k,v in pairs(currentUsersOnG4SJob) do
        if k == user_id then
            TriggerClientEvent("XCEL:openG4SMenu", source)
        end
    end
end)