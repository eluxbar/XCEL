...
RegisterNetEvent("XCEL:attemptBeginRoyalMailJob")
AddEventHandler("XCEL:attemptBeginRoyalMailJob",function()
    local source=source
    local user_id=XCEL.getUserId(source)
    local RoyalMailJobTable={}
    if XCEL.hasGroup(user_id,"Royal Mail Driver")then
        if not usersInRoyalMailJob[user_id] then
            ...
            TriggerClientEvent("XCEL:beginRoyalMailJob",source)
            Wait(100)
            TriggerClientEvent("XCEL:royalMailJobSetNextBlip",source,RoyalMailDrops[RoyalMailJobTable.stopNumber].dropPosition)
        else
            XCELclient.notify(source,{"~r~You are already a Royal Mail Driver."})
        end
    else
        XCELclient.notify(source,{"~r~You are not clocked on as a Royal Mail Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(RoyalMailDrops)do
                    if XCEL.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(XCEL.getUserSource(k)))-RoyalMailDrops[v.currentJob.stopNumber].dropPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            XCEL.giveBankMoney(k,pay)
                            TriggerClientEvent("XCEL:royalMailReachedNextStop",XCEL.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition,pay)
                            if v.currentJob.stopNumber==#RoyalMailDrops then
                                TriggerClientEvent("XCEL:royalMailJobEnd", XCEL.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInRoyalMailJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("XCEL:royalMailJobSetNextBlip",XCEL.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition)
                            end
                        end
                    end
                end
