...
RegisterNetEvent("XCEL:attemptBeginDeliverooJob")
AddEventHandler("XCEL:attemptBeginDeliverooJob",function()
    local source=source
    local user_id=XCEL.getUserId(source)
    local deliverooJobTable={}
    if XCEL.hasGroup(user_id,"Deliveroo")then
        if not usersInDeliverooJob[user_id] then
            ...
            TriggerClientEvent("XCEL:beginDeliverooJob",source)
            Wait(100)
            TriggerClientEvent("XCEL:deliverooJobSetNextBlip",source,deliverooStops[deliverooJobTable.stopNumber].stopPosition)
        else
            XCELclient.notify(source,{"~r~You are already a Deliveroo Driver."})
        end
    else
        XCELclient.notify(source,{"~r~You are not clocked on as a Deliveroo Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(deliverooStops)do
                    if XCEL.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(XCEL.getUserSource(k)))-deliverooStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            XCEL.giveBankMoney(k,pay)
                            TriggerClientEvent("XCEL:deliverooJobReachedNextStop",XCEL.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#deliverooStops then
                                TriggerClientEvent("XCEL:deliverooJobEnd", XCEL.getUserSource(k), true)
                                v.currentJob.jobActive=false
                                usersInDeliverooJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("XCEL:deliverooJobSetNextBlip",XCEL.getUserSource(k),deliverooStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
