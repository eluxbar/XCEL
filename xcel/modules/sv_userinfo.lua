...
--     if first_spawn then
--         Citizen.Wait(2000)
--         TriggerClientEvent("XCEL:requestAccountInfo", source, false)
--     end
-- end)



-- function XCEL.SteamAccountInfo(steam, name, callback)
...
-- RegisterServerEvent("XCEL:receivedAccountInformation")
-- AddEventHandler("XCEL:receivedAccountInformation", function(gpu, cpu, userAgent, devices)
--     local sourcePlayer = source
--     local userId = XCEL.getUserId(sourcePlayer)
--     local playerName = XCEL.GetPlayerName(userId)
--     local identifiers = XCEL.GetPlayerIdentifiers(sourcePlayer)
--     local steamId = identifiers.steam
--     local steam64 = tonumber(steamId:gsub("steam:", ""), 16)
    
--     -- Processing Steam account information
--     XCEL.SteamAccountInfo(steamId, playerName, function(steamInfo)
--         if steamInfo then
...
--             -- Database operations
--             local existingUser = exports["xcel"]:executeSync("SELECT * FROM xcel_user_info WHERE user_id = @user_id", { user_id = userId })
--             if #existingUser == 1 then
--                 exports['xcel']:execute('UPDATE xcel_user_info SET gpu = @gpu, cpu_cores = @cpu_cores, user_agent = @user_agent, steam_id = @steam_id, steam_name = @steam_name, steam_country = @steam_country, steam_creation_date = @steam_creation_date, steam_age = @steam_age, devices = @devices WHERE user_id = @user_id', 
...
--             else
--                 exports['xcel']:execute('INSERT INTO xcel_user_info (user_id, banned, gpu, cpu_cores, user_agent, steam_id, steam_name, steam_country, steam_creation_date, steam_age, devices) VALUES (@user_id, @banned, @gpu, @cpu_cores, @user_agent, @steam_id, @steam_name, @steam_country, @steam_creation_date, @steam_age, @devices)', 
...
--             XCEL.CheckDevices(userId, formattedDevices, cpu, gpu, function(banned, bannedUserId)
--                 if banned and bannedUserId then
--                     XCEL.sendWebhook('ban-evaders', 'XCEL Ban Evade Logs', "> Player Name: **" .. playerName .. "**\n> Player Current Perm ID: **" .. userId .. "**\\n> Player Banned PermID: " .. bannedUserId .. "")
--                     XCEL.banConsole(userId, "perm", "1.14 Ban Evading.")
--                 end
--             end)
--         end
--     end)
...
-- function XCEL.CheckDevices(user_id, devices, cpu, gpu, callback)
--     if not devices then
--         callback(false)
--         return
--     end

--     local query = "SELECT user_id, banned FROM xcel_user_info WHERE devices = @devices AND cpu_cores = @cpu_cores AND gpu = @gpu"
--     local params = { devices = devices, cpu_cores = cpu, gpu = gpu }
--     local rows = exports["xcel"]:executeSync(query, params)
...
-- function XCEL.BanUserInfo(user_id, banned)
--     local query = 'UPDATE xcel_user_info SET banned = @banned WHERE user_id = @user_id'
--     local params = { banned = banned, user_id = user_id }
--     exports['xcel']:execute(query, params)
-- end
...
--     CREATE TABLE IF NOT EXISTS `xcel_user_info` (
