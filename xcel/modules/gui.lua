...
function XCEL.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  XCELclient.openMenuData(source,{menudata})
end

-- force close player menu
function XCEL.closeMenu(source)
  XCELclient.closeMenu(source,{})
end
...
function XCEL.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  XCELclient.prompt(source,{title,default_text})
end
...
function XCEL.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  XCELclient.request(source,{id,text,time}) -- send request to client
...
function XCEL.registerMenuBuilder(name, builder)
...
function XCEL.buildMenu(name, data, cbr)
...
function XCEL.openMainMenu(source)
  XCEL.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    XCEL.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tXCEL.closeMenu(id)
...
function tXCEL.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tXCEL.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tXCEL.requestResult(id,ok)
...
function tXCEL.openMainMenu()
  XCEL.openMainMenu(source)
end
...
function XCEL.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for XCEL.addStaticMenuChoices calls
...
AddEventHandler("XCEL:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    XCELclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
