...
RegisterNetEvent("XCEL:attemptBeginBusJob")
AddEventHandler("XCEL:attemptBeginBusJob",function()
    local source=source
    local user_id=XCEL.getUserId(source)
    local BusJobTable={}
    if XCEL.hasGroup(user_id,"Bus Driver")then
        if not usersInBusJob[user_id] then
            ...
            TriggerClientEvent("XCEL:beginBusJob",source)
            Wait(100)
            TriggerClientEvent("XCEL:setNextBusJobBlip",source,busJobStops[BusJobTable.stopNumber].stopPosition)
        else
            XCELclient.notify(source,{"~r~You are already a Bus Driver."})
        end
    else
        XCELclient.notify(source,{"~r~You are not clocked on as a Bus Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(busJobStops)do
                    if XCEL.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(XCEL.getUserSource(k)))-busJobStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            XCEL.giveBankMoney(k,pay)
                            TriggerClientEvent("XCEL:nextStopReachedBusJob",XCEL.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#busJobStops then
                                TriggerClientEvent("XCEL:endBusJob", XCEL.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInBusJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("XCEL:setNextBusJobBlip",XCEL.getUserSource(k),busJobStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
