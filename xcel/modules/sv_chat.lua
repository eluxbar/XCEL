...
local function routeMessage(source, author, message, mode, fromConsole)
    if source >= 1 then
        author = exports["xcel"]:GetDiscordName(source)
    end
    ...
    if mode == "OOC" then
        TriggerEvent("XCEL:ooc", source, message)
        return
    end
    if mode == "Gang" then 
    ...
    if mode == "Anonymous" then
        TriggerEvent("XCEL:Anon", source, message)
        return
    end
    if mode == "Police" then
    ...
    if mode == "Admin" then
        for k,v in ipairs(GetPlayers()) do
            if exports['xcel']:hasPermission(v, "admin.tickets") then
                TriggerClientEvent('chatMessage', v, "^3Admin Chat | " .. author..": " , { 128, 128, 128 }, message, "ooc", "Admin")
            end
        end
        return
    end
    TriggerClientEvent('chatMessage', -1, "Twitter @"..author..":",  { 255, 255, 255 }, message, "twt", XCEL.getDiscordAvatar(XCEL.getUserId(source)))
end
local cooldown = {}
AddEventHandler('_chat:messageEntered', function(author, color, message, mode)
    ...
    if not WasEventCanceled() then
        if cooldown[source] and not (os.time() > cooldown[source]) then
            TriggerClientEvent('chatMessage', source, "XCEL",  { 255, 255, 255 }, "You are being rate limited.", "alert", mode)
            return
        else
            cooldown[source] = nil
        end
...
AddEventHandler('__cfx_internal:commandFallback', function(command)
    local name = exports["xcel"]:GetDiscordName(source)
