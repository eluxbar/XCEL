...
RegisterNetEvent('XCEL:getStoreRobBlips')
AddEventHandler('XCEL:getStoreRobBlips', function()
    TriggerClientEvent('XCEL:updateStoreRobBlips', source, stores)
end)


RegisterNetEvent('XCEL:initiateStoreRobbery')
AddEventHandler('XCEL:initiateStoreRobbery', function(store)
    local source = source
    local user_id = XCEL.getUserId(source)
    if XCEL.hasPermission(user_id, "police.armoury") then
        TriggerClientEvent('XCEL:resetStorePed', source, store)
    else
        if #XCEL.getUsersByPermission('police.armoury') > 0 then
            for k,v in pairs(stores) do
                if k == store then
                    if v.cooldown == 0 then
                        v.beingrobbed = true
                        v.cooldown = 300
                        TriggerClientEvent('XCEL:updateStoreRobBlips', -1, stores)
                        TriggerClientEvent('XCEL:beginStoreRobbingAnimations', -1, store)
                        TriggerClientEvent('XCEL:storeRobberyInProgress', source, true, store)
                        ...
                        TriggerEvent('XCEL:PDRobberyCall', source, v.storename, v.position)
                    else
            ...
            XCELclient.notify(source, {'~r~There are not enough police on duty to rob a store.'})
            TriggerClientEvent('XCEL:resetStorePed', source, store)
        end
    end
end)

RegisterNetEvent('XCEL:completeSafeCracking')
AddEventHandler('XCEL:completeSafeCracking', function(store)
    local source = source
    local user_id = XCEL.getUserId(source)
    local amount = math.random(650000, 1000000)
    TriggerClientEvent('XCEL:syncCloseSafeDoor', -1, store)
    TriggerClientEvent('XCEL:resetStorePed', -1, store)
    for k,v in pairs(stores) do
        if k == store then
            if #(GetEntityCoords(GetPlayerPed(source)) - v.position) < 10 then
                TriggerClientEvent('XCEL:giveStoreRobberyMoney', source, amount)
                v.beingrobbed = false
                XCEL.giveMoney(user_id, amount)
                XCELclient.notify(source,{"~g~Successfully Robbed £"..getMoneyStringFormatted(amount)})
                XCEL.sendWebhook("store-rob","XCEL Store Robbery","> Name: **"..XCEL.GetPlayerName(user_id).."**\n> PermID: **"..user_id.."**\n> TempID: **"..source.."**\n> Store: **"..v.storename.."**\n> Amount: **£"..getMoneyStringFormatted(amount).."**")
            else
                XCEL.ACBan(15,user_id,'XCEL:completeSafeCracking')
            end
        end
    end
    TriggerClientEvent('XCEL:updateStoreRobBlips', -1, stores)
    TriggerClientEvent('XCEL:storeRobberyInProgress', source, false, store)
end)

RegisterNetEvent('XCEL:forceEndRobbery')
AddEventHandler('XCEL:forceEndRobbery', function(store)
    local source = source
    local user_id = XCEL.getUserId(source)
    TriggerClientEvent('XCEL:resetStorePed', -1, store)
    for k,v in pairs(stores) do
...
RegisterNetEvent('XCEL:syncOpenSafeDoor')
AddEventHandler('XCEL:syncOpenSafeDoor', function(store)
    local source = source
    TriggerClientEvent('XCEL:syncOpenSafeDoor', -1, store)
end)
