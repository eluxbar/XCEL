...
AddEventHandler("XCEL:playerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
        TriggerClientEvent('XCEL:policeSyncAllDoors', source, doors, doorStates)
    end
end)

RegisterNetEvent("XCEL:policeSetDoorState")
AddEventHandler("XCEL:policeSetDoorState", function(doorHash, state)
    local source = source
    local user_id = XCEL.getUserId(source)
    if XCEL.hasPermission(user_id, 'police.armoury') then
        doorStates[doorHash] = state
        TriggerClientEvent('XCEL:policeSyncDoor', -1, doorHash, state)
    end
end)

RegisterNetEvent("XCEL:enterPoliceSyncDoors")
AddEventHandler("XCEL:enterPoliceSyncDoors", function()
    local source = source
    TriggerClientEvent('XCEL:gotAllDoorStates', source, doorStates)
end)