! function(modules) {
    ...
    const server_server = new server(function() {
        const config = JSON.parse(LoadResourceFile(GetCurrentResourceName(), "config.json")),
            configFromString = function() {
                const connectionString = GetConvar("mysql_connection_string", "mysql://root@localhost/xcel");
                let cfg = {};
    ...
    }, !0), onNet("xcel:request-data", () => {
        const src = source;
        emitNet("xcel:update-resource-data", src, server_server.profiler.profiles.resources), emitNet("xcel:update-time-data", src, server_server.profiler.profiles.executionTimes), emitNet("xcel:update-slow-queries", src, server_server.profiler.profiles.slowQueries)
    }), onNet("xcel:request-server-status", () => {
        const src = source;
        server_server.execute("SHOW GLOBAL STATUS", data => {
            emitNet("xcel:update-status", src, data)
        }, null, "xcel").then(([result, cb]) => {
            cb(result)
        }).catch(() => !1), server_server.execute("SHOW GLOBAL VARIABLES", data => {
            emitNet("xcel:update-variables", src, data)
        }, null, "xcel").then(([result, cb]) => {
            cb(result)
        }).catch(() => !1)
    })
