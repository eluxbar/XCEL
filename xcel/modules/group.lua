...
function XCEL.getUserGroups(user_id)
...
function XCEL.addUserGroup(user_id, group)
    if not XCEL.hasGroup(user_id, group) then
        local user_groups = XCEL.getUserGroups(user_id)
        ...
        if ngroup then
            ...
            TriggerEvent("XCEL:playerJoinGroup", user_id, group, gtype)
        end
    end
end
...
function XCEL.getUserGroupByType(user_id, gtype)
    local user_groups = XCEL.getUserGroups(user_id)
    for k, v in pairs(user_groups) do
        local kgroup = groups[k]
        if kgroup then
...
function XCEL.getUsersByGroup(group)
    local users = {}

    for k, v in pairs(XCEL.users) do
        if XCEL.hasGroup(tonumber(k), group) then
...
function XCEL.getUsersByPermission(perm)
    local users = {}

    for k, v in pairs(XCEL.users) do
        if XCEL.hasPermission(tonumber(k), perm) then
            table.insert(users, tonumber(k))
        end
    end
...
function XCEL.removeUserGroup(user_id, group)
    local user_groups = XCEL.getUserGroups(user_id)
    ...
    TriggerEvent("XCEL:playerLeaveGroup", user_id, group, gtype)

    user_groups[group] = nil -- remove reference
end

-- check if the user has a specific group
function XCEL.hasGroup(user_id, group)
    local user_groups = XCEL.getUserGroups(user_id)
...
-- check if the user has a specific permission
function XCEL.hasPermission(user_id, perm)
    local user_groups = XCEL.getUserGroups(user_id)
    ...
    if fchar == "@" then -- special aptitude permission
        ...
        if #parts == 3 then -- decompose group.aptitude.operator
            ...
            local afr = math.floor(XCEL.expToLevel(XCEL.getExp(user_id, group, aptitude)))

            local fop = string.sub(op, 1, 1)
            if fop == "<" then -- less (group.aptitude.<x)
                local xcel = parseInt(string.sub(op, 2, string.len(op)))
                if afr < xcel then
                    return true
                end
            elseif fop == ">" then -- greater (group.aptitude.>x)
                ...
                local xcel = parseInt(string.sub(op, 1, string.len(op)))
                if afr == xcel then
                    return true
                end
            end
    ...
    elseif fchar == "#" then -- special item permission
        ...
        if #parts == 2 then -- decompose item.operator
            ...
            local amount = XCEL.getInventoryItemAmount(user_id, item)
...
function XCEL.hasPermissions(user_id, perms)
...
exports('hasPermission', function(source, perm)
    return XCEL.hasPermission(XCEL.getUserId(source), perm)
end)

exports('hasGroup', function(source, group)
    return XCEL.hasGroup(XCEL.getUserId(source), group)
end)