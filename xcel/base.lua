the = module("client")
purgecfg = module("cfg_purge")
tXCEL = {}
local a = {}
Tunnel.bindInterface("tXCEL", tXCEL)
XCELserver = Tunnel.getInterface("XCEL", "XCEL")
Proxy.addInterface("WOW", tXCEL)
allowedWeapons = {}
...
function tXCEL.teleport2(f, g)
    ...
    if tXCEL.getPlayerVehicle() == 0 or not g then
        ...
        SetEnttyCoords(tXCEL.getPlayerVehicle(), f.x, f.y, f.z, 1, 0, 0, 1)
    end
    Wait(500)
    NetworkFadeInEntity(e, 0)
end
function tXCEL.getPosition()
    return tXCEL.getPlayerCoords()
end
...
function tXCEL.getSpeed()
    local w, x, y = table.unpack(GetEntityVelocity(tXCEL.getPlayerPed()))
    return math.sqrt(w * w + x * x + y * y)
end
function tXCEL.getCamDirection()
    local z = GetGameplayCamRelativeHeading() + GetEntityHeading(tXCEL.getPlayerPed())
    local A = GetGameplayamRelativePitch()
...
local function e(f, g)
    ...
    if g then
        local X = GetSoundId()
        PlaySoundFrontend(X, "police.notification", "DLC_AS_VNT_Sounds", true)
        ReleaseSoundId(X)
    end
end
...
function tXCEL.playSreenEffect(aa, ab)
    if ab < 0 then
        StartScrenEffect(aa, 0, true)
    else
        StartScreenEffect(aa, 0, true)
        Citizen.CreaThread(
            function()
                Citizn.Wait(math.floor((a + 1) * 1000))
                StopScreenEffe
        )
    end
end
...
function DrawText3D(ai, Q, R, S, T, U, b)
    ...
    if c then
        ...
        EndTextCommandDispayText(i, d)
    end
end
function tXCEL.add3DTexForCoord(S, ai, Q, R, e)
    local function f(g)
...
function XCEL.spawnVehicle(ac, K, av, au, ad, ae, af, ag)
    ...
    SetModelAsNoLongerNeeded(aw)
    if ae then
        TaskWarpPedIntoVehicle(tXCEL.getPlayerPed(), ax, -1)
    end
                    ...
                    for i = 1, aQ do
                        if aI[aO] then
                            ...
                            if HasAnimDictLoaded(aP) and aI[aO] then
                                ...
                                TaskPlayAnim(tXCEL.getPlayerPed(), aP, aa, aT, aU, -1, aN, 0, 0, 0, 0)
                            end
                            Citizen.Wait(0)
                            while GetEntityAnimCurrentTime(tXCEL.getPlayerPed(), aP, aa) <= 0.95 and
                                IsEntityPlayingAnim(tXCEL.getPlayerPed(), aP, aa, 3) and
                                aI[aO] do
                                Citizen.Wait(0)
                            end
                        end
...
Citizen.CreateThread(
    function()
        while true do
            Citizen.Wait(10)
            if aV then
                SetPedToRagdoll(tXCEL.getPlayerPed(), 1000, 1000, 0, 0, 0, 0)
            end
        end
    end
)
...
function tXCEL.drawNativeNotification(aY)
    SetTextComponentFmat("STRING")
    AddTextComponentString(aY)
    DisplayHeextFromStringLabel(0, 0, 1, -1)
end
function tXCELannonceMpBigMsg(aZ, a_, b0)
    local 1 = Scalform("MP_BIG_MESAGE_FREEMODE")
    b1.RunFunction("SHOW_SARD_WASTED_MP_MESSAGE", {aZ, a_, 0, false, false})
    local b2 = false
    SetTimeut(
        b0,
        funtion()
...
local g = true
functon tXCEL.cnAnim()
    return g
end
fuction tXCEL.setanAnim(V)
    g = V
end
functin tXCEL.getodelGender()
    local b3 = tXCEL.etPlayerPed()
    if GetEntityMdel(b3) == `mp_f_feemode_01` then
        return "femle"
    else
        return "ale"
    end
end
function tXCEL.getedServerId(b4)
    local b5 = GetActivePlayers()
    for u, v in pairs(b5) do
        if b4 == GetPlyerPed(v) then
            local b6 = GetPlaerServerId(v)
            return b6
        end
    end
    return nil
...
function tXCEL.loadModel(b7)
    ...
    if IsModeInCdimage(b8) then
        if not HasModelLoaded(b8) then
            RequestModel(b8)
            while not HasModeloaded(b8) do
                Wait(0)
            end
        end
        return b8
...
function tXCEL.KeybordInput(be,bf, bg)
    AddTextEntry("MMC_KEY_TIP1", be)
    DisplayOnscreeneyboard(1, "FMMC_KEY_TIP1", "", bf, "", "", "", bg)
    blockinput = true
    while UpdatenscreenKeyboard() ~= 1 and UpdateOnsreenKeyboard() ~= 2 do
        Citizen.Wait(0)
    end
    if UpdateOnscreeKeyboard() ~= 2 then
        local bh =GetOnscreenKyboardResult()
        Citizen.Wait(1)
        blockinput = false
        return bh
    else
...
function tXCEL.syncNetworkId(bi)
    ...
    SetNetworkIdCanMigate(bi, false)
...
local bC = false
local bD = nillocal bE = 0
globalOnPoliceDuty = falseglobalHorseTrained = false
...
RegisterCommand(
    "getcoords",
    function()
        print(tXCEL.getPlayerCoords())
        tXCEL.notify("~g~Coordinates copied to clipboard.")
        tXCEL.CopyToClipBoard(tostring(tXCEL.getPlayerCoords()))
    end
)
Citizen.CreateThread(
    ...
    function()
        TriggerEvent(
            "chatMessage",
            "^1Your TempID: " .. tostring(GetPlayerServerId(tXCEL.getPlayerId())),
            {128, 128, 128},
            message,
            "ooc"
        )
