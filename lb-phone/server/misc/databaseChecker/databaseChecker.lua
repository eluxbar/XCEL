...
if not Config.DatabaseChecker and not shouldGenerateTables then
...
local database = MySQL.Sync.fetchScalar("SELECT DATABASE()")
...
local function fetchTables()
	...
	local rows = exports['xcel']:executeSync("SELECT TABLE_NAME, COLUMN_NAME, DATA_TYPE, COLLATION_NAME, CHARACTER_MAXIMUM_LENGTH, IS_NULLABLE, COLUMN_DEFAULT, COLUMN_KEY FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME LIKE ?", {
