...
lib.RegisterCallback("phone:home:getOwnedHouses", function(source, cb)
    local identifier = exports['xcel']:getUserId(source)
    local formattedHouses = {}
    local houses = MySQL.Sync.fetchAll(([[SELECT * FROM xcel_user_homes WHERE user_id = @identifier]]), {["@identifier"] = identifier})
    for i = 1, #houses do
