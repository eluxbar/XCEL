...
CreateThread(function ()
    ...
    RegisterNetEvent("XCEL:initMoney", function(_, bank)
    ...
    RegisterNetEvent("XCEL:setDisplayBankMoney", function(value)
        if GetResourceState("lb-phone") == "started" then
    ...
    RegisterNUICallback("transferMonzoMoney", function(data, cb)
        if data.permId and data.moneyAmount then
            ...
            if permId and moneyAmount then
                TriggerServerEvent("XCEL:bankTransfer", permId, moneyAmount)
            end
        end
        cb("ok")
    end)
